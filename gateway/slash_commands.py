...
Module-level run.py helpers a handler needs (``_pilotage_home``,
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: pilotage gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Pilotage commands.

        Reads the adapter's ``typed_command_prefix`` capability flag
        (default "/"). Adapters whose platform blocks or reserves a typed "/"
        may declare a different prefix and rewrite it on receive; instruction
        text built for those platforms must show the prefix that actually
        works when typed.
        """
    ...
    async def _handle_kanban_command(self, event: MessageEvent) -> str:
        ...
        import re
        import shlex
        from pilotage_cli.kanban import run_slash
    ...
    async def _resume_target_allowed(
        ...
        Applies the room guard to every adapter so a caller
    ...
    async def _resume_row_visible(
        ...
        Scopes every platform to the caller's own sessions unless an admin
        passes ``--all``.
        """
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`pilotage gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _pilotage_home
        ...
        try:
            ...
            await asyncio.to_thread(
                atomic_json_write,
                _pilotage_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            await asyncio.to_thread(
                atomic_json_write,
                _pilotage_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Pilotage Agent version."""
        from pilotage_cli.slash_exec import CommandContext, execute_command
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        Supports:
          /model                              — interactive picker (Telegram) or text list
        ...
        """
        from gateway.run import _pilotage_home, _load_gateway_config
        ...
        custom_provs = None
        excluded_provs = []
        config_path = (_command_profile_home or _pilotage_home) / "config.yaml"
        try:
        ...
        # No args: show interactive picker (Telegram) or text list
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        try:
                            from pilotage_cli.context_switch_guard import (
                        ...
                        if persist_global:
                            try:
                                ...
                                if isinstance(_raw_model, dict):
                                    ...
                                    _persist_cfg["model"] = _persist_model_cfg
                                try:
                                    from pilotage_cli.route_identity import should_clear_context_pin_async
                                ...
                                from pilotage_cli.config import save_config
        ...
        # The pickers (Telegram inline keyboards, TUI, dashboard)
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Pilotage default runtime
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        All resolution/persistence goes through pilotage_cli.personality —
        ...
        # Persist the selection only — pilotage_cli.personality never writes
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        if lower.startswith("draft"):
            ...
            try:
                import asyncio
                from pilotage_cli.goals import draft_contract
            ...
            from pilotage_cli.goals import parse_contract
    ...
    async def _handle_heartbeat_command(self, event: "MessageEvent") -> str:
        ...
        """
        from pilotage_cli.heartbeat import parse_interval, format_interval, MIN_INTERVAL_SECONDS
        ...
        return (
            ...
            "elapsed. Lives while the gateway runs — use `pilotage cron` for durable schedules."
        )
    ...
    async def _handle_loop_command(self, event: "MessageEvent") -> str:
        ...
        """
        try:
            from pilotage_cli.loops import dispatch_loop_command, goal_blocks_loop_tick
        except Exception as exc:
    ...
    async def _handle_diff_command(self, event: MessageEvent) -> str:
        ...
        Pilotage itself changed. ``--stat`` limits output to the summary.
    ...
    def _save_gateway_config_key(self, key_path: str, value) -> bool:
        ...
        from gateway.run import _pilotage_home
        from pilotage_cli.config import read_user_config_raw
        config_path = _pilotage_home / "config.yaml"
        try:
    ...
    async def _handle_memory_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _pilotage_home
        from pilotage_cli.write_approval_commands import handle_pending_subcommand
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.pilotage/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``pilotage skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _pilotage_home
        from pilotage_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `pilotage skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.pilotage/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_approvals_command(self, event: MessageEvent) -> str:
        ...
        from gateway.slash_access import policy_for_source
        from pilotage_cli.approval_mode import run_approval_mode_command
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _pilotage_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _pilotage_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_save_command(self, event: MessageEvent) -> str:
        ...
        temp_dir = tempfile.mkdtemp(prefix="pilotage_save_")
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from pilotage_cli.session_listing import (
    ...
    async def _handle_reload_skills_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # Slash-command autocomplete (registered once per connect);
            ...
            # BotCommand menu, etc.) are silently
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `pilotage bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``pilotage debug share`` from the CLI.
        """
        import asyncio
        from pilotage_cli.debug import (
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Pilotage Agent to the latest version.

        Spawns ``pilotage update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``pilotage update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from gateway.run import _pilotage_home, _resolve_pilotage_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        ...
        pilotage_cmd = _resolve_pilotage_bin()
        if not pilotage_cmd:
            return t("gateway.update.pilotage_cmd_not_found")

        pending_path = _pilotage_home / ".update_pending.json"
        output_path = _pilotage_home / ".update_output.txt"
        exit_code_path = _pilotage_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        # Spawn `pilotage update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `pilotage update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # pilotage_cmd is a list of argv parts we can pass directly
                ...
                pilotage_cmd_str = " ".join(shlex.quote(part) for part in pilotage_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {pilotage_cmd_str} update --gateway"
