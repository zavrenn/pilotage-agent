...
(``~/.pilotage/plans/god-file-decomposition.md``, Phase 3 mechanical mixin lifts).
...
class GatewayAuthorizationMixin:
    ...
    def _adapter_authorization_is_upstream(
        ...
        *,
        ...
        Mirrors ``BasePlatformAdapter.authorization_is_upstream``: an adapter
        whose events were already authorized by a trusted authenticated
        upstream sets this True. Unlike ``_adapter_enforces_own_access_policy``
        (a LOCAL config policy the gateway mirrors only when it's an
        allowlist), this is an UPSTREAM decision the gateway honors directly.
    ...
    def _adapter_group_has_sender_allowlist(
        ...
        *,
        ...
        restricting which senders inside that group can invoke Pilotage. If such a
    ...
    def _is_user_authorized(
        ...
        *,
        ...
        # An adapter whose authorization is enforced by a trusted
        # authenticated upstream declares ``authorization_is_upstream=True``;
        # its events are already authorized before delivery. This is delegation
        # to a trusted upstream, NOT a fail-open: every direct network-exposed
        # adapter leaves the flag False, so the env-allowlist default-deny below
        # still applies unchanged.
        if allow_adapter_delegation and self._adapter_authorization_is_upstream(
            source.platform,
        ...
        # (pilotage gateway pairing approve / the authenticated dashboard) — an
