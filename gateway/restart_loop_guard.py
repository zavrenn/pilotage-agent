...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.pilotage.gateway")``,
...
State lives in ``<PILOTAGE_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_pilotage_home() / "gateway" / "restart_loop.json"
