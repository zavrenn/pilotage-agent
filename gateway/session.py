...
# ``PILOTAGE_AUTO_CONTINUE_FRESHNESS`` at startup.
...
def auto_continue_freshness_window() -> float:
    ...
    ``PILOTAGE_AUTO_CONTINUE_FRESHNESS`` (bridged from ``config.yaml``
    ...
    """
    raw = os.environ.get("PILOTAGE_AUTO_CONTINUE_FRESHNESS")
...
# ``sessions_dir / f"{session_id}.json"`` in pilotage_state, request-dump
...
class SessionSource:
    ...
    thread_id: Optional[str] = None  # For forum topics, threads, etc.
    chat_topic: Optional[str] = None  # Channel topic/description
    user_id_alt: Optional[str] = None  # Platform-specific stable alt ID
    chat_id_alt: Optional[str] = None  # Platform-specific alt group ID
    is_bot: bool = False  # True when the message author is a bot/webhook
    # Platform-neutral SCOPE discriminator (server / workspace).
    # Drives server/workspace isolation + the relay δ/ε/ζ gate.
...
def build_session_context_prompt(
    ...
    *,
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_pilotage_home()}/cron/output/)"
    )
...
class SessionEntry:
    ...
    # (e.g. thread-context watermarks). Survives gateway restarts via
    ...
    # session it replaced.  Lets the agent recall the prior same-channel
    # session via session_search instead of binding the request to an
...
def build_session_key(
    ...
    Group/channel rules:
      ...
      - thread_id differentiates threads within that parent chat.  When
        ...
        conversations (Telegram forum topics).
    ...
    effective_thread_id = source.thread_id
...
class SessionStore:
    ...
    def _save_sessions_json(self, data: Dict[str, Any]) -> None:
        ...
        data = {
            "_README": (
                ...
                "lives in the gateway_routing table in ~/.pilotage/state.db). "
                "Maps messaging session keys (agent:main:<platform>:...) to "
                "active session IDs. This is NOT the session list. ALL "
                "sessions (CLI, TUI, and gateway) live in ~/.pilotage/state.db "
                "and are shown by `pilotage sessions list` and `/sessions`. "
    ...
    def _find_gateway_session_row(
        ...
        *,
        ...
        ``allow_peer_fallback`` enables SessionDB's platform/chat/user
        fallback; callers that must not revive a differently scoped session
        pass False and rely on the exact session-key lookup alone.
        """
        if not self._db:
            return None
    ...
    def _append_to_transcript_serialized(
        ...
        while True:
            try:
                ...
                from pilotage_state import CompressionSessionClosedError
