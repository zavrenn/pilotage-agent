...
class WebhookAdapter(BasePlatformAdapter):
            ...
            if route.get("deliver_only"):
                ...
                if not deliver or deliver == "log":
                    raise ValueError(
                        ...
                        f"real target (telegram, whatsapp, github_comment, etc.)."
                    )
    ...
    async def _direct_deliver(
        ...
        work in agent mode work here — Telegram, WhatsApp, GitHub
        PR comments, etc.
        """
        deliver_type = delivery.get("deliver", "log")
    ...
    async def _deliver_cross_platform(
        ...
        """Route response to another platform (telegram, whatsapp, etc.)."""
        ...
        # Default adapters first; multiplex may park a platform only on a
        # secondary profile (self._profile_adapters). Fall back so webhook
        # deliver:<platform> still works when default has it disabled.
