...
There are two ways to add a platform to the Pilotage gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.pilotage/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Pilotage code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `pilotage gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `pilotage_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Pilotage core.
...
And add it to the `pilotage-gateway` composite:

```python
"pilotage-gateway": {
    "includes": [..., "pilotage-your-platform"]
}
```
...
## 12. Status Display (`pilotage_cli/status.py`)
...
## 13. Gateway Setup Wizard (`pilotage_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ pilotage_cli/ toolsets.py \
  --include="*.py" -l | sort -u
