...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.pilotage to the active profile home."""
    ...
    if raw.startswith("~/.pilotage/"):
        return pilotage_home / raw.removeprefix("~/.pilotage/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return pilotage_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under PILOTAGE_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from pilotage_constants import get_pilotage_home

    scripts_root = (get_pilotage_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.pilotage" or raw_text.startswith("~/.pilotage/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__pilotage_ignore__") is True
        ):
            return False, None
