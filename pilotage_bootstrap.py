"""Windows UTF-8 bootstrap for Pilotage entry points.
...
should be imported at the very top of every Pilotage entry point
(``pilotage``, ``pilotage-agent``, ``pilotage-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``pilotage_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``pilotage_bootstrap``, never ``pilotage_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Pilotage modules.

    Pilotage ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Pilotage source root
    ...
    """
    root = src_root or os.environ.get("PILOTAGE_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``PILOTAGE_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("PILOTAGE_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import pilotage_bootstrap``
# (or ``from pilotage_bootstrap import apply_windows_utf8_bootstrap``) at
