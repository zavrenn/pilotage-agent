#!/usr/bin/env bash
# Canonical test runner for pilotage-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.pilotage/...)
#
# Usage:
...
# (PILOTAGE_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.pilotage/pilotage-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.pilotage/pilotage-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${PILOTAGE_PYTHON:-}" ] && [ -x "$PILOTAGE_PYTHON" ] \
    && "$PILOTAGE_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: PILOTAGE_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `pilotage` binary rather
  # than the devShell hook.
  PYTHON="$PILOTAGE_PYTHON"
  echo "▶ no local venv — using Nix dev venv via PILOTAGE_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and PILOTAGE_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.pilotage/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.pilotage"
...
# Windows (issues,). These are location variables, not
...
#
#   * PILOTAGE_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * PILOTAGE_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     PILOTAGE_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# PILOTAGE_RUN_SLOW_PET_TESTS / PILOTAGE_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no PILOTAGE_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in PILOTAGE_TEST_IMAGE PILOTAGE_TEST_WORKERS PILOTAGE_TEST_PATHS \
  PILOTAGE_TEST_FILE_TIMEOUT PILOTAGE_TEST_FILE_RETRIES PILOTAGE_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${PILOTAGE_RUN_SLOW_PET_TESTS:+PILOTAGE_RUN_SLOW_PET_TESTS="$PILOTAGE_RUN_SLOW_PET_TESTS"} \
  ${PILOTAGE_E2E_BROWSER:+PILOTAGE_E2E_BROWSER="$PILOTAGE_E2E_BROWSER"} \
