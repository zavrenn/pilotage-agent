#!/usr/bin/env node
/**
 * Pilotage Agent WhatsApp Bridge
 *
 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.pilotage/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.pilotage', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (PILOTAGE_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.PILOTAGE_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.pilotage', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.PILOTAGE_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.pilotage', 'document_cache');
const AUDIO_CACHE_DIR = process.env.PILOTAGE_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.pilotage', 'audio_cache');
...
// `pilotage update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *Pilotage Agent*\n────────────\n';
...
function enqueuePollUpdateEvent({ key, update, selectedOptions, aggregation }) {
  ...
  const pollId = key?.id
    ...
    || '';
  // Only surface votes on polls Pilotage itself created (tracked when
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into Pilotage and trigger
      ...
      // Ignore Pilotage' own reply messages in self-chat mode to avoid loops.
    ...
    switch (type) {
      case 'image':
        if (ext === 'gif') {
          ...
          let tmpGifMp4 = null;
          try {
            tmpGifMp4 = path.join(tmpdir(), `pilotage_gif_${randomBytes(6).toString('hex')}.mp4`);
            execFileSync(
              'ffmpeg',
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `pilotage_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execFileSync(
              'ffmpeg',
