...
# Python-driven tool session (e.g. Pilotage terminal tool). A pre-set PYTHONPATH
...
if [ -n "${PILOTAGE_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$PILOTAGE_INSTALL_DIR"
...
#   code at /usr/local/lib/pilotage-agent, command at /usr/local/bin/pilotage,
#   data still at /root/.pilotage (PILOTAGE_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$PILOTAGE_HOME/.no-bundled-skills so future"
            echo "                   'pilotage update' runs never inject bundled skills either"
            ...
            echo "                   default (non-root):  ~/.pilotage/pilotage-agent"
            echo "                   default (root, Linux): /usr/local/lib/pilotage-agent"
            echo "  --pilotage-home PATH  Data directory (default: ~/.pilotage, or \$PILOTAGE_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Pilotage installs the code under"
            echo "  /usr/local/lib/pilotage-agent and links the command into"
            echo "  /usr/local/bin/pilotage (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$PILOTAGE_HOME"
            echo "  (default /root/.pilotage).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$PILOTAGE_HOME/pilotage-agent are preserved in-place."
...
# intentional, but they leave the checkout dirty — which forces `pilotage update`
...
emit_manifest() {
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Pilotage Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install pilotage command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `pilotage` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $PILOTAGE_HOME/pilotage-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $PILOTAGE_HOME/pilotage-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/pilotage-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $PILOTAGE_HOME/pilotage-agent — then preserve it)
#
# Always no-op when the user set --dir or $PILOTAGE_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in PILOTAGE_HOME.
    if is_termux; then
        INSTALL_DIR="$PILOTAGE_HOME/pilotage-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$PILOTAGE_HOME/pilotage-agent/.git" ]; then
            INSTALL_DIR="$PILOTAGE_HOME/pilotage-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/pilotage-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/pilotage-agent"
        ...
        log_info "  Command: /usr/local/bin/pilotage"
        log_info "  Data:    $PILOTAGE_HOME (unchanged)"
...
# Point a Pilotage-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $PILOTAGE_HOME/node/bin — which is NOT on
...
# Pilotage's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Pilotage-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$PILOTAGE_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$PILOTAGE_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$PILOTAGE_HOME/node/etc/npmrc"
}

get_pilotage_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/pilotage" ]; then
        echo "$link_dir/pilotage"
    else
        echo "pilotage"
    fi
}
...
install_uv() {
    ...
    log_info "Installing managed uv into $PILOTAGE_HOME/bin ..."
    mkdir -p "$PILOTAGE_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/pilotage-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/pilotage-uv-installer.$$.sh")"
    ...
    # directly into $PILOTAGE_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$PILOTAGE_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# with the Pilotage-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Pilotage-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Pilotage-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Pilotage-managed Node from a previous run over a too-old system one.
    if [ -x "$PILOTAGE_HOME/node/bin/node" ] && [ -x "$PILOTAGE_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$PILOTAGE_HOME/node/bin/node" --version)"; then
        export PATH="$PILOTAGE_HOME/node/bin:$PATH"
        log_success "Node.js $("$PILOTAGE_HOME/node/bin/node" --version) found (Pilotage-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Pilotage-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old (Pilotage requires Node >=26) — installing Pilotage-managed Node $NODE_VERSION..."
...
install_node() {
    ...
    log_info "Extracting to ~/.pilotage/node/..."
    ...
    # Place into ~/.pilotage/node/ and symlink binaries into the same bin dir
    # the pilotage command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$PILOTAGE_HOME/node"
    mkdir -p "$PILOTAGE_HOME"
    mv "$extracted_dir" "$PILOTAGE_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$PILOTAGE_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$PILOTAGE_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$PILOTAGE_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$PILOTAGE_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.pilotage/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Pilotage install may complete, but web search and dependency downloads can fail."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                local stash_name
                stash_name="pilotage-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``pilotage update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Pilotage behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up pilotage command..."

    if [ "$USE_VENV" = true ]; then
        PILOTAGE_BIN="$INSTALL_DIR/venv/bin/python"
        PILOTAGE_ENTRYPOINT="$INSTALL_DIR/pilotage"
    else
        PILOTAGE_BIN="$(which pilotage 2>/dev/null || echo "")"
        if [ -z "$PILOTAGE_BIN" ]; then
            log_warn "pilotage not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$PILOTAGE_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$PILOTAGE_ENTRYPOINT" ]; }; then
        log_warn "Pilotage launcher prerequisites not found"
    ...
    # Create a user-facing shim for the pilotage command.
    ...
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/pilotage" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$PILOTAGE_BIN" "$PILOTAGE_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/pilotage" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/pilotage"
    log_success "Installed pilotage launcher → $command_link_display_dir/pilotage"
...
unset PYTHONPATH
unset PYTHONHOME
exec "$PILOTAGE_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/pilotage-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$PILOTAGE_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/pilotage-agent"
    log_success "Installed pilotage-agent launcher → $command_link_display_dir/pilotage-agent"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "pilotage command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v pilotage' \
                >/dev/null 2>&1; then
        ...
        log_info "pilotage not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Pilotage Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Pilotage Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Pilotage Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so pilotage works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.pilotage directory structure (config at top level, code in subdir)
    mkdir -p "$PILOTAGE_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.pilotage/.env (top level, easy to find)
    if [ ! -f "$PILOTAGE_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$PILOTAGE_HOME/.env"
            log_success "Created ~/.pilotage/.env from template"
        else
            touch "$PILOTAGE_HOME/.env"
            log_success "Created ~/.pilotage/.env"
        fi
    else
        log_info "~/.pilotage/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$PILOTAGE_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.pilotage/config.yaml (top level, easy to find)
    if [ ! -f "$PILOTAGE_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$PILOTAGE_HOME/config.yaml"
            log_success "Created ~/.pilotage/config.yaml from template"
        fi
    else
        log_info "~/.pilotage/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in pilotage_cli/default_soul.py — the
    ...
    if [ ! -f "$PILOTAGE_HOME/SOUL.md" ]; then
        cat > "$PILOTAGE_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.pilotage/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.pilotage/"

    # Seed bundled skills into ~/.pilotage/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `pilotage update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            > "$PILOTAGE_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $PILOTAGE_HOME/.no-bundled-skills"
        log_info "  Future 'pilotage update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.pilotage/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.pilotage/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$PILOTAGE_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$PILOTAGE_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.pilotage/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$PILOTAGE_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Pilotage Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Pilotage will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Pilotage Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    # Keep the checkout clean so `pilotage update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `pilotage tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$PILOTAGE_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside Pilotage' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$PILOTAGE_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'pilotage tools')"
    fi
}
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'pilotage setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m pilotage_cli.main setup < /dev/tty
    else
        python -m pilotage_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$PILOTAGE_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Pilotage to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$PILOTAGE_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'pilotage whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                PILOTAGE_CMD="$(get_pilotage_command_path)"
                $PILOTAGE_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'pilotage whatsapp' to pair."
        fi
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $PILOTAGE_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $PILOTAGE_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: pilotage gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: pilotage gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $PILOTAGE_CMD gateway > "$PILOTAGE_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.pilotage/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: pilotage gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: pilotage gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $PILOTAGE_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $PILOTAGE_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $PILOTAGE_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}pilotage${NC}              Start chatting"
    echo -e "   ${GREEN}pilotage setup${NC}        Configure API keys & settings"
    ...
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'pilotage' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'pilotage' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'pilotage' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$PILOTAGE_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$PILOTAGE_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$PILOTAGE_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        complete)
            ...
            # $PILOTAGE_HOME. $PILOTAGE_HOME is a shared data dir (it can be
            # bind-mounted into a Docker gateway too), so a stamp there gets
            # clobbered by the container's 'docker' stamp and wrongly blocks
            # 'pilotage update' on this host install. See detect_install_method().
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $PILOTAGE_HOME.
    # $PILOTAGE_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'pilotage update' on this host install.
