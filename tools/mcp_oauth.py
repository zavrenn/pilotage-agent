...
This module provides the glue:
    - ``PilotageTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Pilotage Agent"
"""
...
_USER_SKIPPED_SENTINEL = "__pilotage_user_skipped__"
...
def _get_token_dir(pilotage_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses PILOTAGE_HOME so each profile gets its own OAuth tokens.
    Layout: ``PILOTAGE_HOME/mcp-tokens/``
    """
    from pilotage_constants import get_pilotage_home

    base = Path(pilotage_home) if pilotage_home is not None else Path(get_pilotage_home())
...
def _cached_redirect_port(storage: "PilotageTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Pilotage restarts and chooses a
...
def _cached_redirect_uri(storage: "PilotageTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `pilotage mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# PilotageTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class PilotageTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        PILOTAGE_HOME/mcp-tokens/<server_name>.json         -- tokens
        PILOTAGE_HOME/mcp-tokens/<server_name>.client.json   -- client info
        PILOTAGE_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """

    def __init__(self, server_name: str, *, pilotage_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._pilotage_home = Path(pilotage_home) if pilotage_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._pilotage_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._pilotage_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._pilotage_home) / f"{self._server_name}.meta.json"
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # Pilotage records an absolute wall-clock ``expires_at`` alongside the
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._pilotage_home)
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Pilotage.</p></body></html>"
            ) if code else (
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `pilotage mcp login <server>` later to "
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = PilotageTokenStorage(server_name, pilotage_home=pilotage_home)
    storage.remove()
...
def _configure_callback_port(
    cfg: dict,
    storage: "PilotageTokenStorage | None" = None,
) -> int:
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "Pilotage Agent" / "Pilotage" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "Pilotage Agent")
    scope = cfg.get("scope")
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: pilotage mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Pilotage auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Pilotage is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    storage = PilotageTokenStorage(server_name)
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`pilotage mcp login {server_name}` interactively first to complete "
