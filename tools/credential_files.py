...
def _resolve_pilotage_home() -> Path:
...
def register_credential_file(
    ...
    *relative_path* is relative to ``PILOTAGE_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside PILOTAGE_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.

    Containment alone is not sufficient, because PILOTAGE_HOME is exactly where
    ...
    """
    pilotage_home = _resolve_pilotage_home()

    # Reject absolute paths — they bypass the PILOTAGE_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to PILOTAGE_HOME)",
            relative_path,
        )
        return False

    host_path = pilotage_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape PILOTAGE_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, pilotage_home)
    ...
    # inside PILOTAGE_HOME and therefore pass the containment check above.
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.pilotage/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="pilotage-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching pilotage_constants.get_pilotage_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_pilotage_dir()`` for backward compatibility with old directory layouts.
    """
    from pilotage_constants import get_pilotage_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_pilotage_dir(new_subpath, old_name)
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.pilotage``,
    SSH ``<remote_home>/.pilotage``, etc.) and whether translation is wanted.
