...
it is automatically downloaded from GitHub releases to $PILOTAGE_HOME/bin/tirith.
...
def _get_pilotage_home() -> str:
    """Return the Pilotage home directory, respecting PILOTAGE_HOME env var."""
    return str(get_pilotage_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_pilotage_home(), ".tirith-install-failed")
...
def _pilotage_bin_dir() -> str:
    """Return $PILOTAGE_HOME/bin, creating it if needed."""
    d = os.path.join(_get_pilotage_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $PILOTAGE_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_pilotage_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $PILOTAGE_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $PILOTAGE_HOME/bin/tirith
    ...
    pilotage_bin = os.path.join(_pilotage_bin_dir(), "tirith")
    if os.path.isfile(pilotage_bin) and os.access(pilotage_bin, os.X_OK):
        _resolved_path = pilotage_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return pilotage_bin
