...
def _expand_tilde(path: str) -> str:
    ...
    if not path or "~" not in path:
        return path
    try:
        from pilotage_constants import get_subprocess_home
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as Pilotage.
...
_pilotage_config_resolved: str | None = None
_pilotage_config_resolved_loaded = False


def _get_pilotage_config_resolved() -> str | None:
    """Return the resolved absolute path of the Pilotage config file (cached)."""
    global _pilotage_config_resolved, _pilotage_config_resolved_loaded
    if _pilotage_config_resolved_loaded:
        return _pilotage_config_resolved
    _pilotage_config_resolved_loaded = True
    try:
        from pilotage_cli.config import get_config_path
        _pilotage_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _pilotage_config_resolved = str(Path(_expand_tilde("~/.pilotage/config.yaml")).resolve())
        except Exception:
            _pilotage_config_resolved = None
    return _pilotage_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Pilotage config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    pilotage_config = _get_pilotage_config_resolved()
    if pilotage_config and (resolved == pilotage_config or normalized == pilotage_config):
        return (
            f"Refusing to write to Pilotage config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.pilotage/config.yaml directly or use 'pilotage config' instead."
        )
    return None
...
# .cursorrules (or a project-local .pilotage config tree) outlives the current
...
_real_pilotage_home_cached: str | None = None
_real_pilotage_home_loaded = False


def _get_real_pilotage_home() -> str | None:
    """Return the realpath of the authoritative Pilotage home (cached)."""
    global _real_pilotage_home_cached, _real_pilotage_home_loaded
    if _real_pilotage_home_loaded:
        return _real_pilotage_home_cached
    _real_pilotage_home_loaded = True
    try:
        from pilotage_constants import get_pilotage_home
        _real_pilotage_home_cached = os.path.realpath(str(get_pilotage_home()))
    except Exception:
        try:
            _real_pilotage_home_cached = os.path.realpath(_expand_tilde("~/.pilotage"))
        except Exception:
            _real_pilotage_home_cached = None
    return _real_pilotage_home_cached
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    """
    try:
        from pilotage_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # The authoritative ~/.pilotage home is governed by its own guards
    ...
    # ``.pilotage`` component rule below, which would otherwise match the
    # home directory itself.
    real_home = _get_real_pilotage_home()
    ...
    for candidate in (normalized, resolved):
        ...
        for pattern in extra_patterns:
            if fnmatch.fnmatch(base_lower, pattern.lower()):
                return base
        # Project-local .pilotage config dirs (e.g. <repo>/.pilotage/config.yaml)
        # are loaded as project context and steer behavior the same way.
        # Scope: the file's IMMEDIATE parent must be ``.pilotage`` — matching
        # any ancestor named .pilotage would gate every write inside a
        # checkout that happens to live under ~/.pilotage (e.g. the
        # pilotage-agent repo itself at ~/.pilotage/pilotage-agent).
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".pilotage":
            return candidate
    return None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    * container-mirror (#32049 follow-up) — writes from inside a Docker
    ...
    # in a session that cd'd into ``~/.pilotage/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Pilotage internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under PILOTAGE_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == PILOTAGE_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Pilotage-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Pilotage-profile guard for
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Pilotage profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Pilotage profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with pilotage_tools.write_file() for very "
            "large files."
        )
