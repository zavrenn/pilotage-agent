...
def auth_json_path():
    """Return the Pilotage auth store path, respecting PILOTAGE_HOME overrides."""
    return get_pilotage_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`pilotage tools`, banner/status paint, provider
...
def read_nous_access_token() -> Optional[str]:
    ...
    try:
        from pilotage_cli.auth import resolve_nous_access_token
...
# managed vendor's gateway URL is pinned: adding one is a Pilotage release, and
