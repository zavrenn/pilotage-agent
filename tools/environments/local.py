...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Pilotage was already launched from (for example ``pilotage-agent``
    while the process cwd is ``~/.pilotage/pilotage-agent``), passing it through
    unchanged makes the wrapper run ``cd pilotage-agent`` *inside* the project
    ...
    # Common recovery for config values like ``pilotage-agent`` when Pilotage was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./pilotage-agent``; use the current directory instead.
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from pilotage_cli.config import OPTIONAL_ENV_VARS
    ...
    blocked.update({
        ...
        "PILOTAGE_DASHBOARD_SESSION_TOKEN",
...
_PILOTAGE_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# project's dependencies into the Pilotage venv path instead of the project's own
...
def _is_pilotage_internal_secret(key: str) -> bool:
    """Return True for Pilotage-internal secrets injected under *dynamic* names.

    ``_PILOTAGE_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    This is the single source of truth for "Pilotage-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`pilotage_subprocess_env` helper all call it, so the
...
def _inject_context_pilotage_home(env: dict) -> None:
    """Bridge the context-local Pilotage home override into subprocess env."""
    try:
        from pilotage_constants import get_pilotage_home_override

        value = get_pilotage_home_override()
        if value:
            env["PILOTAGE_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (PILOTAGE_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Pilotage-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_PILOTAGE_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_pilotage_internal_secret(key):
            continue
        passthrough = _is_passthrough(key)
        if key in _PILOTAGE_PROVIDER_ENV_BLOCKLIST and not passthrough:
            continue
    ...
    for key, value in (extra_env or {}).items():
        if key.startswith(_PILOTAGE_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_PILOTAGE_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_pilotage_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_pilotage_internal_secret(key):
            continue
        else:
            passthrough = _is_passthrough(key)
            if key in _PILOTAGE_PROVIDER_ENV_BLOCKLIST and not passthrough:
    ...
    _inject_context_pilotage_home(sanitized)
...
# narrow subset of _PILOTAGE_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in pilotage_subprocess_env().
...
def pilotage_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_PILOTAGE_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Pilotage spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_PILOTAGE_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and Pilotage-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_pilotage_internal_secret`.
    for key in list(env):
        if key.startswith(_PILOTAGE_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_pilotage_internal_secret(key):
    ...
    # copies os.environ, whose PILOTAGE_SESSION_* mirror is a last-writer-wins
...
def build_subprocess_env(
    ...
    *,
    ...
    function (or :func:`pilotage_subprocess_env` for the model-driving-CLI
    surface) instead of copying ``os.environ`` directly, so profile-home
    propagation (``PILOTAGE_HOME`` / subprocess ``HOME`` contract) and the
    Pilotage secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Pilotage home override into ``PILOTAGE_HOME`` and apply the
      subprocess HOME contract (``pilotage_constants.apply_subprocess_home_env``).
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``PILOTAGE_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs PILOTAGE_HOME override
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("PILOTAGE_GIT_BASH_PATH")
    if custom and os.path.isfile(custom):
        candidates.append(custom)

    # Prefer our own portable Git install — a broken or partially-uninstalled
    # system Git (or a stale PILOTAGE_GIT_BASH_PATH pointing at one) must not
    ...
    #   PortableGit: %LOCALAPPDATA%\pilotage\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\pilotage\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _pilotage_portable_git = os.path.join(_local_appdata, "pilotage", "git") if _local_appdata else ""
    if _pilotage_portable_git:
        for candidate in (
            os.path.join(_pilotage_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_pilotage_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    # PILOTAGE_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    # (``Directory \\drivers\\etc does not exist``) must not win over a
    # healthy portable Git under %LOCALAPPDATA%\\pilotage\\git.
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "PILOTAGE_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
    ...
    raise RuntimeError(
        "Git Bash not found. Pilotage Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set PILOTAGE_GIT_BASH_PATH to your bash.exe location."
    )
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart Pilotage. If the override is blocked or later re-applied, "
...
# Cached directory containing the ``pilotage`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_PILOTAGE_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_pilotage_bin_dir() -> str | None:
    """Return the directory holding the ``pilotage`` console-script, or None.
    ...
    pilotage install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``pilotage``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``pilotage`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``pilotage`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("pilotage")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``pilotage`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _PILOTAGE_BIN_DIR
    if _PILOTAGE_BIN_DIR is not _SENTINEL:
        return _PILOTAGE_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("pilotage")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "pilotage" or base.startswith("pilotage."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "pilotage.exe" if _IS_WINDOWS else "pilotage"
    ...
    _PILOTAGE_BIN_DIR = candidate
    return candidate


def _prepend_pilotage_bin_dir(existing_path: str) -> str:
    """Prepend the pilotage install dir to ``existing_path`` if it's missing.
    ...
    """
    bin_dir = _resolve_pilotage_bin_dir()
    if not bin_dir:
        return existing_path
    sep = os.pathsep
    entries = [e for e in existing_path.split(sep) if e] if existing_path else []
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing Pilotage-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes Pilotage installs for
    itself, so on a machine where Pilotage provisioned its own toolchain a
    command the agent runs resolves a system copy instead — or nothing at all:

    - ``$PILOTAGE_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
      browser toolchain. ``tools/browser_tool.py`` already does this for its own
      subprocesses; the agent's shell deserves the same.
    - ``$PILOTAGE_HOME/bin`` — the managed ``uv``. ``install.sh`` writes it there
    ...
    ``get_pilotage_home()`` is profile-scoped and a managed tree can appear
    mid-process (``heal_pilotage_managed_node``, a first browser install).
    """
    try:
        from pilotage_constants import get_pilotage_home, iter_pilotage_node_dirs

        candidates = [*iter_pilotage_node_dirs(), get_pilotage_home() / "bin"]
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    Pilotage-managed runtime dirs are appended alongside the sane entries, not
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_PILOTAGE_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_PILOTAGE_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_pilotage_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_pilotage_internal_secret(k):
            continue
        else:
            passthrough = _is_passthrough(k)
            if k in _PILOTAGE_PROVIDER_ENV_BLOCKLIST and not passthrough:
                continue
    ...
    if path_key is not None:
        ...
        # Ensure the pilotage install dir is reachable so plugins can shell out
        # to bare ``pilotage`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_pilotage_bin_dir(new_path)

    _inject_context_pilotage_home(run_env)
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Pilotage trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``PILOTAGE_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under PILOTAGE_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_pilotage_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "pilotage_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._pilotage_pgid = os.getpgid(proc.pid)
