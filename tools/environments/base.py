"""Base class for all Pilotage execution environment backends.
...
# PILOTAGE_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("PILOTAGE_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {PILOTAGE_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_pilotage_home() / "sandboxes"
...
def _pipe_stdin(proc: subprocess.Popen, data: str) -> None:
    ...
    recorded on ``proc._pilotage_stdin_errors`` while stdin is still closed
    ...
    proc._pilotage_stdin_errors = errors
    ...
    proc._pilotage_stdin_thread = thread
    thread.start()
...
def _cwd_marker(session_id: str) -> str:
    return f"__PILOTAGE_CWD_{session_id}__"
...
# the FIRST session's PILOTAGE_SESSION_ID into the snapshot makes every LATER
...
# PILOTAGE_SESSION_ID leak via the shared snapshot). Stripping them from the
...
# set), not Pilotage' per-turn session identity.
#
...
def _export_dump_excluding_session_vars(
    ...
    return (
        "{ ( "
        "unset ${!PILOTAGE_SESSION_*} ${!PILOTAGE_CRON_AUTO_DELIVER_*} "
        # AI_AGENT / PILOTAGE_AGENT are per-command attribution markers
        ...
        "AI_AGENT PILOTAGE_AGENT "
        f"PILOTAGE_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; "
        ") || true; } "
        f"> {tmp_path}"
    )
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Pilotage backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._cwd_file = f"{temp_dir}/pilotage-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        _snap_tmp = '"$__pilotage_snap_tmp"'
        snapshot_excluded = self._snapshot_excluded_passthrough_names()
        bootstrap = (
            f"umask 077\n"
            f"__pilotage_snap_tmp=$(mktemp {_snap_tmp_template}) || exit 1\n"
            ...
            f"__pilotage_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__pilotage_fns\" ] && declare -f $__pilotage_fns "
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        for name in passthrough_names:
            marker = f"_PILOTAGE_RUNTIME_PASSTHROUGH_{name}"
        ...
        # under Pilotage via the cross-agent ``AI_AGENT`` standard (read by e.g.
        # huggingface_hub's agent detection) plus the Pilotage-specific
        # ``PILOTAGE_AGENT`` marker.  The value MUST equal our id in the public
        # agent-harness registry (``pilotage-agent`` — see huggingface.js
        ...
        # inherited from the Pilotage process.  ``${VAR:-default}`` semantics:
        # never clobber an outer harness value that arrived via the inherited
        # process env (Pilotage running inside another agent's terminal).
        parts.append(
            'export AI_AGENT="${AI_AGENT:-pilotage-agent}" '
            'PILOTAGE_AGENT="${PILOTAGE_AGENT:-true}"'
        )
        ...
        # Restrict Pilotage metadata files without changing the user's command
        ...
        if self._snapshot_ready:
            parts.append(
                f"__pilotage_snap_tmp=$(mktemp {_snap_tmp_template}) && "
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"PILOTAGE_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        if bounded_capture:
            ...
            try:
                spill_dir = get_pilotage_home() / "cache" / "terminal-output"
        ...
        # --- Debug tracing (opt-in via PILOTAGE_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __PILOTAGE_CWD_{session}__ marker from stdout output.
