...
transport, discovers their tools, and registers them into the pilotage-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.pilotage/config.yaml under the ``mcp_servers`` key.
...
# blew past the gateway's 15s startup budget. We run it off the loop
...
# handshake defeats the inner timeout (the failure mode).
...
# per-profile log file (~/.pilotage/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Pilotage' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Pilotage explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Pilotage and
    its subprocesses can consume those credentials without duplicating them
    in every MCP server's ``env:`` block.
    """
    try:
        from pilotage_cli.env_loader import get_secret_source
...
def _is_method_not_found_error(exc: BaseException) -> bool:
    ...
    server is one such case. Without matching that phrasing the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit is None and sys.platform == "win32" and resolved_env:
            ...
            # its own env can find. Retry with the config's PATHEXT
        ...
        if which_hit:
            ...
            pilotage_home = os.path.expanduser(
                os.getenv(
                    "PILOTAGE_HOME", os.path.join(os.path.expanduser("~"), ".pilotage")
                )
            )
            candidates = [
                os.path.join(pilotage_home, "node", "bin", resolved_command),
                ...
                # image (which the Pilotage Docker image copies node + npm +
                # corepack from since), and macOS Homebrew on Intel.
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Pilotage MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Pilotage gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Pilotage document cache; returns a marker with
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`pilotage_cli.mcp_config._unwrap_exception_group` with
...
def _resolve_identity_header(server_name: str, config: dict):
    ...
    mode resolves the value to the active Pilotage profile name once at
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Pilotage' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (PILOTAGE_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # Rapid-drop budget: a freshly (re)established session is
        ...
        # context doesn't inherit PILOTAGE_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into Pilotage' logging (agent.log via pilotage_logging), tagged
        ...
        tool call were invisible. Port of
        """
    ...
    async def _wait_for_lifecycle_event(self) -> str:
        ...
        state from going stale during idle periods. If the keepalive
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _ensure_mcp_sdk():
            raise ImportError(
                ...
                "it is not installed. Run `pilotage setup` to install MCP support, "
                "then retry."
            )
        ...
        # MCP discovery / gateway startup. The check is fail-open, so
        ...
        # ungraceful exit of this Pilotage process (kill -9, crash, force-quit)
        ...
        # accumulation (see,). The unscoped sweep also
        ...
        try:
            async with stdio_client(server_params, errlog=_errlog) as (
                ...
                async with ClientSession(
                    ...
                    # gated on a stale consecutive-failure count.
                    ...
                    # later, forever. The session must prove itself
                ...
                if (
                    ...
                    post_resp = await client.post(
                        ...
                        content=(
                            ...
                            '"capabilities":{},'
                            '"clientInfo":{"name":"pilotage-probe",'
                            '"version":"0.1"}}}'
                        ),
                    )
    ...
    def _reconnect_or_reraise_group(self, eg: BaseExceptionGroup) -> str:
        ...
        - the group carries a real ``CancelledError`` (task cancellation must
          propagate to asyncio, mirroring the ``run`` guard for);
    ...
    async def _run_http(self, config: dict):
        ...
        if config.get("transport") == "sse":
            ...
            # observation from @amiller in (Router Teamwork,
            ...
            try:
                async with sse_client(**_sse_kwargs) as (read_stream, write_stream):
                    async with ClientSession(
                        ...
                        # stdio path: an endpoint that accepts the
                        ...
                        # gated on a stale consecutive-failure count.
                        _reset_server_error(self.name)
                        # Unproven until keepalive/tool-call success.
                        self._session_proven = False
                ...
                # reconnect immediately instead of backoff/park.
        ...
        if _MCP_NEW_HTTP:
            ...
            try:
                async with httpx.AsyncClient(**client_kwargs) as http_client:
                    async with streamable_http_client(url, http_client=http_client) as (
                        ...
                        async with ClientSession(read_stream, write_stream, **sampling_kwargs) as session:
                            # Bound the handshake — see stdio path.
                            self.initialize_result = await asyncio.wait_for(
                                session.initialize(), timeout=float(connect_timeout)
                            )
                            self.session = session
                            ...
                            # isn't gated on a stale failure count.
                            _reset_server_error(self.name)
                            # Unproven until keepalive/tool-call success.
                            self._session_proven = False
                            reason = await self._wait_for_lifecycle_event()
                ...
                # immediately instead of backoff/park.
                reason = self._reconnect_or_reraise_group(_eg)
            return reason
        else:
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Pilotage' approval system.
        ...
        while True:
            try:
                ...
                # servers respawn forever — 6212 spawns in 63h).
                ...
                if not self._ready.is_set():
                    if failure_class == "permanent":
                        ...
                        # and spamming N identical warnings.
                        #
                        ...
                        # re-authenticated with ``pilotage mcp login``. Parking
                ...
                if self._reconnect_retries > _MAX_RECONNECT_RETRIES:
                    ...
                    # life of the process. Instead, drop the phantom
                    ...
                    # we wake and attempt one reconnect ourselves.
    ...
    async def start(self, config: dict):
        ...
        try:
            ...
            # owner to reap it. Propagate the cancellation so the
...
# Lazy MCP startup: servers whose tools were registered from the
...
# scratch. That is the restart storm in: the failing server is
...
# preventing the 90-iteration burn loop described in.
#
# State machine:
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"Run `pilotage mcp login {server_name}` (or delete the tokens "
        f"file under ~/.pilotage/mcp-tokens/ and restart). Do NOT retry "
...
# only the transport-layer session state needs rebuilding. See.
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent Pilotage processes (e.g.
    ...
    try:
        ...
        if _MCP_DISCOVERY_LOCK_PATH is None:
            _MCP_DISCOVERY_LOCK_PATH = str(
                get_pilotage_home() / ".mcp-discovery.lock"
            )
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local PILOTAGE_HOME override into ``coro``.
    ...
    try:
        from pilotage_constants import (
            get_pilotage_home_override,
            reset_pilotage_home_override,
            set_pilotage_home_override,
        )

        home_override = get_pilotage_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_pilotage_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local PILOTAGE_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_pilotage_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Pilotage config file.
    ...
    ``os.environ`` (which includes ``~/.pilotage/.env`` loaded at startup).
    """
    try:
...
def _ensure_lazy_server_connected(server_name: str) -> bool:
    ...
    connect cooldown, the ``_server_connecting`` dedup set, and
...
def _get_connected_server_for_call(server_name: str) -> Optional[MCPServerTask]:
    ...
    handlers all trigger the deferred spawn.
    """
    with _lock:
        server = _servers.get(server_name)
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        if not server.session:
            ...
            if _wait_for_server_session_ready(
                ...
                # transport and re-arm the breaker forever. Instead,
        ...
        async def _call():
            ...
            # flow through Pilotage' MEDIA: tag convention and out to messaging
            # adapters that render images natively. Without this, image blocks
            # were silently dropped and the agent got an empty response.
            #
            # Distilled from (c3115644151) and (gnanirahulnutakki),
            # both too stale to cherry-pick.'s approach (integrate with
            # Pilotage' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def _normalize_mcp_input_schema(schema: dict | None) -> dict:
    ...
    * Missing or ``null`` ``type`` on an object-shaped node is coerced to
      ``"object"`` (some servers omit it). See.
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Pilotage's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Pilotage uses the ``mcp__<server>__<tool>``
# convention shared by Claude Code, Codex, and OpenCode (anomalyco/opencode
#). The double-underscore delimiter disambiguates the server/tool
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Pilotage registry schema format.
...
def _existing_tool_names() -> List[str]:
    ...
    # yet — their tools live in the registry only.
    with _lock:
        lazy_names = [
            n
...
def _register_server_tools(name: str, server: MCPServerTask, config: dict) -> List[str]:
    ...
    # Rules (matching spec, extended with glob support):
    ...
    if registered_names:
        ...
        # Write-through: refresh the on-disk schema cache after a
...
def _register_from_cache_sync(name: str, config: dict, entry: dict) -> List[str]:
    ...
    Lazy startup (, design by Vansh5632): tools appear in the registry
...
def register_mcp_servers(servers: Dict[str, dict]) -> List[str]:
    ...
    # from multiple entry-points before the first batch finishes.
    with _lock:
        ...
        new_servers = {
            ...
            # not re-registered; they connect on first tool use.
            ...
            # session's discovery pass -- the restart storm. The
        ...
        # Wake them now so their tools come back promptly.
        stale_cached = [
            _servers[k]
            for k in servers
        ...
        for name, result in zip(server_names, results):
            if isinstance(result, BaseException):
                ...
                with _lock:
                    ...
                    # Isolated to this server -- healthy servers
    ...
    try:
        ...
        # entries would block future reconnection attempts.
        with _lock:
...
def discover_mcp_tools() -> List[str]:
    ...
    # Cross-process discovery guard. A lock loser waits for
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``pilotage tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Pilotage registry.
...
def _reinject_post_build_tools(agent, tools_list: list, name_set: set) -> set:
    ...
    # Honor the same enabled_toolsets gate agent_init uses: without it a
...
def shutdown_mcp_servers():
    ...
    # in ``_servers`` (that is the very premise of the cooldown), so
    ...
    async def _shutdown():
        ...
        with _lock:
            ...
            # stale per-server backoff from before the restart.
            _server_connect_retry_after.clear()
            _server_connect_failures.clear()
    ...
    # next start should re-attempt every server immediately.
    with _lock:
        _server_connect_retry_after.clear()
        _server_connect_failures.clear()
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple pilotage
    ...
    def _send_signal(pid: int, sig: int, server_name: str) -> None:
        ...
        if pgid is not None and killpg is not None:
            if _my_pgid is not None and pgid == _my_pgid:
                ...
                # well, crashing it. Fall through to the
...
def _stop_mcp_loop(*, only_if_idle: bool = False) -> bool:
    ...
    if loop is not None:
        ...
        # is closed". ``shutdown_mcp_servers`` only reaps servers held
