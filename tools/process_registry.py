...
CHECKPOINT_PATH = get_pilotage_home() / "processes.json"
...
#
# Wrapping the spawn in ``systemd-run --user --scope --unit=pilotage-worker-<pid>``
        ...
        if not _IS_WINDOWS:
            try:
                ...
                if binary:
                    ...
                    probe_unit = f"pilotage-probe-scope-{os.getpid()}-{uuid.uuid4().hex[:8]}"
...
def _is_supervised_gateway_process() -> bool:
    """Return whether this process is in a supervised Pilotage gateway runtime.

    Both supervisor markers and ``_PILOTAGE_GATEWAY`` are inherited by every
...
def _build_systemd_scope_argv(
    ...
    if binary is None:
        ...
        return shell_argv
    unit_name = f"pilotage-worker-{unit_suffix}"
...
class ProcessRegistry:
    ...
    def _daemon_term_grace_seconds() -> float:
        ...
        """
        try:
            from pilotage_cli.config import read_raw_config, cfg_get, DEFAULT_CONFIG
            cfg = read_raw_config()
        ...
        if use_pty:
            ...
            try:
                ...
                if pty_use_systemd_scope:
                    ...
                    session.systemd_unit = f"pilotage-worker-{session.id}.scope"
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/pilotage_bg_{session.id}.log"
        pid_path = f"{temp_dir}/pilotage_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/pilotage_bg_{session.id}.exit"
    ...
    def is_session_waiting(self, session_id: str) -> bool:
        ...
        Used by the goal-loop wait barrier (``pilotage_cli.goals``) to support
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        if sink is None:
            return {
                "status": "error",
                "error": "close_terminal is only available in the Pilotage desktop app.",
            }
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if _exit in {-15, 143, "-15", "143"}:
        _signal = ", SIGTERM"
    if _reason == "killed":
        _status = f"terminated by {_source or 'Pilotage'}"
    elif _reason == "lost":
