...
# Must match pilotage_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``pilotage-cli`` (which bundles
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``pilotage-cli`` because those also contain
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. pilotage-cli) so that individual
    ...
    # Blocked tools also live inside mixed platform bundles (pilotage-cli,
    # pilotage-telegram, etc.) that _strip_blocked_tools must keep because they
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_pilotage_dir("cache/delegation", "delegation_cache")
    ...
    if _subagent_id:
        if owner_session_id is None:
            try:
                ...
                owner_session_id = get_session_env("PILOTAGE_UI_SESSION_ID", "") or None
            except Exception:
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from pilotage_cli.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    # PILOTAGE_SESSION_ID ContextVar and os.environ with the subagent's internal
    # id before the background-dispatch code below would read it. The
    # request-scoped chat_id binding (the raw X-Pilotage-Session-Id on
    ...
    try:
        ...
        _origin_ui_session_id = get_session_env("PILOTAGE_UI_SESSION_ID", "")
    except Exception:
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-Pilotage-Session-Id once the batch completes.
            ...
            # PILOTAGE_SESSION_ID here would return the subagent's internal id.
        ...
        try:
            ...
            _source = get_session_env("PILOTAGE_SESSION_SOURCE", "")
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'pilotage auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Pilotage config.

    Prefer the shared persistent loader because it follows the active
    PILOTAGE_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``PILOTAGE_IGNORE_USER_CONFIG=1`` (``pilotage chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("PILOTAGE_IGNORE_USER_CONFIG") == "1"
