...
<url>``) is spawned as a direct child of the Pilotage process. Pilotage's own
...
Pilotage process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Pilotage startup's opt-in
