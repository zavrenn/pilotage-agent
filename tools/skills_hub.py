#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Pilotage Skills Hub.
...
Used by pilotage_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _pilotage_home() -> Path:
    return get_pilotage_home()


def _skills_dir() -> Path:
    forced = _override("SKILLS_DIR")
    return Path(forced) if forced is not None else _pilotage_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            pilotage_meta = metadata.get("pilotage", {})
            if isinstance(pilotage_meta, dict):
                tags = pilotage_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            pilotage_meta = metadata.get("pilotage", {})
            if isinstance(pilotage_meta, dict):
                raw_tags = pilotage_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.pilotage/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``pilotage update``. Rather than telling them to update first, resolve
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                pilotage_meta = meta_block.get("pilotage", {})
                if isinstance(pilotage_meta, dict):
                    tags = pilotage_meta.get("tags", [])
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``pilotage_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
def _pilotage_index_cache_file() -> Path:
    return _index_cache_dir() / "pilotage-index.json"


def _load_pilotage_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for PILOTAGE_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    pilotage_index_cache_file = _pilotage_index_cache_file()
    if pilotage_index_cache_file.exists():
        try:
            age = time.time() - pilotage_index_cache_file.stat().st_mtime
            if age < PILOTAGE_INDEX_TTL:
                return json.loads(pilotage_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Pilotage index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Pilotage index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Pilotage index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        pilotage_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        pilotage_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class PilotageIndexSource(SkillSource):
    """Skill source backed by the centralized Pilotage Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_pilotage_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "pilotage-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "pilotage-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        PilotageIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "pilotage-index"
