...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.pilotage/config.yaml``. Pilotage
...
# Base URL now resolved via pilotage_cli.models.deepinfra_base_url (shared).
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``pilotage tools``); otherwise the historical Edge backend remains active.
    """
...
# custom voice-cloning scripts, etc.) into Pilotage without any Python code
...
#           output_format: wav
#
# Pilotage writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__PILOTAGE_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _command_provider_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of Pilotage secrets by
...
def _run_command_tts(
    ...
    scrubbed = pilotage_subprocess_env(inherit_credentials=False)
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``pilotage_cli.models`` helpers so every DeepInfra surface
    resolves them identically.
    """
    api_key = _resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `pilotage setup` to configure, "
    ...
    from pilotage_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        ...
        # Include Pilotage client context following Gemini's partner
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Pilotage caches Piper voice models.

    Resolves to ``~/.pilotage/cache/piper-voices/`` under the active
    PILOTAGE_HOME so voice downloads follow profile boundaries.
    """
    from pilotage_constants import get_pilotage_dir
    root = Path(get_pilotage_dir("cache/piper-voices", "piper_voices_cache"))
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run `pilotage setup` to install Mistral support."
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run pilotage setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'pilotage setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'pilotage tools' and select Piper under TTS, or install manually: "
