...
~/.pilotage/skills/. Existing skills (bundled, hub-installed, or user-created)
...
# All skills live in ~/.pilotage/skills/ (single source of truth)
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.pilotage/skills/) first, then any
...
def _org_mirror_write_guard(name: str, skill_path: Path, action: str) -> Optional[Dict[str, Any]]:
    ...
    reaches the organisation when the user runs `pilotage sync propose`, or
        ...
        if is_org_mirror_path(skill_path, _skills_dir()):
            return {
                ...
                "error": (
                    ...
                    "and can be proposed back with `pilotage sync propose "
                    f"{name}`.)"
                ),
            }
    except Exception:
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Pilotage profiles.
    ...
    # Default profile (~/.pilotage/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.pilotage/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`pilotage -p "
            ...
            base += (
                ...
                f"Switch profiles (`pilotage -p <name>`) to edit there, or "
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_pilotage_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `pilotage curator unpin <name>`. "
