...
# display_pilotage_home imported lazily at call site (stale-module safety during pilotage update)
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("PILOTAGE_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("PILOTAGE_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("PILOTAGE_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (PILOTAGE_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    # interactive Pilotage password prompt or the sudo -S password-pipe path.
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.pilotage/.env or environment variables."
        )
...
def _ensure_terminal_env_bridged() -> None:
    ...
    be stale from ``pilotage setup``). Environment values for omitted terminal
    ...
    try:
        from pilotage_cli.config import apply_terminal_config_to_env, read_raw_config
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "notify_on_complete=true) so Pilotage tracks the process, then run readiness "
...
def terminal_tool(
    ...
    try:
        ...
        # Hard-block: gateway lifecycle commands (systemctl/launchctl/pilotage
        # restart|stop targeting pilotage-gateway) must never run inside the
        ...
        # never restart. This mirrors the `pilotage gateway restart` guard in
        # pilotage_cli/gateway.py and the cron-path guard in pilotage_cli/cron.py,
        # but applies unconditionally (force=True cannot help here).
        if os.environ.get("_PILOTAGE_GATEWAY") == "1":
            ...
            if contains_launchctl_submit_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Use Pilotage cron for one-shot delayed work, or install an "
            ...
            if contains_gateway_lifecycle_command_or_referenced_script(
                ...
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "to child processes). Run `pilotage gateway restart` from a "
        ...
        if background:
            ...
            try:
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "name='github/pilotage-agent-dev', "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        _gw_platform = _gse("PILOTAGE_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("PILOTAGE_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse("PILOTAGE_SESSION_THREAD_ID", "")
                            _gw_user_id = _gse("PILOTAGE_SESSION_USER_ID", "")
                            _gw_user_name = _gse("PILOTAGE_SESSION_USER_NAME", "")
                            _gw_message_id = _gse("PILOTAGE_SESSION_MESSAGE_ID", "")
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                if has_sudo_prompt_callback or env_var_enabled("PILOTAGE_INTERACTIVE"):
                    output += (
