...
def _is_pilotage_provider_credential(name: str) -> bool:
    """True if ``name`` is a Pilotage-managed provider credential (API key,
    token, or similar) per ``_PILOTAGE_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Pilotage API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Pilotage credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated Pilotage-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_pilotage_internal_secret(name):
        return True
    return name in _PILOTAGE_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Pilotage-managed provider credentials (from
    ``_PILOTAGE_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Pilotage-managed
    ...
    Non-Pilotage third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_pilotage_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Pilotage provider "
                "credential %r (blocked by _PILOTAGE_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Pilotage-managed provider credentials must not be passed
                ...
                if _is_pilotage_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Pilotage "
                        "provider credential %r from config.yaml (blocked "
                        "by _PILOTAGE_PROVIDER_ENV_BLOCKLIST). Operator "
