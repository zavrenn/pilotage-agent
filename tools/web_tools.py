...
Backend is selected during ``pilotage tools`` setup (web.backend in config.yaml).
When available, Pilotage can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Pilotage config-aware env, falling back to process env.
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.pilotage/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``pilotage tools``).
    ...
    return "ddgs"  # default — the only bundled web provider
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        f"Re-enable it with `pilotage plugins enable {disabled_key}` "
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        "Run `pilotage tools` to set one up."
                    ),
                }
        else:
            logger.info(
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`pilotage plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
if __name__ == "__main__":
    ...
    # Check whether a search backend is usable
    ...
    if web_available:
        ...
        print("Install the ddgs package, or configure a plugin-registered provider.")
