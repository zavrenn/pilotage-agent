"""Lightweight internationalization (i18n) for Pilotage static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by Pilotage itself -- approval prompts, a handful of gateway slash
...
Usage::
    ...
    print(t("approval.choose_long", lang="ar"))            # explicit override

Language resolution order:
    1. Explicit ``lang=`` argument passed to :func:`t`
    2. ``PILOTAGE_LANGUAGE`` environment variable (for tests / quick override)
    3. ``display.language`` from config.yaml
    4. ``"en"`` (baseline)

Supported languages: en, fr, ar.  Unknown values fall back to en.  Upstream
shipped 17 catalogs; Pilotage keeps only the baseline (en), the language it
actually runs (fr), and ar.  Drop a ``locales/<lang>.yaml`` in and add the code
to :data:`SUPPORTED_LANGUAGES` to restore any of the others.
"""
...
SUPPORTED_LANGUAGES: tuple[str, ...] = ("en", "fr", "ar")
DEFAULT_LANGUAGE = "en"

# Accept a few natural aliases so users who type "french" / "fr-MA" / "arabic"
# get the right catalog instead of silently falling back to English.  Only the
# three shipped catalogs have aliases; anything else lands on English via
# :func:`_normalize_lang`.
_LANGUAGE_ALIASES: dict[str, str] = {
    "english": "en", "en-us": "en", "en-gb": "en",
    # French — Morocco (fr-ma) is the deployment locale; the rest are the usual
    # regional tags a user might type.
    "french": "fr", "français": "fr", "france": "fr",
    "fr-fr": "fr", "fr-ma": "fr", "fr-be": "fr", "fr-ca": "fr", "fr-ch": "fr",
    # Arabic — bare "arabic"/endonym plus the common regional BCP-47 tags.
    "arabic": "ar", "العربية": "ar",
    "ar-ma": "ar", "ar-sa": "ar", "ar-eg": "ar", "ar-ae": "ar", "ar-dz": "ar",
}
...
def _locales_dir() -> Path:
    ...
    1. ``PILOTAGE_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
    ...
    """
    override = os.getenv("PILOTAGE_BUNDLED_LOCALES", "").strip()
    if override:
        candidate = Path(override)
        if candidate.is_dir():
            return candidate
        logger.warning(
            "PILOTAGE_BUNDLED_LOCALES points to a non-directory path (%s); "
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a pilotage dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`pilotage_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("PILOTAGE_LANGUAGE")
    if env_lang:
