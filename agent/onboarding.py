...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Pilotage starts and finds ``~/.openclaw/``.

    Points users at ``pilotage claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``pilotage claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Pilotage, run "
        "`pilotage claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `pilotage claw cleanup` (renames it to ~/.openclaw.pre-migration — "
