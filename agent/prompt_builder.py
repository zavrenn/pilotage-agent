...
_PILOTAGE_MD_NAMES = (".pilotage.md", "PILOTAGE.md")


def _find_pilotage_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.pilotage.md`` or ``PILOTAGE.md``.
    ...
    # could pick up a .pilotage.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _PILOTAGE_MD_NAMES:
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.pilotage/kanban.db`. Your task id is in "
    "`$PILOTAGE_KANBAN_TASK`; your workspace is `$PILOTAGE_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $PILOTAGE_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $PILOTAGE_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${PILOTAGE_KANBAN_BRANCH:-wt/$PILOTAGE_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$PILOTAGE_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`pilotage profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `pilotage kanban <verb>` for board operations. Use "
...
#
# The pilotage-agent runtime already executes a batch of tool calls
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Pilotage "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Pilotage WebUI, a browser-based chat interface. "
...
def build_environment_hints() -> str:
    ...
    # Embedder-supplied environment description. Lets a host that wraps Pilotage
    ...
    extra = (os.getenv("PILOTAGE_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_pilotage_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("PILOTAGE_PLATFORM") or os.environ.get("PILOTAGE_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.pilotage/skills/`` directory.  External dirs
    ...
    # PILOTAGE_HOME would otherwise fall back to the launch (default) home and
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_pilotage_home_override(str(skills_dir.parent))
    else:
    ...
    try:
        ...
        if _home_token is not None:
            reset_pilotage_home_override(_home_token)
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Pilotage Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `pilotage-agent` skill "
            "first. It has the actual commands (e.g. `pilotage config set …`, `pilotage tools`, "
            "`pilotage setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    try:
        from pilotage_cli.nous_subscription import get_nous_subscription_features
    ...
    lines.extend(
        [
            ...
            "Useful commands: pilotage setup, pilotage setup tools, pilotage setup terminal, pilotage status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(
    ...
    """Load SOUL.md from PILOTAGE_HOME and return its content, or None.
    ...
    try:
        ...
        logger.debug("Could not ensure PILOTAGE_HOME before loading SOUL.md: %s", e)

    _home = Path(home_override) if home_override is not None else get_pilotage_home()
...
def _load_pilotage_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.pilotage.md / PILOTAGE.md — walk to git root."""
    pilotage_md_path = _find_pilotage_md(cwd_path)
    if not pilotage_md_path:
        return ""
    try:
        content = pilotage_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = pilotage_md_path.name
        try:
            rel = str(pilotage_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".pilotage.md", context_length=context_length,
            read_path=str(pilotage_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", pilotage_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    # their launch dir IS the user's shell cwd (developing Pilotage in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Pilotage install tree (%s) — set terminal.cwd to "
    ...
    # SOUL.md from PILOTAGE_HOME only — skip when already loaded as identity
    if not skip_soul:
