"""Pilotage-tools-as-MCP server for the codex_app_server runtime.

When the user runs `openai/*` turns through the codex app-server, codex
owns the loop and builds its own tool list. By default, that means
Pilotage' richer tool surface — web search, browser automation,
delegate_task subagents, vision analysis, persistent memory, skills,
cross-session search, image generation, TTS — is unreachable.

This module exposes a curated subset of those Pilotage tools to the
spawned codex subprocess via stdio MCP. Codex registers it as a normal
MCP server (per `~/.codex/config.toml [mcp_servers.pilotage-tools]`) and
the user gets full Pilotage capability inside a Codex turn.
...
What we DO NOT expose:
  ...
  - delegate_task / memory /             — `_AGENT_LOOP_TOOLS` in Pilotage
...
Run with: python -m agent.transports.pilotage_tools_mcp_server
...
# Tools we expose. Each name MUST match a registered Pilotage tool that
...
#     `_AGENT_LOOP_TOOLS` in Pilotage (model_tools.py:493). They require
#     the running AIAgent context to dispatch (mid-loop state), so a
#     stateless MCP callback can't drive them. Pilotage' default runtime
...
def _build_server() -> Any:
    """Create the FastMCP server with Pilotage tools attached. Lazy imports
    ...
    try:
        ...
        raise ImportError(
            f"pilotage-tools MCP server requires the 'mcp' package: {exc}"
        ) from exc

    # Discover Pilotage tools so dispatch works.
    ...
    mcp = FastMCP(
        "pilotage-tools",
        instructions=(
            "Pilotage Agent's tool surface, exposed for use inside a Codex "
    ...
    # Pull authoritative Pilotage tool schemas for the ones we expose, so
    # MCP clients see the same parameter docs Pilotage gives the model.
    all_defs = {
    ...
    for name in EXPOSED_TOOLS:
        ...
        if spec is None:
            logger.debug(
                "skipping %s — not registered in this Pilotage process", name
            )
            continue

        description = spec.get("description") or f"Pilotage {name} tool"
    ...
    logger.info(
        "pilotage-tools MCP server registered %d/%d tools",
        exposed_count,
        len(EXPOSED_TOOLS),
    )
    return mcp
...
def main(argv: Optional[list[str]] = None) -> int:
    """Entry point for `python -m agent.transports.pilotage_tools_mcp_server`."""
    ...
    # Quiet mode: keep Pilotage' own banners off stdout (which is the MCP wire).
    os.environ.setdefault("PILOTAGE_QUIET", "1")
    os.environ.setdefault("PILOTAGE_REDACT_SECRETS", "true")

    try:
        server = _build_server()
    except ImportError as exc:
        sys.stderr.write(f"pilotage-tools MCP server cannot start: {exc}\n")
        return 2
    ...
    try:
        ...
        logger.exception("pilotage-tools MCP server crashed")
        sys.stderr.write(f"pilotage-tools MCP server error: {exc}\n")
        return 1
    return 0
