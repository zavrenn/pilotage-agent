...
def _pilotage_home_path() -> Path:
    """Resolve the active PILOTAGE_HOME (profile-aware) without circular imports."""
    try:
        from pilotage_constants import get_pilotage_home  # local import to avoid cycles
        return get_pilotage_home()
    except Exception:
        return Path(os.path.expanduser("~/.pilotage"))


def _pilotage_root_path() -> Path:
    """Resolve the Pilotage root dir (always the parent of any profile, never per-profile)."""
    try:
        from pilotage_constants import get_default_pilotage_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    pilotage_home = _pilotage_home_path()
    pilotage_root = _pilotage_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(pilotage_root / "cache" / "bws_cache.enc.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved PILOTAGE_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("PILOTAGE_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    for base in (_pilotage_home_path(), _pilotage_root_path()):
        try:
            ...
            if real not in pilotage_dirs:
                pilotage_dirs.append(real)
        except Exception:
            continue

    for base_real in pilotage_dirs:
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside PILOTAGE_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Pilotage path.
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.pilotage/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active PILOTAGE_HOME (profile-aware) AND the global
    ...
    for hd in pilotage_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Pilotage cache file "
                "and cannot be read directly to prevent prompt injection. "
                "Use the skills_list or skill_view tools instead."
            )

    # Credential / secret stores. Exact-file matches under either
    # PILOTAGE_HOME or <root>.
    ...
    for hd in pilotage_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Pilotage credential store "
    ...
    for hd in pilotage_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Pilotage MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Pilotage MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Pilotage read (see
...
#
# Pilotage profiles are separate PILOTAGE_HOME dirs under
...
#
# Reference: May 2026 incident where a pilotage-security profile session
# edited skills under both ``~/.pilotage/profiles/pilotage-security/skills/``
# AND ``~/.pilotage/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under PILOTAGE_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from PILOTAGE_HOME.

    ``~/.pilotage``              -> ``"default"``
    ``~/.pilotage/profiles/X``  -> ``"X"``

    Falls back to ``"default"`` on any resolution failure so the guard
    never raises into the tool path.
    """
    try:
        home_real = _pilotage_home_path().resolve()
        root_real = _pilotage_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Pilotage scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Pilotage entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Pilotage profile {info['target_profile']!r}, but the "
...
#
#   <PILOTAGE_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.pilotage/...
#
...
# regardless of which Pilotage profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.pilotage/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.pilotage`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.pilotage/…`` and returns the
    index where the inner Pilotage-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .pilotage / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".pilotage":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Pilotage state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.pilotage``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.pilotage`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Pilotage resolver to
    succeed, so it works correctly even when called from contexts where
    PILOTAGE_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Pilotage process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real PILOTAGE_HOME. Use the host-side tool for "
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host PILOTAGE_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Pilotage state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Pilotage "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real PILOTAGE_HOME. Use the "
