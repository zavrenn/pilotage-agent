...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in pilotage_state.py — kept local
# to avoid importing pilotage_state at module load time (its module-level
# DEFAULT_DB_PATH = get_pilotage_home() / "state.db" breaks tests that
# monkeypatch get_pilotage_home to return a str).
...
from pilotage_constants import PARTIAL_STREAM_STUB_ID
...
def _restore_user_after_reference_handoff(
    ...
    call; this helper only decides whether a restorable ask exists.
    """
...
def _should_skip_model_call_for_reference_handoff(
    ...
    """Guard post-compaction continues against sole-handoff active turns."""
...
# Fallback final_response for a turn ended by the sole-handoff skip.
# Deliberately NOT a replay of the last assistant text: finalize_turn's
# non-assistant-tail chokepoint appends final_response as a fresh
...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    mechanism as the ~/.pilotage/prefill.json incident, 20/20 blocked with
    ...
    reply*, echo it, and self-replicate ghost rows across turns.
    ...
    if messages and messages[-1].get("role") == "assistant":
        ...
        # then echoes / incomplete else branch).
...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    ...
    def host_info_value(label: str) -> str:
        ...
        occurrence, so only Pilotage' own emitted block can satisfy the read.
        """
        prefix = f"{label}:"
...
# calls. Named for the same reason as the nudges above — its
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `pilotage fallback add`."
)
...
def _clone_message_for_send(msg):
    ...
    silently rewrites the stored transcript (: an unrepairable
...
def _compression_deferred_result(
    ...
    gateway auto-resets (wipes) the session on exhaustion /),
    ...
    **meta: Any,
    ...
    try:
        hook(
            ...
            # nested containers into the transcript aliasing class).
...
def run_conversation(
    ...
    # Adopt any ~/.pilotage/.env credential/base-url edits made since the last
    # turn — a Settings save updates .env but not this worker's client, which
    # was built at agent init. No-op when.env is unchanged.
    try:
    ...
    # 'disk', or 'unknown' — see pilotage_state.classify_persistence_error).
    ...
    # over instead of spinning. Reset here so each turn starts fresh. See.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # Drop legacy ghost rows from the incomplete else branch BEFORE
        ...
        # Dropping before repair lets repair_message_sequence fix
        ...
        # so the turn-end flush doesn't skip the assistant/tool chain.
        ...
        # cache prefix.  The system prompt is reserved for Pilotage internals.
        #
        ...
        # (schema overhead / post-compaction over-count,); (2) skip
        ...
        # LLM cooldown + anti-thrash guards. compression_attempts is a
        ...
        if (
            ...
            # turn-context preflight reset (silent-overflow fix).
            ...
            if messages is _pre_api_input and compression_skipped_due_to_lock(agent):
                ...
                #/) and leave the insufficient-progress blocker
                ...
                if _should_skip_model_call_for_reference_handoff(
                    ...
                    # after a completed assistant response.
                    logger.info(
                        "Skipping post-compaction model call: reference-only "
                        "handoff would be the sole active user turn"
                    )
        ...
        while retry_count < max_retries:
            try:
                ...
                # Outbound-request surrogate chokepoint: the messages
                ...
                try:
                    from pilotage_cli.lifecycle import (
                        has_hook,
                    ...
                    if has_hook("pre_api_request"):
                        ...
                        # consumed by observability plugins.
                ...
                from pilotage_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Pilotage/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if finish_reason == "length":
                    ...
                    if agent.api_mode == "chat_completions":
                        ...
                        if assistant_message is not None and _trunc_has_tool_calls:
                            ...
                            # never reaches finalize_turn class).
                ...
                _IMAGE_REJECTION_PHRASES = (
                    ...
                    # the P1 in. The 404 passes the 4xx gate below.
                ...
                # and omits the field, and Pilotage' local compression takes
                ...
                # Ported from anomalyco/opencode. When the user has
                ...
                # Rate limits / billing are terminal for the turn; transport
                # errors get one retry first (transient hiccups recover).
                ...
                if is_rate_limited or (_is_transport_failure and retry_count >= 2):
                    # Stay quiet if credential pool rotation may still recover.
                    # See _pool_may_recover_from_rate_limit for the
                    # single-credential-pool exception.
                    if not _ra()._pool_may_recover_from_rate_limit(
                        agent._credential_pool,
                    ):
                        if _is_transport_failure:
                            agent._buffer_status("⚠️ Provider unreachable.")
                        else:
                            agent._buffer_status("⚠️ Rate limited.")
                ...
                if is_payload_too_large:
                    ...
                    if messages is _overflow_input and compression_skipped_due_to_lock(agent):
                        # lock-skip: the provider proved the request
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # Pilotage may add API-only content not present in persisted
                    ...
                    # death-loops until "cannot compress further".
                    ...
                    # recovery arms on the TRUE request that overflowed. See pilotage-lcm engine
                ...
                is_local_validation_error = (
                    ...
                    # programming bugs. Even after made our own
                ...
                # balance with no recovery mechanism left — see
                ...
                if is_client_error:
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        pilotage fallback add   (interactive picker — same as `pilotage model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        _policy_response = (
                            ...
                            "(not a Pilotage/gateway failure).\n\n"
                ...
                if retry_count >= max_retries:
                    ...
                    # agent/reasoning_timeouts.py (Fixes) so the
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.pilotage/config.yaml` to extend the per-call "
                            f"timeout. (Pilotage's built-in floor is 600s for "
                    ...
                    if _resp_headers and hasattr(_resp_headers, "get"):
                        ...
                        if _ra_raw:
                            try:
                                ...
                                # windows while still rejecting pathological values.
        ...
        if _retry.restart_with_rebuilt_messages:
            ...
            # call. Hoisted here (the single consumer) so every
        ...
        try:
            ...
            if agent.api_mode == "codex_responses" and finish_reason == "incomplete":
                ...
                if (
                    ...
                    # dedup and causes message storms.
                ...
                if agent._codex_incomplete_retries < 3:
                    ...
                    # thinking spinner ("infinite thinking",).
            ...
            if assistant_message.tool_calls:
                ...
                if _mixed_invalid_batch:
                    ...
                    if agent._invalid_tool_retries >= 3:
                        ...
                        # interrupt aborts /) so the next user
                    ...
                    for tc in assistant_message.tool_calls:
                        ...
                        if _tc_name not in agent.valid_tool_names:
                            ...
                            # blank-name anti-priming rationale.
                ...
                # post-tool fallback replay that token forever after compaction.
                if (
                    assistant_message.tool_calls
                ...
                try:
                    ...
                    # terminates Pilotage mid-turn, resume logic still sees the
                    ...
                    from pilotage_state import classify_persistence_error
                ...
                if (
                    ...
                    # true request size from the engine's overflow guard (upstream review).
                    messages, active_system_prompt = agent._compress_context(
                        messages, system_message,
                    ...
                    if (
                        ...
                        # lock-skip: this pass no-oped because another
                    ...
                    # turn-context preflight guard (silent-overflow fix).
                    _block_reason = None
                ...
                # timeout (PILOTAGE_AGENT_TIMEOUT, default 1800s) and the
                ...
                if not agent._has_content_after_think_block(final_response):
                    ...
                    if reasoning_text:
                        ...
                        logger.warning(
                            "Empty response (no content or reasoning) "
                            "after %d retries. model=%s provider=%s",
                    ...
                    # answer earlier in the ladder — prefill continuation and
                    # empty-content retries both run first. Only at this
                    # terminal, where the alternative is returning nothing, is
                    # showing the model's own reasoning (clearly labeled as
                    # such) strictly more useful.
                    if reasoning_text:
                        final_response = (
                            "⚠️ The model produced only internal reasoning and "
                            "no final answer, despite retries. Its last "
                            "reasoning, which may contain the answer:"
                            "\n\n" + reasoning_preview
                        )
                    else:
                ...
                if _verify_nudge:
                    ...
                    # from unrelated error/recovery exits.
                ...
                # otherwise loses a reply the user already saw. Same
                # contract as the tool-call exit and the verify exits
