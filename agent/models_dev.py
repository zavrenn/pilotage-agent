...
Data resolution order:
  ...
  2. Disk cache (~/.pilotage/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Pilotage ↔ models.dev
# ---------------------------------------------------------------------------
...
def _models_dev_to_pilotage_ids(mdev_id: str) -> List[str]:
    """Return the Pilotage provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        reverse: Dict[str, List[str]] = {}
        for pilotage_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            reverse.setdefault(mapped, []).append(pilotage_id)
...
def _get_cache_path() -> Path:
    ...
    return get_pilotage_home() / "models_dev_cache.json"


def _get_etag_path() -> Path:
    """Return path to the ETag sidecar file for conditional GET."""
    from pilotage_constants import get_pilotage_home
    return get_pilotage_home() / "models_dev_cache.etag"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``pilotage config refresh``, the
...
def lookup_models_dev_context(
    ...
    # Suffix-aware fallback: some catalog entries carry :cloud / -cloud
    # suffixes in models.dev while the live API returns bare names.
    # Without this the bare name misses its suffixed entry and falls
    # through to stale metadata reporting 32768 — tripping the 64k
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    # Reverse: caller passed a models.dev id, config keyed by Pilotage id.
    for pilotage_id in _models_dev_to_pilotage_ids(provider_key):
        if pilotage_id != provider_key:
            candidates.append(pilotage_id)
...
def _get_provider_models(
    ...
    """Resolve a Pilotage provider ID to its models dict from models.dev.
...
def list_provider_models(
    ...
    picker (``pilotage model``), which is an interactive user-facing flow
...
def get_provider_info(
    ...
    Accepts either a Pilotage provider ID (e.g. "kilocode") or a models.dev
    ...
    ``allow_network=False``.
    """
    # Resolve Pilotage ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Pilotage or models.dev provider ID.  Tries exact match then
