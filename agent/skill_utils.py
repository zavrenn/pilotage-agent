...
from pilotage_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.pilotage/skills/_org/<org_id>/. Resolution is
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``pilotage_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``PILOTAGE_PLATFORM`` or
            ``PILOTAGE_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("PILOTAGE_PLATFORM")
        or get_session_env("PILOTAGE_SESSION_PLATFORM")
    )
...
# which becomes the dominant cost of ``pilotage`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.pilotage/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``pilotage`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against PILOTAGE_HOME, not cwd
        if not p.is_absolute():
            p = (pilotage_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.pilotage/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.pilotage/skills/`` (including via symlinks) or configured
    ...
    # ~/.pilotage/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Pilotage can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    pilotage = metadata.get("pilotage") or {}
    if not isinstance(pilotage, dict):
        pilotage = {}
    return {
        "fallback_for_toolsets": pilotage.get("fallback_for_toolsets", []),
        "requires_toolsets": pilotage.get("requires_toolsets", []),
        "fallback_for_tools": pilotage.get("fallback_for_tools", []),
        "requires_tools": pilotage.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    pilotage = metadata.get("pilotage")
    if not isinstance(pilotage, dict):
        return []
    raw = pilotage.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Pilotage metadata, VCS, virtualenv/dependency, cache, and skill
