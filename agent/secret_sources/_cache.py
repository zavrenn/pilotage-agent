...
cache problem must never block Pilotage startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Pilotage home used for cache paths.

    ``home_path`` is whatever ``load_pilotage_dotenv()`` already resolved;
    falling back to ``$PILOTAGE_HOME`` / ``~/.pilotage`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from pilotage_constants import get_pilotage_home

        home_path = get_pilotage_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<pilotage_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if PILOTAGE_HOME is ever made traversable.
            try:
