...
Every Pilotage request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Pilotage Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=pilotage-agent",
        "client=pilotage-client-v<__version__>",
    ]

The version is sourced live from ``pilotage_cli.__version__`` so it auto-aligns
...
``pilotage_cli.__version__`` is the canonical source of truth.
"""
...
def _pilotage_version() -> str:
    """Return the current Pilotage release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``pilotage_cli`` cannot be imported (should
...
def pilotage_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=pilotage-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=pilotage-client-v{_pilotage_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Pilotage session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Pilotage conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.pilotage/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Pilotage
    ...
    """
    tags = ["product=pilotage-agent", pilotage_client_tag()]
