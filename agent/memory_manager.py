...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_PILOTAGE_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Pilotage expands the turn into
    ...
    def _prefetch_provider(
        ...
        # Propagate the caller's contextvars (profile PILOTAGE_HOME override)
    ...
    def _submit_background(self, fn, *, kind: str = "write") -> None:
        ...
        dashboard, cron) is a ContextVar-scoped PILOTAGE_HOME override, and
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``pilotage_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_pilotage_home()`` themselves.
        """
        if "pilotage_home" not in kwargs:
            from pilotage_constants import get_pilotage_home
            kwargs["pilotage_home"] = str(get_pilotage_home())
        for provider in self._providers:
            try:
