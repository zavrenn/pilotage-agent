...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``pilotage trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Pilotage' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "pilotage-traces"
_PILOTAGE_VERSION = "pilotage-agent"
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Pilotage' shared redactor with
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.pilotage/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `pilotage trace upload`)."
)
