...
``auth.json``.  They intentionally have no dependency on ``pilotage_cli.auth`` so
...
# Sources Pilotage owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Pilotage-owned OAuth/device-code state)
