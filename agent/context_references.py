...
from pilotage_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags
...
_SENSITIVE_PILOTAGE_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    pilotage_home = get_pilotage_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(pilotage_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(pilotage_home / rel for rel in _SENSITIVE_PILOTAGE_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Pilotage path and cannot be attached")
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Pilotage path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
