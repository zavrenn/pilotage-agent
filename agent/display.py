...
# Diff colors — fixed true-color escapes tuned for dark terminals.
...
def get_skin_tool_prefix() -> str:
    """Return the tool-output prefix character."""
    return "┊"


def get_tool_emoji(tool_name: str, default: str = "⚡") -> str:
    """Return the display emoji for a tool, from the registry or *default*."""
    try:
...
def build_status_phrase(tool_name: str, args: dict | None, max_len: int = 49) -> str | None:
    ...
    name ("Pilotage is running …"), so it starts lowercase with "is".
...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in Pilotage' inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class KawaiiSpinner:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("PILOTAGE_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue
