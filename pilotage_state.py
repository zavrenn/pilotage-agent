#!/usr/bin/env python3
"""
SQLite State Store for Pilotage Agent.
...
from pilotage_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Set, Tuple, TypeVar

from pilotage_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from pilotage_state_portability import SessionPortabilityMixin
from pilotage_state_schema import SessionSchemaMixin
from pilotage_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``pilotage -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_pilotage_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.pilotage`` even when a test fixture later
    redirects ``PILOTAGE_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_pilotage_home()`` fresh so a runtime
       ``PILOTAGE_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_pilotage_home() / "state.db"
...
# ~/.pilotage/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects PILOTAGE_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without PILOTAGE_HOME, a stale worktree
# without the re-pin, or a developer shell that exports PILOTAGE_HOME to the
...
_STATE_DB_GUARD_BYPASS_ENV = "PILOTAGE_STATE_DB_GUARD_BYPASS"

#: Additional production roots to refuse (beyond the platform default
#: ``~/.pilotage``).  The test conftest injects the pre-sandbox production
#: root here so custom-``PILOTAGE_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Pilotage root for the guard.

    Deliberately avoids ``Path.home()`` / ``pilotage_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``pilotage_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "pilotage"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "pilotage"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".pilotage"
        return root.resolve()
    except Exception:
        return None
...
#: redirects ``PILOTAGE_HOME`` to the per-session tmp isolation root.  Its
...
#: is OURS: it declares "this process tree is running under Pilotage test
#: isolation", and it inherits into subprocess children by default — so a
#: child that received the patched ``PILOTAGE_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape.
_TEST_ISOLATION_MARKER_ENV = "PILOTAGE_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``PILOTAGE_HOME`` redirect: that child aims at the production DB *and*
    disarms the guard in one step. Ancestry is the one test-context
...
def _in_test_context() -> bool:
    ...
    so a real ``pilotage`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Pilotage home *root*.
    ...
    under ``~/.pilotage/pilotage-agent/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``PILOTAGE_HOME``) paths.
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``PILOTAGE_HOME`` together,
    which is precisely the state in which it writes to production.
    """
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Pilotage root {root}). "
                "Tests must run against a temporary PILOTAGE_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "PILOTAGE_HOME. If this test genuinely needs the live "
...
# Dedup WARNING for the WAL-reset vulnerability fallback.
...
def _is_stale_tool_call_marker_message(msg: Dict[str, Any]) -> bool:
    ...
    Before the fix in ``agent.conversation_loop``, a local tool-call
...
def _strip_stale_tool_call_markers(
    ...
    """Clear bare protocol-marker content persisted before the fix.
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``pilotage sessions optimize`` (FTS merge + VACUUM)
...
def _apply_macos_checkpoint_barrier(conn: sqlite3.Connection) -> None:
    ...
    This is the trigger in ("SIGTERM during launchd
...
, refuse to enable WAL on fresh / non-WAL databases
    (prefer DELETE).  If the on-disk DB is already WAL, keep WAL and warn
    — never live-downgrade under possible concurrent openers.

    This gate is deliberately RETAINED. An earlier revision of the
    lock-cancellation fix reverted it on the theory that DELETE was
    ...
    #: honor the canonical database.journal_mode setting. Existing
    ...
    try:
        ...
        # connection bursts,), but it can also be a one-shot
...
def _wal_reset_repair_hint() -> str:
    ...
    ``pilotage update`` can actually do for this install.
    """
    try:
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Pilotage"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Pilotage-managed install (uv-managed venv)
    # so the warning doesn't promise a repair path that doesn't exist
    # for git/pip/system Python installs.
...
def apply_database_pragmas(
    ...
    * ``journal_size_limit`` — max journal/WAL size in bytes
    ...
    """
    try:
        # Local import avoids a circular import with pilotage_cli.config.
        from pilotage_cli.config import cfg_get, load_config_readonly
...
# threads inside ONE interpreter, yet a normal Pilotage host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``pilotage serve`` backend, interactive CLI sessions,
...
def _cross_process_repair_lock(db_path: Path):
    ...
    with no traceback (the failure shape of).
    """
...
# ── Repair-loop bounding + dead-backup hygiene ─────────────────────
#
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    refused backup as a HARD STOP (see: proceeding without the
    ...
    connection's POSIX advisory locks (see ``pilotage_cli.sqlite_safe_read``).
    ...
    while backup_path.exists():
        ...
        seq += 1
    try:
        # Dedupe: a repair loop used to copy the SAME damaged bytes
        ...
        # Retention cap: keep only the newest few forensic copies.
...
def preflight_db_writability(
    ...
    *,
    ...
    Port of Kilo-Org/kilocode's startup preflight. A stray read-only
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Pilotage home tree (``get_pilotage_home()``) — the safe repair scope:
      Pilotage owns those files, and the OS makes ``chmod`` fail on files the
    ...
    try:
        home: Optional[Path] = Path(get_pilotage_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Pilotage needs read-write access to open the database. "
...
def _db_opens_cleanly(db_path: Path) -> Optional[str]:
    ...
    try:
        ...
        # through the triggers. It does NOT catch partial FTS5
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (pilotage_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_pilotage_state.py:600-632)
                ...
                # This is the corruption class actually wants caught:
        ...
        probe_session_id = f"_pilotage_fts_health_probe_{time.time_ns()}"
        try:
...
def repair_state_db_schema(db_path: Path, *, backup: bool = True) -> Dict[str, Any]:
    ...
    write-corruption class where base tables read fine and
...
def _repair_state_db_schema_locked(
    ...
    if backup:
        ...
        if bpath is None:
            # HARD STOP: every strategy below mutates the damaged
...
# index speed. Contributed by Soju06.
#
...
# (``~/.pilotage/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``pilotage sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("PILOTAGE_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_pilotage_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("PILOTAGE_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship pilotage_state without pilotage_cli). A
    ...
    try:
        ...
        logger.debug(
            "pilotage_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch pilotage_state.sqlite3.connect keep control of connection creation;
...
def quarantine_zeroed_state_db(path: Path) -> Optional[Path]:
    ...
    Uses a cross-process lock (````) so two concurrent startups cannot
    ...
    try:
        ...
        if not acquired:
            ...
            # the two processes can act on the same live file review).
            logger.error(
                ...
                "state-snapshots via `pilotage snapshot list` / "
                "`pilotage snapshot restore <id>`.",
                path,
            )
            return None
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple pilotage processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling Pilotage
    ...
    # `pilotage update` leaves mixed-version processes sharing the DB until
    ...
    # Observation-only activity heartbeat/label writes review S1):
    ...
    # the overwhelming majority of concurrent turns. It deliberately
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            #: zeroed state.db (size>0, all-NUL header) used to fail as a
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `pilotage snapshot list` / "
                    f"`pilotage snapshot restore <id>` if available. "
            ...
            def _connect_and_init_with_lock_patience():
                ...
                #). The store is healthy; wait it out with the same
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`pilotage db optimize`),
            ...
            # ``pilotage_state._set_last_init_error(None)`` explicitly.
    ...
    def _db_has_legacy_inline_fts(cursor: sqlite3.Cursor) -> bool:
        ...
        (v10-era, pre-). We therefore detect "needs optimize" as "the
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `pilotage update` to rebuild the venv with a "
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `pilotage sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Pilotage process held the "
    ...
    def _try_wal_checkpoint(self) -> None:
        ...
        from checkpointing thousands of frames at once.
        """
        try:
            with self._lock:
        ...
        with self._lock:
            if self._conn:
                if not self.read_only:
                    ...
                    # pages -- the corruption this class's own periodic
    ...
    #
    # `optimize_fts_storage()` (the `pilotage sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`pilotage sessions optimize-storage`) ──
    #
...
, or lose its owning profile and be aggregated as "default" every
        ...
        without a recoverable routing mapping.
        """
        def _do(conn):
            ...
            if parent_session_id:
                ...
                # Belt-and-suspenders for gateway routing metadata:
    ...
    def record_gateway_session_peer(
        ...
        *,
        ...
        and full origin metadata so consumers (mcp_serve, mirror,
        ...
        Self-healing: when the target row does not exist yet — the
        ...
        def _do(conn):
            ...
            # Self-heal: the UPDATE is a silent no-op when the row
    ...
    def set_expiry_finalized(self, session_id: str, finalized: bool = True) -> None:
        ...
        survives even if the JSON index is pruned or lost.
        """
        if not session_id:
            return
    ...
    # ── Gateway routing index (replaces sessions.json, follow-up) ────
    ...
    def list_never_active_keyed_sessions(
        ...
        That is exactly the shape of a leaked test fixture — and
    ...
    def find_latest_gateway_session_for_peer(
        ...
        *,
        ...
        (dashboard viewer disconnect before) are treated as recoverable;
        explicit conversation boundaries such as /new, /resume switches, and
        compression splits are not.

        Ordering and emptiness: candidates are ranked by actual
        ...
        Reset boundaries fence recovery: an intentional boundary
    ...
    # ── Orphaned gateway-session repair ──────────────────────────
    ...
    # offline repair path behind ``pilotage sessions repair-routing``.
        ...
        def _do(conn):
            ...
            conn.execute(
                ...
                (
                    ...
                    # compression-fork backfill / cross-profile jump
    ...
    def get_compression_ineffective_count(self, session_id: str) -> int:
        ...
        from zero across process restarts.
        """
        if not session_id:
            return 0
        with self._lock:
    ...
    def touch_session_activity(
        ...
        *,
        ...
        even when no new message row has been written yet / ).
        ...
        # write-patience budget review S1). Under contention a
    ...
    def clear_session_activity_labels(self, session_id: str) -> None:
        ...
        *what was happening at* that timestamp during an active turn; once
        ...
        Response-critical-path contract review S1): runs in the
...
. Callers without provider knowledge leave any stored
        provider untouched.
        """
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``pilotage --resume`` into a fresh
    ...
    def update_session_billing_route(
        ...
        *,
        ...
        behavior of ``update_session_model`` (see,).
        """
        # Barrier against queued token deltas — see update_session_model.
        self.flush_token_counts()
    ...
    def update_token_counts(
        ...
        # initial model. Recording the per-call delta into
    ...
    def _record_model_usage(
        ...
        *,
        ...
        via :meth:`record_auxiliary_usage`.
        """
        row = conn.execute(
    ...
    def record_auxiliary_usage(
        ...
        *,
        ...
        """Record an auxiliary LLM call's usage against *session_id*.
    ...
    def finalize_orphaned_compression_sessions(self) -> int:
        ...
        end_reason='orphaned_compression'. Fix for.
        """
    ...
    def _check_transcript_write_guards(
        ...
        (this guard has already needed targeted fixes — see the
        patience note below).
        """
    ...
    def append_message(
        ...
        # process's FTS optimize) can't destroy a healthy turn.
    ...
    def replace_messages(
        ...
        turns that :meth:`archive_and_compact` keeps on disk for
    ...
    def has_archived_messages(self, session_id: str) -> bool:
        ...
        ``archive_and_compact``,); kept for tests and diagnostics.
        """
        with self._lock:
            cursor = self._conn.execute(
 ...
 WITHOUT destroying history:
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        # DEFENSE-IN-DEPTH against: before that fix, a bare tool-call
        ...
        if repair_alternation and messages:
            # Lazy import: pilotage_state already depends on agent.* (see
    ...
    # =========================================================================
    # Rewind (soft-delete) — see /rewind slash command +
    # =========================================================================
    ...
    def search_sessions(
        ...
        ``pilotage -c``/``--resume`` so the "last" session is the last one in
    ...
    def has_platform_message_id(
        ...
        lookup. Used by the gateway's transient-failure dedupe guard
    ...
    def purge_stale_tool_call_markers(
        ...
        fix in ``agent.conversation_loop``.
        ...
        if affected_ids:
            logger.info(
                "Permanently cleared %d stale tool-call marker row(s) in state.db",
                len(affected_ids),
            )
        return {
            "dry_run": False,
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Pilotage, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Pilotage session.

        A Pilotage session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Pilotage session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `pilotage sessions vacuum` runs in a transient
            # CLI process, and a TRUNCATE reset here would race a live gateway
            # writer and tear B-tree pages. VACUUM folds the WAL back
