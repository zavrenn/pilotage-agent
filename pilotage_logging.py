"""Centralized logging setup for Pilotage Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.pilotage/logs/`` (profile-aware via ``get_pilotage_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``pilotage_cli.web_server``,
    ``pilotage_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Pilotage (TUI, gateway, ``hy_memory`` server, MCP
...
from pilotage_constants import get_config_path, get_pilotage_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_pilotage_session_injector", False):
    ...
    _session_record_factory._pilotage_session_injector = True  # type: ignore[attr-defined]
...
def setup_logging(
    *,
    ...
    """Configure the Pilotage logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    pilotage_home
        Override for the Pilotage home directory.  Falls back to
        ``get_pilotage_home()`` (profile-aware).
    log_level
    ...
    home = pilotage_home or get_pilotage_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_pilotage_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._pilotage_verbose = True  # type: ignore[attr-defined]
...
# the module header): when several Pilotage processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._pilotage_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="pilotage-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_pilotage_queue", False):
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse pilotage_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when pilotage_cli.config isn't importable (bare
        # pilotage_logging consumers).
        try:
            from pilotage_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
