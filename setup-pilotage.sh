...
# 5. Symlinks the 'pilotage' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Pilotage Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/pilotage-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink pilotage into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up pilotage command..."

PILOTAGE_BIN="$SCRIPT_DIR/venv/bin/pilotage"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$PILOTAGE_BIN" "$COMMAND_LINK_DIR/pilotage"
echo -e "${GREEN}✓${NC} Symlinked pilotage → $COMMAND_LINK_DISPLAY_DIR/pilotage"
...
# ============================================================================
# Seed bundled skills into ~/.pilotage/skills/
# ============================================================================

PILOTAGE_SKILLS_DIR="${PILOTAGE_HOME:-$HOME/.pilotage}/skills"
mkdir -p "$PILOTAGE_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.pilotage/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$PILOTAGE_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  pilotage status        # Check configuration"
if is_termux; then
    echo "  pilotage gateway       # Run gateway in foreground"
else
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m pilotage_cli.main setup
fi
