...
Extracted verbatim from pilotage_state.py so the SessionDB mixin modules
(pilotage_state_search / pilotage_state_schema / pilotage_state_portability) can
reference them without importing pilotage_state (which would be a cycle).
pilotage_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``pilotage sessions optimize-storage``. A legacy DB sits at
...
# `pilotage sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `pilotage db optimize`. This is
