...
Usage:
    pilotage pairing list              # Show all pending + approved users
    pilotage pairing approve <platform> <request-id|code>  # Approve a pairing request
    pilotage pairing revoke <platform> <user_id> # Revoke user access
    pilotage pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle pilotage pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: pilotage pairing {list|approve|revoke|clear-pending}")
        print("Run 'pilotage pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: pilotage pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.pilotage/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'pilotage pairing list' to see pending requests.\n")
