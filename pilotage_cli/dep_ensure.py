...
from pilotage_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import pilotage_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $PILOTAGE_HOME/node is
...
def _has_pilotage_agent_browser() -> bool:
    ...
    # install.sh installs globally into $PILOTAGE_HOME/node/bin/ via npm -g --prefix
...
def ensure_dependency(
    ...
    if shell == "powershell":
        ...
        cmd = [
            ...
            "-File", str(script),
            "-Ensure", dep,
            "-PilotageHome", str(get_pilotage_home()),
        ]
    else:
        cmd = ["bash", str(script), "--ensure", dep]

    run_env = pilotage_subprocess_env(inherit_credentials=False)
