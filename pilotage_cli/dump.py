"""
Dump command for pilotage CLI.

Outputs a compact, plain-text summary of the user's Pilotage setup
...
from pilotage_cli.config import get_pilotage_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.pilotage/.env.
...
def _redact(value: str) -> str:
    ...
    ``pilotage dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from pilotage_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(pilotage_home: Path) -> int:
...
def _cron_summary(pilotage_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = pilotage_home / "cron" / "jobs.json"
...
def _configured_platforms() -> list[str]:
    ...
    checks = {
        ...
        "whatsapp_cloud": "WHATSAPP_CLOUD_ACCESS_TOKEN",
...
def run_dump(args):
    ...
    lines.append(f"pilotage_home:      {display_pilotage_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.pilotage/.env: a managed
    ...
    toolsets = config.get("toolsets", ["pilotage-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(pilotage_home)}")
    lines.append(f"  skills:             {_count_skills(pilotage_home)}")
