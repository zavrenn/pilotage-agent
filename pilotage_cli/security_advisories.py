"""
Security advisory checker for Pilotage Agent.
...
run in environments where the rest of Pilotage failed to import.
"""
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Pilotage requires 3.10+ but defensive.
        return None
    try:
...
# ``~/.pilotage/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_pilotage_home()) / "cache"
