"""pilotage webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    pilotage webhook subscribe <name> [options]
    pilotage webhook list
    pilotage webhook remove <name>
    pilotage webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.pilotage/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _pilotage_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
    _dhh = display_pilotage_home()
    return f"""
  Webhook platform is not enabled. To set it up:

  1. Run the gateway setup wizard:
     pilotage gateway setup
  ...
  Then start the gateway: pilotage gateway run
"""
...
def webhook_command(args):
    """Entry point for 'pilotage webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: pilotage webhook {subscribe|list|remove|test}")
        print("Run 'pilotage webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (pilotage gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: pilotage webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from pilotage webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (pilotage gateway run)")
