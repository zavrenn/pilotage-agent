...
on Japanese locales, etc.).  Pilotage's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``pilotage_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Pilotage is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``PILOTAGE_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("PILOTAGE_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Pilotage-managed Git install directories so
    ...
    # already-running shells don't see the change — which means pilotage
    ...
    # install without Pilotage-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Pilotage's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Pilotage's
    ``pilotage config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Pilotage (User env var in Windows Settings, or
    export in a PowerShell profile) and Pilotage picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Pilotage-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\pilotage\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``pilotage`` from right after install) retain their old PATH.

    Any subprocess Pilotage spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Pilotage-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "pilotage", "git", "cmd"),
        os.path.join(local_appdata, "pilotage", "git", "bin"),
        os.path.join(local_appdata, "pilotage", "git", "usr", "bin"),
        # Pilotage venv Scripts directory — host of the pilotage.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes pilotage, but harmless to include.
        os.path.join(local_appdata, "pilotage", "pilotage-agent", "venv", "Scripts"),
