"""
Baked-in build metadata for Pilotage Agent.

Source installs report their git revision live via ``git rev-parse`` (see
``pilotage_cli/dump.py`` and ``pilotage_cli/banner.py``).  That doesn't work inside
...
To make ``pilotage dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$PILOTAGE_GIT_SHA``
arg into ``<project_root>/.pilotage_build_sha``.  This module is the single
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``PILOTAGE_GIT_SHA`` build-arg fall through to live-git
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".pilotage_build_sha"


def get_build_sha(short: int = 8) -> Optional[str]:
    """Return the baked-in build SHA, truncated to ``short`` chars, or None.

    Reads ``<project_root>/.pilotage_build_sha`` if present.  The file is
    written by the Dockerfile's ``PILOTAGE_GIT_SHA`` build-arg and contains
