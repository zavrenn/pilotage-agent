"""``pilotage logs`` — view and filter Pilotage log files.
...
Usage examples::
    ...
    pilotage logs                    # last 50 lines of agent.log
    pilotage logs -f                 # follow agent.log in real time
    pilotage logs errors             # last 50 lines of errors.log
    pilotage logs gateway -n 100    # last 100 lines of gateway.log
    pilotage logs desktop -f        # follow desktop.log (Electron app boot/backend)
    pilotage logs --level WARNING    # only WARNING+ lines
    pilotage logs --session abc123   # filter by session ID substring
    pilotage logs --component tools  # only tool-related lines
    pilotage logs --since 1h         # lines from the last hour
    pilotage logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_pilotage_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Pilotage runs — try 'pilotage chat' first)")
        sys.exit(1)
    ...
    if component:
        from pilotage_logging import COMPONENT_PREFIXES
    ...
    if follow:
        print(f"--- {display_pilotage_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_pilotage_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_pilotage_home()}/logs/")
        return

    print(f"Log files in {display_pilotage_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'pilotage chat' to generate logs)")
