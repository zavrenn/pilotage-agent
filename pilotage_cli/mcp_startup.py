...
def _has_configured_mcp_servers() -> bool:
    ...
    try:
        ...
        if isinstance(mcp_servers, dict) and len(mcp_servers) > 0:
            return True
        from pilotage_cli.agent_plugins import has_enabled_agent_plugin_mcp
...
def start_background_mcp_discovery(*, logger, thread_name: str) -> None:
    ...
    with _mcp_discovery_lock:
        ...
        # Capture the caller's context-local PILOTAGE_HOME override (profile
        ...
        def _discover() -> None:
            token = None
            try:
                from pilotage_constants import set_pilotage_home_override
            ...
            try:
                ...
                if token is not None:
                    try:
                        from pilotage_constants import reset_pilotage_home_override
...
def _resolve_discovery_timeout(
    ...
    When ``single_query`` is True (``pilotage -z "..."`` / ``-q``), the larger
    ...
    try:
        from pilotage_cli.config import load_config, DEFAULT_CONFIG
...
def ensure_mcp_discovery_before_agent_build(
    *,
    ...
    Non-interactive first turns (``chat -q``, ``pilotage -z``) can construct
