...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from pilotage_cli.inventory import load_picker_context
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from pilotage_cli.models import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    for pilotage_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        try:
            ...
            _prof = _gpp(pilotage_id)
            if _prof is not None:
                _canonical = _prof.name
        except Exception:
            pass
        if _canonical != pilotage_id:
            continue
        if pilotage_id.lower() in seen:
            continue
        if pilotage_id.lower() in _excluded_set or mdev_id.lower() in _excluded_set:
            continue
        ...
        if pconfig and pconfig.auth_type != "api_key":
            continue
        from pilotage_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(pilotage_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(pilotage_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        pilotage_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if has_creds:
            slugs.append(pilotage_id)
            seen.add(pilotage_id.lower())

    # --- Section 2: Pilotage-only providers (PILOTAGE_OVERLAYS) ---
    ...
    for pid, overlay in PILOTAGE_OVERLAYS.items():
        ...
        if pilotage_slug.lower() in seen:
            continue
        ...
        if not has_creds:
            try:
                ...
                if pid in providers_store or pilotage_slug in providers_store:
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
            try:
                ...
                pass
        if has_creds:
            slugs.append(pilotage_slug)
            seen.add(pid.lower())
            seen.add(pilotage_slug.lower())
...
def list_authenticated_providers(
    ...
    *,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from pilotage_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    for pilotage_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if slug.lower() in seen_slugs:
            continue
        if pilotage_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Pilotage may not route yet.
        ...
        # /model picker sees the SAME list `pilotage model` would build, with
        ...
        results.append({
            ...
            "is_current": (
                ...
                or pilotage_id == current_provider
    ...
    for pid, overlay in PILOTAGE_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                _ids = cached_provider_model_ids(pilotage_slug)
        ...
        _record_builtin_endpoint(pilotage_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or PILOTAGE_OVERLAYS (keeps /model in sync
    # with `pilotage model`).
    try:
        from pilotage_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Pilotage writes ``models:`` as a dict keyed by model id, but older
        ...
        for grp in ep_groups.values():
            ...
            if _discovery_allowed:
                try:
                    from pilotage_cli.models import cached_fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Pilotage's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, pilotage_cli/config.py) already consume that dict.
        ...
        for grp in groups.values():
            ...
            #   ``pilotage model``. Pin a dict catalog with
