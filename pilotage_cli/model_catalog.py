...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.pilotage/cache/model_catalog.json``.
...
# Remote catalog fetching is DISABLED by default: Pilotage has no hosted
# catalog yet. Set model_catalog.url (and enabled: true) in config.yaml to
# point at a self-hosted manifest using the same schema.
DEFAULT_CATALOG_URL = ""
...
def _cache_path() -> Path:
    ...
    return get_pilotage_home() / "cache" / "model_catalog.json"
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model Pilotage silently lands on when
    ...
    network-free. The cache is kept fresh by the picker/`pilotage update` paths;
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``pilotage update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.pilotage/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``pilotage model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
