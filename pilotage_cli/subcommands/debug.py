"""``pilotage debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Pilotage Agent. Use 'pilotage debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    pilotage debug share              Upload debug report (asks for confirmation)
    pilotage debug share --yes        Skip confirmation (for scripts/CI)
    pilotage debug share --lines 500  Include more log lines
    pilotage debug share --expire 30  Keep paste for 30 days
    pilotage debug share --local      Print report locally (no upload)
    pilotage debug share --no-redact  Disable upload-time secret redaction
    pilotage debug share --nous       Upload to Nous-internal storage (private)
    pilotage debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'pilotage debug share'",
    )
