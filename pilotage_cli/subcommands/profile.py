"""``pilotage profile`` subcommand parser.
...
def build_profile_parser(subparsers, *, cmd_profile: Callable) -> None:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Pilotage instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `pilotage update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Pilotage profile distribution. SOURCE can be a git URL "
