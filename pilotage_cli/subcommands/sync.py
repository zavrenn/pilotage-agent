"""``pilotage sync`` subcommand parser — Skill Sync.

Cloned from ``pilotage_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    pilotage sync status                 show gate/opt-in/head state
    pilotage sync pull                   pull and materialize opted-in skills
    pilotage sync push                   push opted-in skills
    pilotage sync now                    reconcile: pull then push
    pilotage sync enable <skill>         opt a skill into sync
    pilotage sync disable <skill>        opt a skill out of sync
    pilotage sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    pilotage sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  pilotage sync status            what is synced, and from where\n"
            "  pilotage sync enable my-skill   include a skill in your sync\n"
            "  pilotage sync now               pull, then push\n"
            "  pilotage sync propose my-skill  share a skill with your team\n"
        ),
