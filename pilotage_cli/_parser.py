...
_EPILOGUE = """
Examples:
    pilotage                        Start interactive chat
    pilotage chat -q "Hello"        Single query mode
    pilotage -c                     Resume the most recent session
    pilotage -c "my project"        Resume a session by name (latest in lineage)
    pilotage --resume <session_id>  Resume a specific session by ID
    pilotage --resume latest        Resume the most recent session (same as -c)
    pilotage setup                  Run setup wizard
    pilotage logout                 Clear stored authentication
    pilotage auth add <provider>    Add a pooled credential
    pilotage auth list              List pooled credentials
    pilotage auth remove <p> <t>    Remove pooled credential by index, id, or label
    pilotage auth reset <provider>  Clear exhaustion status for a provider
    pilotage model                  Select default model
    pilotage fallback [list]        Show fallback provider chain
    pilotage fallback add           Add a fallback provider (same picker as `pilotage model`)
    ...
    pilotage sessions browse        Interactive session picker
    pilotage sessions rename ID T   Rename/title a session
    pilotage logs                   View agent.log (last 50 lines)
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="pilotage",
        description="Pilotage Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `pilotage chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to PILOTAGE_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.pilotage/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Pilotage Agent",
    )
    ...
    # top-level parser: when the user writes `pilotage -m foo chat`, argparse
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also PILOTAGE_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.pilotage/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Pilotage itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
