"""Migrate Pilotage' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Pilotage' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Pilotage mcp_servers.<n>.command/args/env  → codex stdio transport
  Pilotage mcp_servers.<n>.url/headers       → codex streamable_http transport
  Pilotage mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Pilotage mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Pilotage-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Pilotage' own
...
MIGRATION_MARKER = (
    "# managed by pilotage-agent — `pilotage codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end pilotage-agent managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Pilotage config.")
...
_KEYS_DROPPED_WITH_WARNING = {
    # Pilotage' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, pilotage_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Pilotage MCP server config to the codex inline-table dict
    ...
    if not isinstance(pilotage_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(pilotage_cfg.get("command"))
    has_url = bool(pilotage_cfg.get("url"))
    ...
    if has_command:
        # Stdio transport
        out["command"] = str(pilotage_cfg["command"])
        args = pilotage_cfg.get("args") or []
        if args:
            out["args"] = [str(a) for a in args]
        env = pilotage_cfg.get("env") or {}
        if env:
            # Codex expects string values
            out["env"] = {str(k): str(v) for k, v in env.items()}
        cwd = pilotage_cfg.get("cwd")
        if cwd:
            out["cwd"] = str(cwd)
    elif has_url:
        # streamable_http transport (codex covers both http and SSE here)
        out["url"] = str(pilotage_cfg["url"])
        headers = pilotage_cfg.get("headers") or {}
        if headers:
            out["http_headers"] = {str(k): str(v) for k, v in headers.items()}
        # Pilotage' transport: sse hint is informational; codex auto-negotiates
        if pilotage_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]

    # Timeouts
    if "timeout" in pilotage_cfg:
        try:
            out["tool_timeout_sec"] = float(pilotage_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in pilotage_cfg:
        try:
            out["startup_timeout_sec"] = float(pilotage_cfg["connect_timeout"])
        except (TypeError, ValueError):
            skipped.append("connect_timeout (not numeric)")

    # Enabled flag (codex defaults to true so we only emit when explicitly false)
    if pilotage_cfg.get("enabled") is False:
        out["enabled"] = False

    # Detect keys we explicitly drop with warning
    for key in pilotage_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_PILOTAGE_KEYS:
            skipped.append(f"{key} (unknown Pilotage key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (PILOTAGE_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Pilotage)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Pilotage' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Pilotage' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Pilotage
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="pilotage-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a PILOTAGE_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed pilotage-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.pilotage`` symlink that happens to live under ``/private/var/folders``
...
def _build_pilotage_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Pilotage' own
    ...
    so a pilotage installed under /opt/, /usr/local/, or a venv all work.
    PILOTAGE_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # PILOTAGE_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_pilotage_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever PILOTAGE_HOME its launcher sets
    ...
    pilotage_home = os.environ.get("PILOTAGE_HOME") or ""
    if pilotage_home and _looks_like_test_tempdir(pilotage_home):
        pilotage_home = ""
    if pilotage_home:
        env["PILOTAGE_HOME"] = pilotage_home
    # PYTHONPATH passes through so a worktree-launched pilotage finds the
    # branch's modules instead of the installed package.
    pythonpath = os.environ.get("PYTHONPATH")
    if pythonpath:
        env["PYTHONPATH"] = pythonpath
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["PILOTAGE_QUIET"] = "1"
    env["PILOTAGE_REDACT_SECRETS"] = env.get("PILOTAGE_REDACT_SECRETS", "true")

    out: dict[str, Any] = {
        "command": sys.executable,
        "args": ["-m", "agent.transports.pilotage_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    pilotage_config: dict,
    *,
    codex_home: Optional[Path] = None,
    dry_run: bool = False,
    discover_plugins: bool = True,
    default_permission_profile: Optional[str] = ":workspace",
    expose_pilotage_tools: bool = True,
) -> MigrationReport:
    """Translate Pilotage mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        pilotage_config: full ~/.pilotage/config.yaml dict
        ...
        expose_pilotage_tools: when True (default), register Pilotage' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Pilotage for tools
    ...
    pilotage_servers = (pilotage_config or {}).get("mcp_servers") or {}
    if not isinstance(pilotage_servers, dict):
        report.errors.append(
            "mcp_servers in Pilotage config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in pilotage_servers.items():
    ...
    # Inject Pilotage' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Pilotage for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/pilotage_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_pilotage_tools:
        translated["pilotage-tools"] = _build_pilotage_tools_mcp_entry()
        if "pilotage-tools" not in report.migrated:
            report.migrated.append("pilotage-tools")
