"""
Backup and import commands for pilotage CLI.

`pilotage backup` creates a zip archive of the entire ~/.pilotage/ directory
(excluding the pilotage-agent repo and transient files).

`pilotage import` restores from a backup zip, overlaying onto the current
PILOTAGE_HOME root.
"""
...
from pilotage_constants import get_default_pilotage_root, get_pilotage_home, display_pilotage_home
from utils import (
    _preserve_file_mode,
...
from pilotage_cli.sizefmt import format_bytes as _format_size
...
# ``pilotage-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/pilotage-agent/``
...
# PILOTAGE_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "pilotage-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under PILOTAGE_HOME) —
...
# File names that ``pilotage import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE PILOTAGE_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Pilotage backup slot."""
...
@contextmanager
def _backup_operation_lock(pilotage_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = pilotage_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Pilotage backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside PILOTAGE_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to pilotage root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``pilotage-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/pilotage-agent/`` — must be preserved.
        if part == "pilotage-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside PILOTAGE_HOME.
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # pilotage_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the Pilotage home directory."""
    pilotage_root = get_default_pilotage_root()

    if not pilotage_root.is_dir():
        print(f"Error: Pilotage home directory not found at {pilotage_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(pilotage_root):
            _run_backup_locked(args, pilotage_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, pilotage_root: Path) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"pilotage-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"pilotage-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_pilotage_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(pilotage_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(pilotage_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``pilotage-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "pilotage-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(pilotage_root)
    ...
    # outside PILOTAGE_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_pilotage_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: pilotage import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Pilotage backup.
    ...
    # Look for telltale files that a pilotage home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Pilotage backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.pilotage/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a pilotage dir name
        if prefix in {".pilotage", "pilotage"}:
            return prefix + "/"
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``pilotage import``, which is the
    ...
    so without the chown a ``sudo pilotage import`` would silently re-own every
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``pilotage import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to Pilotage' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_pilotage_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (pilotage_root / "config.yaml").exists()
        has_env = (pilotage_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Pilotage configuration.")
        ...
        pilotage_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under PILOTAGE_HOME.
            ...
            target = pilotage_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(pilotage_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_pilotage_home()}."
            )
        ...
        profiles_dir = pilotage_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # pilotage_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: pilotage profile list  (after installing pilotage)")

        # Guidance
        print()
        if not (pilotage_root / "pilotage-agent").is_dir():
            print("Note: The pilotage-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: pilotage update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  pilotage -p {pname} gateway install")
        ...
        try:
            from pilotage_cli.gateway import ensure_gateway_service, _is_service_running
        ...
        print("Done. Your Pilotage configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and pilotage backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to PILOTAGE_HOME).
...
def _quick_snapshot_root(pilotage_home: Optional[Path] = None) -> Path:
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: pilotage snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(pilotage_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside PILOTAGE_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``pilotage update``.
    ...
    Args:
        ...
        pilotage_home: Override for the Pilotage home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(pilotage_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for pilotage backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_pilotage_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, pilotage_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(pilotage_root):
            return _write_full_zip_backup_locked(out_path, pilotage_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, pilotage_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``pilotage_root`` to ``out_path``.
...
def _pre_update_backup_dir(pilotage_home: Optional[Path] = None) -> Path:
    home = pilotage_home or get_pilotage_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of PILOTAGE_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<PILOTAGE_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``pilotage update``) should continue even if the backup fails.
    """
    pilotage_root = pilotage_home or get_default_pilotage_root()
    if not pilotage_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(pilotage_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, pilotage_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `pilotage claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of PILOTAGE_HOME under ``backups/`` before a
    ``pilotage claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``pilotage import <archive>``.  Writes to
    ``<PILOTAGE_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `pilotage import` and the
