"""Pilotage update pipeline — extracted from ``pilotage_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``pilotage_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``pilotage_cli.main`` reference — so existing call sites and test monkeypatches
that target ``pilotage_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``pilotage_cli.main``.
...
Imports are one-way: ``pilotage_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``pilotage_cli.main`` reference.

    Lets callers keep patching ``pilotage_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``pilotage_cli.main`` -> ``pilotage_cli.update_cmd`` stays one-way
    at import time.
    """
    from pilotage_cli import main
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``pilotage update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``pilotage update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``pilotage_cli.config_defaults``,
    ``pilotage_cli.config``, and ``pilotage_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.
    """
    import importlib

    importlib.invalidate_caches()
    for mod_name in ("pilotage_cli.config_defaults", "pilotage_cli.config", "pilotage_cli.config_migrations"):
...
def _run_config_check_fresh() -> tuple:
    ...
    from pilotage_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from pilotage_cli.config import migrate_config

    return migrate_config(interactive=interactive, quiet=quiet)


# Critical files that Pilotage must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "pilotage_cli/main.py",
    "pilotage_cli/config.py",
    "pilotage_cli/__init__.py",
    "pilotage_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "pilotage_constants.py",
)
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``pilotage`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="pilotage-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``pilotage update`` can be driven by a
    ...
    """
    from pilotage_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from pilotage_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('pilotage_'):\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``pilotage update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `pilotage update`.
...
def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `pilotage update`.
    ...
    try:
        ...
        return
    db_path = get_pilotage_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `pilotage sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    pilotage sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  pilotage sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``pilotage update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``pilotage update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: pilotage curator status)"
    )
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.pilotage-update-staging"
    backup = f"{dst}.pilotage-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `pilotage update`" advice with
...
def _print_update_completion(message: str) -> None:
    ...
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== pilotage-update completed {action_id} ===")


def _update_via_zip(args):
    """Update Pilotage Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `pilotage update "
            f"--branch {branch}`, or update against main with `pilotage update`."
        )
        _m().sys.exit(1)
    zip_url = (
    ...
    tmp_dir = tempfile.mkdtemp(prefix="pilotage-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"pilotage-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for pilotage-agent itself
        ...
        # GitHub ZIPs extract to pilotage-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"pilotage-agent-{branch}")
    ...
    from pilotage_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if not import_ok:
        ...
        print("  Re-run `pilotage update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: pilotage skills list-modified  "
    ...
    try:
        from pilotage_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from pilotage_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_pilotage_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_pilotage_home())
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "pilotage-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Pilotage behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes pilotage completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Pilotage couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Pilotage couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Pilotage behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Pilotage couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Pilotage couldn't drop "
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_pilotage_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_pilotage_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    ``pilotage update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from pilotage_constants import get_default_pilotage_root

    default_home = get_default_pilotage_root()
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "pilotage.exe"
    lines = ["✗ Another pilotage.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Pilotage Desktop, exit any open `pilotage` REPLs, and")
    lines.append("  stop the gateway (`pilotage gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `pilotage update --force` if you've already")
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``pilotage tools``."""
    try:
        from pilotage_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Pilotage Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``pilotage tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Pilotage Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Pilotage Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``pilotage update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `pilotage update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `pilotage update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    try:
        from pilotage_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from pilotage_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$PILOTAGE_HOME/bin/uv``, but on Termux the official
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.pilotage/logs/update.log`` only, never the terminal.

    During ``pilotage update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``pilotage update --check``: fetch and report without installing.
    ...
    """
    from pilotage_cli.config import detect_install_method, recommended_update_command_for_method
    method = detect_install_method(_m().PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``pilotage update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from pilotage_cli.config import format_docker_update_message
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from pilotage_cli.banner import _github_compare_behind
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``pilotage update`` without requiring a reinstall.  The
    ...
    ``pilotage`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/pilotage.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v pilotage'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/pilotage, code at /usr/local/lib/pilotage-agent).
    fhs_link = Path("/usr/local/bin/pilotage")
    ...
    path_comment = (
        "# Pilotage Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    snapshot_id = None
    try:
        from pilotage_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from pilotage_constants import
        # get_pilotage_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from pilotage_cli.config import get_pilotage_home as _get_home
    ...
    try:
        from pilotage_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_pilotage_home so the user sees ~/.pilotage/...
    try:
        from pilotage_constants import get_pilotage_home, display_pilotage_home

        home = get_pilotage_home()
        try:
            display_path = f"{display_pilotage_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  pilotage import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``pilotage update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``pilotage update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run pilotage from any interpreter), so report healthy to
        # avoid forcing reinstalls. But when an interrupted update leaves
        # `.update-incomplete`, the venv IS the install — its absence means a repair got interrupted after
        # the old venv was moved aside, and "Already up to date!" would
        # gaslight the user while nothing can run.
        managed_markers = (_m()._update_marker_path(),)
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The pilotage.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m pilotage_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``pilotage
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m pilotage_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "pilotage_cli.main" in cmdline_low:
        ...
        # the `-m pilotage_cli.main gateway run` argv, so autostarted gateways
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `pilotage` launch will complete it")
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Pilotage processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Pilotage Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Pilotage desktop app / other Pilotage terminals, then re-run:"
    )
    lines.append("    pilotage update")
    lines.append("  (or use `pilotage update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``pilotage update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from pilotage_cli._scan_venv_blockers import _is_pausable_gateway
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    pilotage.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating Pilotage...")
    try:
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: pilotage gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``pilotage gateway start`` uses (``gateway_windows._spawn_detached``).
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``pilotage-gateway*.service`` from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not unit.startswith("pilotage-gateway"):
            continue
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``pilotage_cli.gateway`` itself no longer imports, which is one of the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    try:
        ...
        cache_dir = Path(_m().get_pilotage_home()) / "bootstrap-cache"
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``pilotage update`` sees a clean tree instead of autostashing every run.
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_pilotage_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `pilotage serve` backend) will keep .pyd
    ...
    # the pilotage.exe shim guard above, but its lock probe only checks the shim
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Pilotage windows/gateways and re-run: pilotage update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Pilotage gateways, Desktop backends, or other "
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``pilotage
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``pilotage update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``pilotage update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``pilotage doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && pilotage update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # the old bytecode (e.g. get_pilotage_home added to pilotage_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``pilotage`` launch finishes the install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `pilotage` again "
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_pilotage_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.pilotage/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit PILOTAGE_HOME so
        # it is not affected by sync_skills()'s module-level PILOTAGE_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's PILOTAGE_HOME env var points at the default or a named profile.
        try:
        ...
        # ``pilotage update`` process is the PRE-pull Python process — its
        # ``sys.modules`` cache holds the OLD ``pilotage_cli.config`` and
        # ``pilotage_cli.config_migrations`` from before ``git pull`` updated
        # the source files. A function-level ``from pilotage_cli.config import
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'pilotage config migrate' to retry.")
            ...
            if assume_yes:
                ...
                try:
                    ...
                    print(
                        ...
                        "Run 'pilotage config migrate' manually to configure."
                    )
                    response = "n"
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: pilotage config migrate")
            else:
                print()
                print("Skipped. Run 'pilotage config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from pilotage_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `pilotage curator status`. Self-stamps after printing so it
        ...
        # When running as ``pilotage update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_pilotage_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... pilotage-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... pilotage-gateway*)
            ...
            try:
                from pilotage_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all pilotage-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: pilotage gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `pilotage update` restart it automatically, allow\n"
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: pilotage gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: pilotage -p <profile> gateway run  for each profile)"
                        )
        ...
        try:
            from pilotage_cli.gateway import (
                has_legacy_pilotage_units,
                _find_legacy_pilotage_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_pilotage_units():
                print()
                print("⚠ Legacy Pilotage gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_pilotage_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (pilotage.service) fight the current")
                print("  pilotage-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    pilotage gateway migrate-legacy")
                print()
        ...
        print("  pilotage model              # Select provider and model")
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``pilotage_cli.gateway`` no longer imports,
