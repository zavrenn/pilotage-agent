...
``pilotage_cli.config.migrate_config``. Each step is a function
...
``pilotage_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``pilotage_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``pilotage_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``pilotage_cli.config`` (e.g.
``patch("pilotage_cli.config.read_raw_config", ...)``) keep working, because
...
#: PILOTAGE_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_pilotage_home()}/config.yaml and run `pilotage setup` to "
...
def _cfg():
    """Return the live ``pilotage_cli.config`` module (lazy, cycle-free)."""
    from pilotage_cli import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$PILOTAGE_HOME/plugins/`` for currently installed user plugins.
        grandfathered: List[str] = []
        try:
            user_plugins_dir = get_pilotage_home() / "plugins"
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `pilotage plugins enable <name>` to activate."
                )
