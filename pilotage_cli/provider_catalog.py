...
The provider list shown by ``pilotage model`` (CLI/TUI) and the desktop Settings
→ Providers tabs (Accounts + API keys) **must be the same set**.  Historically
they were not: the CLI picker read :data:`pilotage_cli.models.CANONICAL_PROVIDERS`
...
* ``auth_type`` / ``api_key_env_vars`` / ``base_url_env_var`` from
  :data:`pilotage_cli.auth.PROVIDER_REGISTRY` (credential truth), and
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``pilotage model`` shows.
"""
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `pilotage model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``pilotage model`` universe.
    ...
    # Pilotage overlays carry auth_type for providers that have no registry/profile
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = PILOTAGE_OVERLAYS.get(slug)

        # auth_type: registry is authoritative; fall back to profile, then the
        # Pilotage overlay (e.g. moa → "virtual"), then api_key.
        auth_type = (
