"""``pilotage approvals suggest`` — mine approval history into allowlist proposals.

Pilotage has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.pilotage/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, pilotage self-management, …) are eligible.
...
def normalize_command(command: str) -> str:
    """Fold user/pilotage home prefixes and collapse whitespace.

    Reuses tools.approval's home-folding machinery so proposals are portable
    across machines/users (``/home/alice/x`` -> ``~/x``).
    """
    from tools.approval import (
        _rewrite_resolved_pilotage_home,
        _rewrite_resolved_user_home,
    )

    folded = _rewrite_resolved_user_home(_rewrite_resolved_pilotage_home(command))
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  pilotage approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.pilotage/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``pilotage approvals suggest``."""
...
def approvals_command(args) -> int:
    """Dispatch ``pilotage approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from pilotage_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: pilotage approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `pilotage approvals <subcommand> -h` for details."
    )
    return 1
