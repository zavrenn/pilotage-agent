"""Shell completion script generation for pilotage CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_pilotage_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Pilotage Agent bash completion
# Add to ~/.bashrc:
#   eval "$(pilotage completion bash)"

_pilotage_profiles() {{
    local profiles_dir="$HOME/.pilotage/profiles"
...
_pilotage_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_pilotage_profiles)" -- "$cur"))
        return
    fi
...
complete -F _pilotage_completion pilotage
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef pilotage
# Pilotage Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(pilotage completion zsh)"

_pilotage_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.pilotage/profiles" ]]; then
        profiles+=($HOME/.pilotage/profiles/*(N/:t))
    fi
...
_pilotage() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_pilotage_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'pilotage command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Pilotage Agent fish completion",
        "# Add to your config:",
        "#   pilotage completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __pilotage_profiles",
        "    echo default",
        "    if test -d $HOME/.pilotage/profiles",
        "        for d in $HOME/.pilotage/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c pilotage -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c pilotage -f -s p -l profile"
        " -d 'Profile name' -xa '(__pilotage_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__pilotage_profiles)' -d 'Profile name'"
                )
