"""Offline, non-destructive recovery for a damaged Pilotage session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    keeps this path consistent with ``pilotage_state._backup_db_file``.
    """
    from pilotage_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
...
def _snapshot_and_inspect(
    ...
    temp_dir = tempfile.TemporaryDirectory(
        prefix="pilotage-session-recovery-",
        dir=str(work_root),
    )
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Pilotage process using this profile and retry. "
                "This includes the interactive `pilotage` CLI session this "
                ...
                "Pilotage process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `pilotage` session open, "
...
def _recover_via_lost_and_found(
    *,
    ...
    from pilotage_cli.session_lost_and_found import (
