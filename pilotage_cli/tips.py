...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or pilotage -c.",
    ...
    "Ctrl+Z suspends Pilotage to the background — run fg in your shell to resume.",
    ...
    "pilotage chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "pilotage chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "pilotage --yolo bypasses all dangerous command approval prompts for the entire session.",
    "pilotage chat --source telegram tags the session for filtering in pilotage sessions list.",
    ...
    "pilotage config check scans for missing or stale configuration options.",
    "pilotage sessions browse opens an interactive session picker with search.",
    "pilotage sessions stats shows session counts by platform and database size.",
    "pilotage sessions prune --older-than 30 cleans up old sessions.",
    "pilotage skills search react --source skills-sh searches the skills.sh public directory.",
    "pilotage skills check scans installed hub skills for upstream updates.",
    "pilotage skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "pilotage skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "pilotage mcp add github --command npx adds MCP servers from the command line.",
    ...
    "pilotage gateway install sets up Pilotage as a system service (systemd/launchd).",
    "pilotage memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "pilotage webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: pilotage tools disables unused tools, pilotage skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "pilotage models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as pilotage -w).",
    ...
    "PILOTAGE_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    "Profile names become shell commands — 'pilotage profile create coder' creates the 'coder' command.",
    "pilotage profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Pilotage prints a resume command with session ID and stats.",
    "pilotage sessions export backup.jsonl exports all sessions for backup or analysis.",
    "pilotage -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "pilotage skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .pilotage/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: pilotage cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Pilotage runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "pilotage gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Pilotage via a local macOS server.",
    ...
    "Context files (.pilotage.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Pilotage loads project context from .pilotage.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "pilotage mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "pilotage mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "pilotage mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — pilotage tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.pilotage/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "pilotage chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    # --- Miscellaneous ---
    "Prompt caching reduces costs by reusing cached system prompt prefixes.",
    ...
    "pilotage acp runs Pilotage as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "PILOTAGE_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "pilotage auth add openai-codex sets up OAuth-based providers.",
    ...
    "pilotage status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.pilotage/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Pilotage your own.",
    ...
    "Before a gateway session reset, Pilotage auto-flushes important facts to memory in the background.",
    ...
    "The gateway caches AIAgent instances per session — destroying this cache breaks prompt caching.",
    "Any website can expose skills via /.well-known/skills/index.json — the skills hub discovers them automatically.",
    "The skills audit log at ~/.pilotage/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "pilotage claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "PILOTAGE_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via pilotage tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.pilotage/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.pilotage/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Pilotage auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Pilotage config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Pilotage is working.',
    ...
    "PILOTAGE_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.pilotage/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    ...
    'PILOTAGE_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'PILOTAGE_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'PILOTAGE_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'PILOTAGE_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    'pilotage -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'pilotage chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'pilotage chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'pilotage chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    ...
    'PILOTAGE_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'PILOTAGE_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.pilotage/plugins/ — trust-gated by design.',
    "PILOTAGE_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'PILOTAGE_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'PILOTAGE_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'PILOTAGE_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'PILOTAGE_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'PILOTAGE_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'PILOTAGE_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'PILOTAGE_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'PILOTAGE_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'PILOTAGE_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'PILOTAGE_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Pilotage block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from pilotage sessions list by default — set --source explicitly to see them.',
