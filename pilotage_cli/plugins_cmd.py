"""``pilotage plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.pilotage/plugins/``.
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other Pilotage subprocess resolution: :func:`shutil.which` first,
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.pilotage/.env``."""
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from pilotage_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_pilotage_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_pilotage_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_pilotage_home()}/.env later)[/dim]")
...
def _install_metadata_path() -> Path:
    return get_pilotage_home() / "plugins" / _INSTALL_METADATA_FILE
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update Pilotage.",
                ) from None

        if target.exists() and not force:
            raise PluginOperationError(
                f"Plugin '{plugin_name}' already exists. Use force reinstall "
                f"or run `pilotage plugins update {plugin_name}`."
            )
...
def _resolve_index_name(identifier: str, console) -> tuple[str, Optional[str]]:
    ...
    """
    from pilotage_cli.plugin_index import SECURITY_FOOTER, load_index, resolve_name
    ...
    if entry is None:
        if len(candidates) > 1:
            ...
            console.print(
                ...
                f"community index ({source}). Use `pilotage plugins search <term>` to "
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (target / "plugin.json").exists() and not (
        ...
        console.print(
            ...
            f"plugin.json, or __init__.py. It may not be a valid Pilotage plugin.",
        )
    ...
    console.print("[dim]  pilotage gateway restart[/dim]")
    console.print()
...
def cmd_update(name: str) -> None:
    ...
    if install_record.get("pinned") is True:
        ...
        console.print(
            ...
            f"`pilotage plugins install {recorded_source} --force "
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] pilotage plugins install owner/repo")
        return
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as Pilotage
    ...
    try:
        from pilotage_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.pilotage/plugins/<name>`` if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir, allow_subdir=True)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.pilotage/plugins/<name>``."""
    ...
    if install_record.get("pinned") is True:
        ...
        return {
            ...
            "error": (
                ...
                f"run `pilotage plugins install {recorded_source} --force "
    ...
    # Sibling of the CLI ``pilotage plugins update`` path: drop bytecode
    ...
    try:
        ...
        if dirty:
            ...
            push = _run_plugin_git(
                ...
                "-m", "pilotage-plugin-update-autostash",
            )
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.pilotage/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def cmd_search(
    ...
    *,
    ...
    from pilotage_cli.plugin_index import (
        SECURITY_FOOTER,
        load_index,
        search_index,
    )
    ...
    console.print(table)
    console.print(f"[dim]Index source: {source}. Install: pilotage plugins install <name>[/dim]")
    console.print(f"[dim]{SECURITY_FOOTER}[/dim]")
...
def plugins_command(args) -> None:
    ...
    if action == "install":
        ...
        from pilotage_cli.plugin_packs import pack_command
