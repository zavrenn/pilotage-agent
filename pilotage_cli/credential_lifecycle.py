"""Unified provider-credential lifecycle across every store Pilotage reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.pilotage/.env``                     — the canonical secret store
    2. ``~/.pilotage/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.pilotage/config.yaml``              — inline mirrors written by the
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `pilotage auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    """
    from pilotage_cli.config import load_env, save_env_value
    ...
    # that reads this var.
    try:
        from pilotage_cli.auth import unsuppress_credential_source
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    """
    from pilotage_cli.config import load_env, remove_env_value
