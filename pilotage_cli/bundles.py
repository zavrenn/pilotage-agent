"""Implementation of the ``pilotage bundles`` CLI subcommand.

Mirrors the structure of ``pilotage_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `pilotage bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  pilotage bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``pilotage bundles`` argparse tree.

    Called from ``pilotage_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `pilotage bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``pilotage bundles <subcommand>`` to the right handler."""
