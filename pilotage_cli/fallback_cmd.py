"""
pilotage fallback — manage the fallback provider chain.
...
Subcommands:
  pilotage fallback [list]   Show the current fallback chain (default when no subcommand)
  pilotage fallback add      Pick provider + model via the same picker as `pilotage model`,
                           then append the selection to the chain
  pilotage fallback remove   Pick an entry to delete from the chain
  pilotage fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.pilotage/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from pilotage_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `pilotage model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  pilotage fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `pilotage model`, then append the selection to the chain."""
    from pilotage_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `pilotage model` — select the provider + model you want as a fallback.")
    print()
    ...
    # semantics owned by agent.backend_identity //): same
    ...
    print()
    print("  Run `pilotage fallback list` to view, or `pilotage fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``pilotage fallback [subcommand]``."""
