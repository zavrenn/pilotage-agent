"""
Skills configuration for Pilotage Agent.
`pilotage skills` enters this module.

Toggle individual skills or categories on/off, globally or per-platform.
Config stored in ~/.pilotage/config.yaml under:
...
from pilotage_cli.config import cfg_get, load_config, save_config
from pilotage_cli.colors import Colors, color
from pilotage_cli.platforms import PLATFORMS as _PLATFORMS
...
def skills_command(args=None):
    """Entry point for `pilotage skills`."""
    from pilotage_cli.curses_ui import curses_checklist
