...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from pilotage_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `pilotage model` to reactivate. "
                "Showing the saved model only."
            )
            extras.append(
                {
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            if auth_type == "api_key" and key_env
            else f"run `pilotage model` to configure ({auth_type})"
        )
