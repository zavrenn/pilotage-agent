...
# $PILOTAGE_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Pilotage-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $PILOTAGE_HOME/models_dev_cache.json directly (no network). Falls
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Pilotage that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Pilotage that doesn't ship
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_pilotage_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$PILOTAGE_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# In-repo fallback for the model Pilotage silently lands on when the user never
...
# ``pilotage model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in pilotage_cli/web_server.py and
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    pilotage-agent is tool-calling-first — every provider path assumes the model
...
# pilotage_constants.VALID_REASONING_EFFORTS ("none" included so an explicit
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from pilotage_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Pilotage lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from pilotage_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return pilotage-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; pilotage's picker expects
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``pilotage auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.pilotage/.env``.
    ...
    try:
        from pilotage_cli.auth import read_credential_pool
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from pilotage_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a Pilotage release.
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _PILOTAGE_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Pilotage' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Pilotage' native Anthropic adapter. The live catalog may
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from pilotage_constants import get_pilotage_home
    return get_pilotage_home() / "ollama_cloud_models_cache.json"
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `pilotage moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
