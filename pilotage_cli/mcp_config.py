...
from pilotage_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_pilotage_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from pilotage_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.pilotage/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``pilotage mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from tools.mcp_oauth import PilotageTokenStorage
        return PilotageTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # pilotage_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  pilotage mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  pilotage mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  pilotage mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_pilotage_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: pilotage mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_pilotage_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `pilotage mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  pilotage mcp add <name> --url <endpoint>')
        _info('  pilotage mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``pilotage mcp login`` and
    ``pilotage mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `pilotage mcp remove` + `pilotage mcp add` to reconfigure auth.")
        return False
    ...
    #
    # force_interactive_oauth: `pilotage mcp login` is *explicitly* user-
    # initiated even when stdin isn't a TTY (Pilotage desktop / agent-
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `pilotage mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``pilotage mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``pilotage mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: pilotage mcp reauth <name>   |   pilotage mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
# ─── pilotage mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'pilotage mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``pilotage mcp`` subcommands."""
    ...
    if action == "picker":
        from pilotage_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from pilotage_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from pilotage_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `pilotage plugin`.
        from pilotage_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("pilotage mcp                                    Open the catalog picker (default)")
        _info("pilotage mcp catalog                            List Nous-approved MCPs")
        _info("pilotage mcp install <name>                     Install a catalog MCP")
        _info("pilotage mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("pilotage mcp add <name> --command <cmd>         Add a stdio server")
        _info("pilotage mcp add <name> --preset <preset>       Add from a known preset")
        _info("pilotage mcp remove <name>                      Remove a server")
        _info("pilotage mcp list                               List configured servers")
        _info("pilotage mcp test <name>                        Test connection")
        _info("pilotage mcp configure <name>                   Toggle tools")
        _info("pilotage mcp login <name>                       Re-authenticate OAuth")
        _info("pilotage mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
