"""Slash command definitions and autocomplete for the Pilotage CLI.

Central registry for all slash commands. Every consumer -- CLI help, gateway
dispatch, Telegram BotCommands, autocomplete --
...
from pilotage_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``pilotage_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``pilotage_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("title", "Set a title for the current session", "Session",
               args_hint="[name]"),
    CommandDef("handoff", "Hand off this session to a messaging platform (Telegram, WhatsApp)", "Session",
    ...
    CommandDef("goal", "Set a standing goal Pilotage works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey Pilotage' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Pilotage is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.pilotage/skills/ for newly installed or removed skills",
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`pilotage_cli.plugins.PluginContext.register_command`. They behave
...
# Telegram allows up to 100 BotCommands. Pilotage ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Pilotage
...
_CMD_NAME_LIMIT = 32
"""Max command name length for platform slash commands."""
...
def _clamp_command_names(
    ...
    Platform slash command names are capped at 32 characters.
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``pilotage skills
