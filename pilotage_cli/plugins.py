"""
Pilotage Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with pilotage-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.pilotage/plugins/<name>/``
3. **Project plugins** – ``./.pilotage/plugins/<name>/`` (opt-in via
   ``PILOTAGE_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``pilotage_agent.plugins``
   entry-point group.
...
from pilotage_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
from pilotage_cli.plugin_capabilities import (  # noqa: F401 — re-exported
    CAPABILITY_REGISTRY,
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``PILOTAGE_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("PILOTAGE_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``PILOTAGE_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.pilotage/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("PILOTAGE_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When PILOTAGE_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Pilotage loggers.
    """
    ...
    logger.debug(
        "PILOTAGE_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Pilotage' shipped guidance lives in the evidence-based
    ...
    "pre_transcription",
    # Kanban task lifecycle hooks. Fired by pilotage_cli.kanban_db when a task
    ...
    # separate `pilotage -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `pilotage kanban dispatch`),
    ...
    # ``pilotage kanban dispatch``); on_kanban_task_updated fires in whichever
    ...
    #   result: pilotage_cli.kanban_db.DispatchResult (spawned, reclaimed,
    ...
    #       Fired today: Telegram "reaction" + "message_edited"; other
...
ENTRY_POINTS_GROUP = "pilotage_agent.plugins"
ENTRY_POINT_CAPABILITIES_GROUP = "pilotage_agent.plugin_capabilities"
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    * **Capability declarations** — read from the companion
      ``pilotage_agent.plugin_capabilities`` entry-point group (declarations
...
PLUGIN_SECTIONS_START = "<!-- pilotage-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- pilotage-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- pilotage-plugin-section-chars:{len(content)} -->\n\n"
        f"{content}"
    )
...
# Reserved event namespace prefix — only core may publish ``pilotage:<event>``.
PILOTAGE_EVENT_NAMESPACE = "pilotage"
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Pilotage home."""
    token = set_pilotage_home_override(home)
    try:
        yield
    finally:
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "capabilities", "emits", "listens", "pilotage", "depends",
}

# Highest manifest schema version this Pilotage understands.
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Pilotage "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `pilotage plugins enable %s` if "
                    "it is installed.",
                    k, dep_id, dep_id,
                )
                continue
...
class PluginManifest:
    ...
    # ``platform``: gateway messaging platform adapter (e.g. Telegram). Bundled
    #              platform plugins auto-load so every shipped platform is
    #              available out of the box; user-installed platform plugins
    #              in ~/.pilotage/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``pilotage plugins list``. For a flat plugin at
    ...
    # (``pilotage plugins show``); a plugin may emit/subscribe without declaring.
...
class PluginState:
    ...
    def data_dir(self) -> Path:
        ...
        return get_pilotage_home() / "plugin-data" / self._data_namespace
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        # threads. Other Pilotage config writers still retain their existing
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from pilotage_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        """Return the active Pilotage profile name (e.g. ``"default"``).
        ...
        running under ``~/.pilotage/profiles/<name>``, or ``"custom"`` when
        ``PILOTAGE_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Pilotage core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``pilotage honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``pilotage <subcommand>``
        ...
        command with an argument field — for example a native slash
        command picker. Plugin commands without ``args_hint`` register as
        parameterless natively and still accept trailing text when invoked
        as free-form chat.
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``pilotage model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: pilotage_cli.main imports plugins indirectly
        from pilotage_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        including ``pilotage:x`` or a foreign ``other:x``) is rejected with a
        ``ValueError`` and a logged warning — fail-closed. The ``pilotage:``
        ...
        if ":" in event:
            logger.warning(
                ...
                plugin_key, event, plugin_key, PILOTAGE_EVENT_NAMESPACE,
            )
            raise ValueError(
                f"Plugin '{plugin_key}' may not emit '{event}': emit only the "
                f"bare event name; the namespace is forced to '{plugin_key}:' "
                f"and the '{PILOTAGE_EVENT_NAMESPACE}:' prefix is reserved for core"
            )
    ...
    def subscribe(self, event: str, callback: Callable) -> None:
        ...
        *event* is the full ``<plugin_key>:<event>`` name (or ``pilotage:<event>``
    ...
    def register_skill(
        ...
        ``~/.pilotage/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # original scope.
        self.scope_key = scope_key or pilotage_home_key()
        ...
        # — i.e. per (pilotage_home, plugin_id) — and every release/restore
        ...
        # contributed, so `pilotage plugins list` still attributes them once the
        # full plugin loads.
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if env_var_enabled("PILOTAGE_SAFE_MODE"):
                logger.info("PILOTAGE_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with pilotage and must
            ...
            # Bundled platform plugins (gateway adapters: telegram,
            # whatsapp) are registered LAZILY. Their modules import
            # heavy, platform-specific SDKs at module level (lark_oapi,
            # python-telegram-bot, baileys, ...), so eagerly loading
            # all ~20 of them added several seconds to every `pilotage`
            # invocation — including plain `pilotage chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Pilotage ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `pilotage plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from pilotage_cli.approval_transport import RegisteredApprovalTransport
        ...
        self._approval_transports[clean] = RegisteredApprovalTransport(
            ...
            present=present_fn,
            plugin_id=plugin_id,
            profile_home=str(get_pilotage_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None:
            return None
        if registered.profile_home != str(get_pilotage_home().resolve()):
            return None
        return registered
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        # 2. User plugins (~/.pilotage/plugins/)
        user_dir = get_pilotage_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.pilotage/plugins/), only when explicitly opted
        # in. This must match the full discovery gate exactly.
        if _env_enabled("PILOTAGE_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".pilotage" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set PILOTAGE_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        """
        if _env_enabled("PILOTAGE_SAFE_MODE"):
            return False
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from pilotage_cli.agent_plugins import _discover_mcp
    ...
    def _classify_entrypoint_kind(self, ep) -> str:
        ...
        module's import cost in every Pilotage process (e.g. a pip
    ...
    def _scan_entry_points(self) -> List[PluginManifest]:
        ...
        declarations from the ``pilotage_agent.plugin_capabilities`` group.
    ...
    def _platform_name_from_manifest(self, manifest: PluginManifest) -> str:
        """Derive the gateway platform name (e.g. ``telegram``) for a platform plugin.
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``pilotage plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`pilotage plugins
    ...
    def _register_deferred_platform_tools(
        ...
        try:
            ...
            # `pilotage plugins list` under-reports what the process is actually
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Pilotage does not install plugin dependencies "
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from pilotage_cli.agent_plugins import load_agent_plugin

            package = load_agent_plugin(
                Path(manifest.path),
                get_pilotage_home() / "plugin-data" / manifest.skill_namespace,
            )
            ctx = PluginContext(manifest, self)
    ...
    def _load_directory_module(
        ...
        *,
        ...
        """Import a directory-based plugin as ``pilotage_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``pilotage_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
        ...
        # from a different Pilotage home (profile switch reusing a slug
        # like "pilotage-lcm") or from an earlier force=True reload in the
...
# Keyed cache: resolved Pilotage home -> PluginManager. Pilotage supports
# multiple profiles via different PILOTAGE_HOME directories, and a single
# long-lived process (gateway multiplexer, test session, embedder) can
# switch between them via ``set_pilotage_home_override()`` — which is a
# ContextVar and deliberately does NOT touch os.environ (see
# pilotage_constants.set_pilotage_home_override). A process-wide single-slot
...
def _plugin_home_key() -> Path:
    ...
    Plugins are discovered from ``get_pilotage_home() / "plugins"`` and some
    plugins (notably context engines such as pilotage-lcm) capture that home
    at registration time for profile-scoped storage. A long-lived process
    can temporarily switch Pilotage home (env var *or* the context-local
    ``set_pilotage_home_override()``) while serving another profile, so the
    plugin manager must be scoped to the active Pilotage home instead of
    being one process-wide singleton.
    """
    try:
        return get_pilotage_home().expanduser().resolve()
    except Exception:
        return get_pilotage_home().expanduser()
...
def _clear_plugin_submodules(manager: Optional[PluginManager]) -> None:
    ...
    ``pilotage_plugins.<slug>`` and registers that top-level module in
    ...
    *relative* import (``from . import foo``, ``from .sub import bar``)
    ...
    ``pilotage_plugins.<slug>.<submodule>``. When we swap in a fresh manager
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Pilotage profile/home.
    ...
    profile switch — via ``PILOTAGE_HOME`` or the context-local
    ``set_pilotage_home_override()`` — gets its own manager with its own
    ...
    with _plugin_managers_lock:
        ...
        if manager is None:
            manager = PluginManager(scope_key=pilotage_home_key(current_home))
...
def _plugin_toolset_keys_cache_path():
    ...
    return get_pilotage_home() / "cache" / "plugin_toolset_keys.json"
...
def _get_pre_tool_call_directive_details(
    ...
    from pilotage_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="pilotage-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_subscriptions() -> Dict[str, List[Callable]]:
    ...
    (``<plugin_key>:<event>`` or ``pilotage:<event>``) to subscriber callbacks in
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``pilotage tools`` TUI so plugin-provided toolsets appear
