"""Bounded product contract for the first Pilotage shared-metrics slice."""
...
SCHEMA_KEY = "pilotage.metrics.schema_version"
SCHEMA_VERSION = "pilotage.metrics.event.v2"
MODEL_CALL_SCOPE = "pilotage.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "pilotage.task_run"
TOOL_CALL_SCOPE = "pilotage.tool_call"
CLIENT_ACTIVE_MARK = "pilotage.client.active"
TOOL_APPROVAL_MARK = "pilotage.tool_approval"
SKILL_LIFECYCLE_MARK = "pilotage.skill.lifecycle"
SKILL_LOAD_MARK = "pilotage.skill.load"
SUBSCRIBER_NAME = "pilotage.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "pilotage.client.active"
LEGACY_MODEL_CALL_METRIC = "pilotage.model_call.count"
MODEL_ROUTE_METRIC = "pilotage.model_route.count"
TASK_STARTED_METRIC = "pilotage.task_run.started"
TASK_FINISHED_METRIC = "pilotage.task_run.finished"
TOOL_CALL_METRIC = "pilotage.tool_call.count"
TOOL_APPROVAL_METRIC = "pilotage.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "pilotage.skill.lifecycle.count"
SKILL_LOAD_METRIC = "pilotage.skill.load.count"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(pilotage_version or "").strip()
    ...
    return {
        ...
        "pilotage_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("pilotage_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Pilotage logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("pilotage.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from pilotage_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Pilotage terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Pilotage registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Pilotage tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Pilotage."""
