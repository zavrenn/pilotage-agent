"""Durable aggregation and local export for Pilotage shared metrics."""
...
from pilotage_cli.sqlite_util import write_txn
...
_PACKAGE_SCHEMA_VERSION = "pilotage.shared_metrics.v2"
...
class SharedMetricsStore:
    ...
    def __init__(
        ...
        root = get_pilotage_home() / "telemetry" / "shared_metrics"
        ...
        return [
            {
                ...
                "resource": {
                    "pilotage_version": row["pilotage_version"],
    ...
    def _ensure_schema(self) -> None:
        with self._connection(busy_timeout_ms=_SCHEMA_BUSY_TIMEOUT_MS) as connection:
            # Serialize first-run creation and upgrades across Pilotage processes.
            with write_txn(connection):
    ...
    def _create_counter_aggregates_table(connection: sqlite3.Connection) -> None:
        connection.execute(
            ...
            CREATE TABLE IF NOT EXISTS counter_aggregates (
                ...
                pilotage_version TEXT NOT NULL,
    ...
    def _create_package_in_transaction(
        ...
        resource = {
            "pilotage_version": period_row["pilotage_version"],
