"""Profile distributions — shareable, packaged Pilotage profiles via git.

A distribution is a Pilotage profile published as a git repository (or
...
* ``pilotage profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``pilotage skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``pilotage profile``, not a parallel tree):

    pilotage profile install <source> [--name N] [--alias] [--force] [--yes]
    pilotage profile update  <name>  [--force-config] [--yes]
    pilotage profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "pilotage_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".pilotage_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "pilotage-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            pilotage_requires=str(data.get("pilotage_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.pilotage_requires:
            out["pilotage_requires"] = self.pilotage_requires
        if self.author:
...
def check_pilotage_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Pilotage {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Pilotage distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``pilotage profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Pilotage profile distribution."
            )
...
def plan_install(
    ...
    from pilotage_cli import __version__ as pilotage_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Pilotage distribution."
        )

    # Version check up-front so we fail fast
    check_pilotage_requires(manifest.pilotage_requires, pilotage_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.pilotage).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="pilotage_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `pilotage profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`pilotage profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="pilotage_dist_update_") as tmp:
        plan = plan_install(
