...
This module is imported by ``pilotage_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no pilotage_cli.config,
...
``pilotage_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(pilotage_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect PILOTAGE_HOME?"""
    active_profile = os.path.join(pilotage_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    ...
    if pilotage_home:
        return pilotage_home
    return os.path.join(os.path.expanduser("~"), ".pilotage")
...
def container_mode_may_be_active() -> bool:
    ...
    pilotage_home = os.environ.get("PILOTAGE_HOME", "").strip()
    if pilotage_home:
        if os.path.exists(os.path.join(pilotage_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(pilotage_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(pilotage_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".pilotage")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect PILOTAGE_HOME.
    """
...
def print_fast_version_info() -> None:
    from pilotage_cli import __release_date__, __version__

    print(f"Pilotage Agent v{__version__} ({__release_date__})")
    ...
    print("Run 'pilotage version' for update status.")


def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``pilotage --version`` before the heavy import wall.

    Termux keeps its historical contract (also accepts the ``version``
    subcommand + the PILOTAGE_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
    ...
    if is_termux and os.environ.get("PILOTAGE_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
