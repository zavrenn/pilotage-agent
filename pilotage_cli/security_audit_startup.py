...
3. Running inside a container with no persistent volume mount over the
   PILOTAGE_HOME data dir (state is ephemeral — lost on container restart).
...
logger = logging.getLogger("pilotage.security_audit")
...
def _running_as_root() -> Optional[str]:
    ...
    return (
        ...
        "full host compromise. Run Pilotage as an unprivileged user (or in a "
...
def _container_no_volume_mount(pilotage_home: Optional[Path]) -> Optional[str]:
...
def run_security_audit(
    *, pilotage_home: Optional[Path] = None, config: Optional[dict] = None
) -> list[str]:
    ...
    for check in (
        ...
        try:
            ...
            continue
    try:
        r = _container_no_volume_mount(pilotage_home)
        if r:
            findings.append(r)
    except Exception:
        pass
...
def log_startup_security_warnings(
    *,
    ...
    _AUDIT_RAN = True
    try:
        findings = run_security_audit(pilotage_home=pilotage_home, config=config)
    except Exception:
        return []
    if findings:
        logger.warning(
