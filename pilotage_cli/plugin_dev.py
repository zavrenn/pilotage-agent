...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary PILOTAGE_HOME
    with outbound socket connects blocked.
    """
    temporary_home = tempfile.TemporaryDirectory(prefix="pilotage-plugin-doctor-")
    stack = ExitStack()
    ...
    stack.enter_context(
        patch.dict(
            os.environ,
            {
                ...
                "PILOTAGE_BUNDLED_PLUGINS": str(bundled),
                "PILOTAGE_ENABLE_PROJECT_PLUGINS": "0",
            },
            clear=False,
        )
    )
    ...
    from pilotage_cli.plugins import PluginManager
    ...
    modules_before = {
        ...
        if name == "pilotage_plugins" or name.startswith("pilotage_plugins.")
    }
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"Pilotage discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
        ...
        for name in list(sys.modules):
            if (
                ...
                and (name == "pilotage_plugins" or name.startswith("pilotage_plugins."))
            ):
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    user_root = get_pilotage_home() / "plugins"
    ...
    try:
        ...
        pass
    candidates.append(Path.cwd() / ".pilotage" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from pilotage_cli.plugins import SUPPORTED_MANIFEST_VERSION

    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Pilotage supports "
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — Pilotage never auto-installs plugin dependencies; "
    ...
    if schema:
        from pilotage_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Pilotage' real scanner and registration path."""
    try:
