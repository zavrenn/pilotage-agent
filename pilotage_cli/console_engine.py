"""Safe Pilotage Console command engine.

This module backs ``pilotage console`` and is intentionally narrower than the
full Pilotage CLI. It exposes a curated set of native adapters that can later be
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["PilotageConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'pilotage doctor'")
        and last.startswith("Run 'pilotage setup'")
    ):
        return text.rstrip()
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="pilotage", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `pilotage "):
        return ""
    return summary
...
# dashboard opens a fresh PilotageConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("pilotage_cli.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace], None] | None = None,
) -> Callable[["PilotageConsoleEngine", list[str]], str]:
    def handler(_engine: PilotageConsoleEngine, args: list[str]) -> str:
...
def _register_command_family(
    engine: "PilotageConsoleEngine",
    *,
    root: str,
    paths: Iterable[Sequence[str]],
    handler_factory: Callable[[Sequence[str]], Callable[["PilotageConsoleEngine", list[str]], str]],
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `pilotage {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `pilotage {usage}`?",
        )


class PilotageConsoleEngine:
    """Curated line-command executor for Pilotage Console."""
    ...
    def execute(self, line: str, *, confirmed: bool = False) -> ConsoleResult:
        ...
        try:
            ...
            if tokens and tokens[0] == "pilotage":
                tokens = tokens[1:]
            if not tokens:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Pilotage Console does not run shell syntax. Use one supported "
                    "Pilotage command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Pilotage component status.", _status)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Pilotage logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Pilotage configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Pilotage Console."""

        extracted = {
            "version": (
                "pilotage_cli.subcommands.version",
            ...
            "debug": (
                "pilotage_cli.subcommands.debug",
                "build_debug_parser",
                "cmd_debug",
                [("share",), ("delete",)],
                {("share",), ("delete",)},
            ...
            "prompt-size": (
                "pilotage_cli.subcommands.prompt_size",
            ...
            "insights": (
                "pilotage_cli.subcommands.insights",
            ...
            "security": (
                "pilotage_cli.subcommands.security",
            ...
            "backup": (
                "pilotage_cli.subcommands.backup",
                "build_backup_parser",
                "cmd_backup",
                [()],
                {()},
            ...
            "import": (
                "pilotage_cli.subcommands.import_cmd",
            ...
            "config": (
                "pilotage_cli.subcommands.config",
                "build_config_parser",
                "cmd_config",
                [("env-path",), ("check",)],
                set(),
            ...
            "tools": (
                "pilotage_cli.subcommands.tools",
                "build_tools_parser",
                "cmd_tools",
            ...
            "plugins": (
                "pilotage_cli.subcommands.plugins",
            ...
            "skills": (
                "pilotage_cli.subcommands.skills",
                "build_skills_parser",
                "cmd_skills",
                [
                    ("browse",),
            ...
            "memory": (
                "pilotage_cli.subcommands.memory",
                "build_memory_parser",
                "cmd_memory",
                [("status",), ("off",), ("reset",)],
                {("off",), ("reset",)},
            ...
            "pairing": (
                "pilotage_cli.subcommands.pairing",
            ...
            "webhook": (
                "pilotage_cli.subcommands.webhook",
            ...
            "hooks": (
                "pilotage_cli.subcommands.hooks",
                "build_hooks_parser",
                "cmd_hooks",
            ...
            "profile": (
                "pilotage_cli.subcommands.profile",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Pilotage configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "pilotage_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("pilotage_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("pilotage_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported Pilotage Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Pilotage Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`pilotage {first}` is not available in Pilotage Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in Pilotage Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Pilotage Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in Pilotage Console.",
            ("skills", "publish"): "`skills publish` is not available in Pilotage Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in Pilotage Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Pilotage Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in Pilotage Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in Pilotage Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Pilotage Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Pilotage Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Pilotage Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Pilotage Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Pilotage Console.",
            ("kanban", "gc"): "`kanban gc` is not available in Pilotage Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Pilotage Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Pilotage Console.")
...
def _status(_engine: PilotageConsoleEngine, args: list[str]) -> str:
...
def _doctor(_engine: PilotageConsoleEngine, args: list[str]) -> str:
...
def _logs(_engine: PilotageConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Pilotage Console.")
    ...
    from pilotage_cli.logs import list_logs, tail_log
...
def _sessions_list(_engine: PilotageConsoleEngine, args: list[str]) -> str:
...
def _sessions_stats(_engine: PilotageConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: PilotageConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config show")
    from pilotage_cli.config import show_config

    return _capture_output(show_config)


def _config_path(_engine: PilotageConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config path")
    from pilotage_cli.config import get_config_path

    return str(get_config_path())


def _config_set(_engine: PilotageConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
        raise ConsoleCommandError("Usage: config set <key> <value>")
    key = args[0]
    value = " ".join(args[1:])
    from pilotage_cli.config import set_config_value

    return _capture_output(lambda: set_config_value(key, value))


def _config_migrate(_engine: PilotageConsoleEngine, args: list[str]) -> str:
...
def _sessions_export(_engine: PilotageConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: PilotageConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: PilotageConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: PilotageConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from pilotage_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: PilotageConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch_extracted_subcommand(
        root="profile",
        fixed=(),
        args=(),
        module_name="pilotage_cli.subcommands.profile",
        builder_name="build_profile_parser",
        main_handler_name="cmd_profile",
    )


def _cron_list(_engine: PilotageConsoleEngine, args: list[str]) -> str:
    parser = _ArgumentParser(prog="cron list", add_help=False)
    parser.add_argument("--all", action="store_true")
    ns = parser.parse_args(args)
    from pilotage_cli.cron import cron_list

    return _capture_output(lambda: cron_list(show_all=ns.all))


def _cron_status(_engine: PilotageConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from pilotage_cli.cron import cron_status

    return _capture_output(cron_status)


def _cron_pause(_engine: PilotageConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron pause <job>")
    from cron.jobs import AmbiguousJobReference, pause_job

    try:
        job = pause_job(args[0], reason="paused from pilotage console")
...
def _cron_resume(_engine: PilotageConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def _cron_run(_engine: PilotageConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def run_console_repl(
    *,
    ...
    """Run the local ``pilotage console`` REPL."""
    ...
    engine = PilotageConsoleEngine()
    if interactive:
        print("Pilotage Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("pilotage> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
