...
def _model_flow_openrouter(config, current_model=""):
    ...
    # in-flow (K/R/C) instead of having to edit ~/.pilotage/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `pilotage setup` from scratch. OpenRouter
    ...
    from pilotage_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from pilotage_constants import AI_GATEWAY_BASE_URL
    from pilotage_cli.main import _prompt_api_key
    from pilotage_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.pilotage/.env by hand.
    ...
    from pilotage_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    if not presets:
        print("No MoA presets configured. Run `pilotage moa configure <name>` first.")
        return
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from pilotage_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from pilotage_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    # live only in the pool (e.g. after ``pilotage auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from pilotage_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from pilotage_cli.auth import (
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from pilotage_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from pilotage_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from pilotage_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Pilotage will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Pilotage will install it now (the preflight below "
...
def _model_flow_copilot(config, current_model=""):
    ...
    from pilotage_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from pilotage_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from pilotage_cli.config import save_env_value, load_config, save_config
    from pilotage_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from pilotage_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from pilotage_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Pilotage turns to `copilot --acp`.")
    print("  Pilotage currently starts its own ACP subprocess for each request.")
    print("  Pilotage uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set PILOTAGE_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    # Check env var and credential pool (keys added via `pilotage auth`)
    from pilotage_cli.auth import _resolve_api_key_provider_secret, ProviderConfig
    ...
    if selected:
        ...
        # Save the API key env var name so pilotage knows where to find it
...
def _model_flow_vertex(config, current_model=""):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    from pilotage_cli.config import load_config, save_config, get_env_value
    from pilotage_cli.models import _PROVIDER_MODELS
    ...
    if sa_path:
        ...
        print("      • set VERTEX_CREDENTIALS_PATH in ~/.pilotage/.env to a service account JSON")
    print()
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Pilotage typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Pilotage, enable billing on your "
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from pilotage_cli.main import _run_anthropic_oauth_flow
    from pilotage_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
    ...
    if selected:
        ...
        # providers if the user switches without running 'pilotage model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
