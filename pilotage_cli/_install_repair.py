...
the corrupted-venv state it exists to repair. ``pilotage_cli.main`` imports
``managed_uv``, ``pilotage_constants``, and friends only in its late path; the
...
:func:`pilotage_cli._early_recovery._find_uv_binary` lookup and falls back to
...
from pilotage_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (pilotage_cli.main's version lives behind imports)."""
...
def _stdout_to_stderr():
    ...
    ``pilotage acp`` speaks JSON-RPC on stdout; an inherited-fd install child
...
def _quarantine_running_pilotage_exe(scripts_dir: Path) -> list[tuple[Path, Path]]:
    """Rename live pilotage*.exe shims aside so the installer can rewrite them.
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    moved = _quarantine_running_pilotage_exe(scripts_dir) if scripts_dir else []
    try:
...
def run_core_install(root: Path) -> None:
    ...
    - quarantine live ``pilotage*.exe`` shims on Windows so they can be replaced
