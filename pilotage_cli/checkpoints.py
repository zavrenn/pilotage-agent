"""`pilotage checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.pilotage/checkpoints/``.  Actions:

    pilotage checkpoints               # same as `status`
    pilotage checkpoints status        # total size, project count, breakdown
    pilotage checkpoints list          # per-project checkpoint counts + workdir
    pilotage checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    pilotage checkpoints clear [-f]    # nuke the entire base (asks first)
    pilotage checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    pilotage checkpoints
    pilotage checkpoints prune --retention-days 3 --max-size-mb 200
    pilotage checkpoints clear -f
...
from pilotage_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: pilotage checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``pilotage checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `pilotage checkpoints` → status
