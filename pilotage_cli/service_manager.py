...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``pilotage gateway start/stop/restart``).
    """
    ...
    # `pilotage gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged pilotage
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in pilotage_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from pilotage_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from pilotage_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from pilotage_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from pilotage_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in pilotage_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from pilotage_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from pilotage_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from pilotage_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from pilotage_cli.gateway import _probe_launchd_service_running
...
#
# Per-profile gateways are registered dynamically when `pilotage profile create`
# runs inside the container (Phase 4). Static services (main-pilotage, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's PILOTAGE_HOME blindly;
    derive the shared profile root from the current PILOTAGE_HOME and map the
    ...
    pilotage_home = Path(os.environ.get("PILOTAGE_HOME", "/opt/data"))
    if pilotage_home.parent.name == "profiles":
        root = pilotage_home.parent.parent
    else:
        root = pilotage_home
...
# UID/GID of the in-image ``pilotage`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the pilotage user.
    ...
    (the ``pilotage`` user — UID 10000 — running every Pilotage runtime
    ...
    with pilotage ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           pilotage:pilotage, 0755 (parent must already exist)
    ``svc_dir/event/``                     pilotage:pilotage, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 pilotage:pilotage, 0755
    ``svc_dir/supervise/event/``           pilotage:pilotage, 03730
    ``svc_dir/supervise/control``          pilotage:pilotage, 0660    (FIFO)
    ...
    slot's supervise/ was pilotage-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _PILOTAGE_UID, _PILOTAGE_GID)
        except PermissionError:
            # Running as the pilotage user already — directory is pilotage-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _PILOTAGE_UID, _PILOTAGE_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is pilotage-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _PILOTAGE_UID, _PILOTAGE_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``pilotage -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`pilotage profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-pilotage, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources PILOTAGE_HOME (and any extra env) via with-contenv —
             so e.g. ``-e PILOTAGE_HOME=/data/pilotage`` is honored at run
          ...
          4. Drops to the pilotage user and exec's
             ``pilotage -p <profile> gateway run`` (or just ``pilotage
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``pilotage gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root PILOTAGE_HOME profile" (the implicit profile that exists at
        the top of $PILOTAGE_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``pilotage_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$PILOTAGE_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``PILOTAGE_HOME``,
 ...
 review item I5 retired both the allocator and the
        ...
        lines = [
            ...
            "cd /opt/data",
            ". /opt/pilotage/.venv/bin/activate",
        ]
        ...
        # guard.
        lines.append("export PILOTAGE_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's PILOTAGE_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``pilotage gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-PILOTAGE_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The PILOTAGE_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own PILOTAGE_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "pilotage gateway run --replace"
        else:
            gateway_cmd = f"pilotage -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid pilotage {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${PILOTAGE_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the PILOTAGE_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e PILOTAGE_HOME=/data/pilotage``
        gets its logs under /data/pilotage/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``pilotage logs`` reads and what
        ...
        return (
            ...
            f': "${{PILOTAGE_HOME:=/opt/data}}"\n'
            f'log_dir="$PILOTAGE_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock as pilotage when
            # this script starts as root. Never chown or unlink pilotage-writable
            ...
            f'exec s6-setuidgid pilotage s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``pilotage -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with pilotage ownership
            ...
            # the pilotage user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is pilotage-owned (see ``_seed_supervise_skeleton``), and on
