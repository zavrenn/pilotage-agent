"""``pilotage skin`` — list, switch, and tweak skins from the CLI.
...
from pilotage_constants import display_pilotage_home, get_pilotage_home
...
def _skin_set(key: str, value: str, skin: str | None) -> int:
    ...
    if path.exists():
        ...
        from pilotage_cli.skin_engine import load_skin
    ...
    print(f"✓ {key} = {value} in {display_pilotage_home()}/skins/{target}.yaml (live within ~1s)")
    return 0
