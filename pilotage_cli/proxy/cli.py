"""CLI handlers for the ``pilotage proxy`` subcommand."""
...
from pilotage_cli.proxy.adapters import ADAPTERS, get_adapter
from pilotage_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "pilotage proxy requires aiohttp. Run `pilotage setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"pilotage auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Pilotage proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Pilotage proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: pilotage proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``pilotage proxy <subcommand>``."""
    ...
    print(
        "pilotage proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  pilotage proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  pilotage proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  pilotage proxy providers\n"
