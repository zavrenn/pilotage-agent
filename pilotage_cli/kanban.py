"""CLI for the Pilotage Kanban board — ``pilotage kanban …`` subcommand.

Exposes the full Kanban command surface documented in the design spec
(``docs/pilotage-kanban-v1-spec.pdf``).  All DB work is delegated to
...
from pilotage_cli import kanban_swarm as ks
...
def _check_dispatcher_presence(
    pilotage_home: Optional[Path] = None,
) -> tuple[bool, str]:
    """Return ``(running, message)``.

    - ``running=True``: a gateway is alive for this PILOTAGE_HOME and its
    ...
    Used by ``pilotage kanban create`` (and callers) to warn when a task
    ...
    ``pilotage_home`` scopes the probe to a named profile's directory. The
    ...
    try:
        ...
        liveness = resolve_gateway_liveness(
            profile_dir=pilotage_home, use_cache=False
        )
    except Exception:
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`pilotage kanban daemon --force`)."
        )
    return (
        False,
        "No gateway is running — the task will sit in 'ready' until you "
        "start it. Run:\n"
        "    pilotage gateway start\n"
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    # PILOTAGE_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `pilotage kanban boards switch <slug>` or the "
            "PILOTAGE_KANBAN_BOARD env var). Use `pilotage kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_create.add_argument("--project", default=None,
                          help="Link to a project (id or slug). Anchors the task's "
                               ...
                               "deterministic branch. See `pilotage project list`.")
    ...
    p_list.add_argument("--mine", action="store_true",
                        help="Filter by $PILOTAGE_PROFILE as assignee")
    ...
    p_comment.add_argument("--author", default=None,
                           help="Author name (default: $PILOTAGE_PROFILE or 'user')")
    ...
    p_attach.add_argument("--author", default=None,
                          help="uploaded_by label (default: $PILOTAGE_PROFILE or 'user')")
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `pilotage gateway start`.",
    )
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.pilotage/profiles/ and current assignees on the board)",
    )
    ...
    p_specify.add_argument(
        ...
        help="Author name recorded on the audit comment "
             "(default: $PILOTAGE_PROFILE or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
    ...
    p_decompose.add_argument(
        ...
        default=None,
        help="Author name recorded on the audit comment "
             "(default: $PILOTAGE_PROFILE or 'decomposer')",
    )
    p_decompose.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``pilotage kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: pilotage kanban <action> [options]\n"
                "Run 'pilotage kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0

    # Fast-fail for clearer CLI UX only. The durable trust boundary is lower in
    # pilotage_cli.kanban_db, because children can import DB mutators directly.
    ...
    # env-var pin for the duration of this call. Using PILOTAGE_KANBAN_BOARD
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `pilotage kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    # PILOTAGE_HOME. Previously only `init` and `daemon` triggered
    ...
    with board_scope:
        ...
        # every `pilotage kanban repair` into "could not initialize database"
...
def _profile_author() -> str:
    ...
    for env in ("PILOTAGE_PROFILE_NAME", "PILOTAGE_PROFILE"):
...
def _is_delegated_child_cli_mutation(args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        return bool(os.environ.get("PILOTAGE_DELEGATED_CHILD_CONTEXT"))


# ---------------------------------------------------------------------------
# Boards management (pilotage kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``pilotage kanban boards <action>``.
    ...
    DB, so a fresh PILOTAGE_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `pilotage kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `pilotage kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `pilotage kanban boards switch {meta['slug']}` to make it current.")
    return 0
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.pilotage/profiles/.")
        print("Create one with `pilotage -p <name> setup` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print("  pilotage gateway start")
    print()
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `pilotage -p <name> setup`)")
        return 0
    # Header
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    # comments / runs.
    from pilotage_cli import kanban_diagnostics as kd
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``pilotage kanban show <id>``
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            "pilotage kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    pilotage gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`pilotage kanban list --status ready` / "
                    f"`pilotage kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real Pilotage profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from pilotage_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from pilotage_cli import kanban_decompose as decomp
