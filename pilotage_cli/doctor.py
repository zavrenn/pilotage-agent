"""
Doctor command for pilotage CLI.

Diagnoses issues with Pilotage Agent setup.
"""
...
_DHH = display_pilotage_home()  # user-facing display path (e.g. ~/.pilotage or ~/.pilotage/profiles/coder)

# Load environment variables from ~/.pilotage/.env so API key checks work
_env_path = get_env_path()
load_pilotage_dotenv(pilotage_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")
...
def _pilotage_database_paths(pilotage_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for Pilotage-managed SQLite databases."""
    # backup.py owns the canonical list of per-profile stores; reuse it.
    from pilotage_cli.backup import _QUICK_STATE_FILES

    entries = [
        (name, pilotage_home / name)
...
def _format_db_size(db_path: Path) -> str:
    ...
    from pilotage_cli.backup import _format_size
...
def _report_database_journal_modes(
    pilotage_home: Path | None = None,
    version_info: tuple[int, ...] | None = None,
) -> None:
    """List each database's journal mode; warn on WAL under a vulnerable SQLite."""
    from pilotage_state import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable

    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    home = pilotage_home if pilotage_home is not None else PILOTAGE_HOME
    try:
        databases = _pilotage_database_paths(home)
    except Exception as exc:
        check_warn(f"Could not list Pilotage databases: {exc}")
        return
    exposed = []
...
def _termux_install_all_fallback_notes() -> list[str]:
    return [
        ...
        "Some optional extras are excluded on Termux (native builds fail there).",
        "STT: OpenAI Whisper via VOICE_TOOLS_OPENAI_KEY, or a plugin-registered provider.",
    ]


def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.pilotage/.env contains provider auth/base URL settings."""
...
from pilotage_cli.sizefmt import format_bytes as _human_bytes
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or legacy_trigram:
            detail += (
                "; run 'pilotage sessions optimize-storage' offline "
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. PILOTAGE_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("PILOTAGE_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("PILOTAGE_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches pilotage_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``pilotage --version`` reporting a stale version while
    ...
    """
    try:
        from pilotage_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != pilotage_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'pilotage update', or set "
            "pilotage_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-pilotage and dashboard static services are up
    ...
    for static in ("main-pilotage", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `pilotage profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `pilotage doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from pilotage_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the PILOTAGE_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("PILOTAGE_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via PILOTAGE_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `pilotage`.
    os.environ.setdefault("PILOTAGE_INTERACTIVE", "1")

    # Handle `pilotage doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.pilotage/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Pilotage Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`pilotage doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        from pilotage_cli.config import load_config
        from pilotage_cli.mcp_security import validate_mcp_server_entry
    ...
    try:
        import sqlite3
        from pilotage_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Pilotage already refuses to enable WAL on fresh DBs.
    ...
    # Detect drift between pyproject.toml and pilotage_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.pilotage/.env (primary location for user config)
    env_path = PILOTAGE_HOME / '.env'
    if env_path.exists():
        ...
        # matches pilotage_cli.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'pilotage setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'pilotage setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'pilotage setup' to create one")
                issues.append("Run 'pilotage setup' to create .env")
    
    # Check ~/.pilotage/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = PILOTAGE_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from pilotage_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'pilotage config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
                ...
                try:
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.pilotage/.env or run 'pilotage setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'pilotage setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'pilotage config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from pilotage_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from pilotage_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'pilotage setup' to migrate config")
                else:
                    issues.append("Run 'pilotage doctor --fix' or 'pilotage setup' to migrate config")
            else:
                ...
                if should_fix:
                    ...
                    for k in stale_root_keys:
                        if not model_section.get(k):
                            ...
                            raw_config.pop(k)
                    from pilotage_cli.config import atomic_config_write
                    atomic_config_write(config_path, raw_config)
                    check_ok("Migrated stale root-level keys into model section")
                    fixed_count += 1
                else:
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'pilotage doctor --fix'")
        except Exception:
            pass
        ...
        try:
            from pilotage_cli.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("PILOTAGE_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"PILOTAGE_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `pilotage setup` run)",
                )
                if should_fix:
                    if remove_env_value("PILOTAGE_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale PILOTAGE_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove PILOTAGE_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the PILOTAGE_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale PILOTAGE_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'pilotage doctor --fix'"
                    )
        except Exception:
            pass
        ...
        try:
            from pilotage_cli.config import load_env as _load_env_depr
            from pilotage_cli.config import read_user_config_raw as _read_raw_depr
    ...
    if pilotage_home.exists():
        ...
        pilotage_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = pilotage_home / subdir_name
    ...
    soul_path = pilotage_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Pilotage a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Pilotage Agent Persona\n\n"
                "<!-- Edit this file to customize how Pilotage communicates. -->\n\n"
                "You are Pilotage, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = pilotage_home / "memories"
    ...
    state_db_path = pilotage_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from pilotage_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'pilotage doctor --fix' "
                        "(or 'pilotage sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from pilotage_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'pilotage doctor --fix' "
                        "(or 'pilotage sessions repair') to recover hidden sessions"
                    )
            else:
                check_warn(f"{_DHH}/state.db exists but has issues: {e}")

        # Health/stats snapshot (#statedb-visibility): a multi-GB state.db
        # with a runaway WAL was previously invisible to every Pilotage
        ...
        try:
            from pilotage_state import collect_state_db_stats, count_db_holders
                ...
                if _kind == "warn":
                    ...
                    if "auto_prune" in _detail:
                        issues.append(
                            ...
                            "in config.yaml"
                            + (
                                " and run 'pilotage sessions optimize-storage' "
    ...
    wal_path = pilotage_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'pilotage doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "pilotage"
        ...
        _cmd_link = _cmd_link_dir / "pilotage"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(pilotage not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/pilotage → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/pilotage points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/pilotage → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/pilotage — run 'pilotage doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/pilotage exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/pilotage not found",
                    "(pilotage command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/pilotage → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/pilotage symlink — run 'pilotage doctor --fix'")
    ...
    try:
        from pilotage_constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "(pip install 'pilotage-agent[vercel]')",
                "Install the Vercel optional dependency: pip install 'pilotage-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        if _resolved_ab and _is_npx_agent_browser_sentinel(_resolved_ab):
            ...
            if should_fix:
                ...
                # `pilotage update` does, so a session's first browser call
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `pilotage doctor` invocation.
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Pilotage one.
                        check_info(
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'pilotage setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = PILOTAGE_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: pilotage skills list)")
    ...
    try:
        from pilotage_cli.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = PILOTAGE_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: pilotage memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: pilotage memory setup",
                    "No Honcho API key — run 'pilotage memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run pilotage memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: pilotage memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: pilotage memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from pilotage_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "pilotage -p" in content:
                            _m = _re.search(r"pilotage -p (\S+)", content)
    ...
    # `pilotage doctor --live` (real network calls; bounded + read-only).
    try:
        from pilotage_cli.doctor_live import maybe_run_live_checks
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'pilotage doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
