"""``pilotage portal`` — the human-readable entry point for Nous Portal.

Running ``pilotage portal`` with no subcommand performs the one-shot Portal
...
``pilotage auth add nous --type oauth`` (which still works), is identical to
``pilotage setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``pilotage auth`` or ``pilotage tools``. It's the onboarding + discovery
...
def _cmd_status(args) -> int:
    ...
    from pilotage_cli.auth import get_nous_auth_status_local
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `pilotage model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `pilotage portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `pilotage auth add nous --type
    oauth`. It reuses the exact wiring behind `pilotage setup --portal` (which in
    ...
    """
    from pilotage_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `pilotage portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `pilotage portal` is the
        # human-readable alias for `pilotage auth add nous --type oauth` /
        # `pilotage setup --portal`.
    ...
    print("Run `pilotage portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `pilotage portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `pilotage portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `pilotage auth add "
            "nous --type oauth`, identical to `pilotage setup --portal`). "
