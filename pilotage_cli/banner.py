...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    if _is_official_ssh_remote(origin_url):
        ...
        # ahead checkout as behind nudges the user into `pilotage update`,
...
def check_for_updates() -> Optional[int]:
    """Check whether a Pilotage update is available.

    Two paths: if ``PILOTAGE_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = pilotage_home / ".update_check"
    embedded_rev = os.environ.get("PILOTAGE_REVISION") or None
    ...
    if embedded_rev:
        ...
        # $PILOTAGE_HOME/pilotage-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = pilotage_home / "pilotage-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Pilotage git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$PILOTAGE_HOME/pilotage-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``pilotage_cli/build_info.py``) and return it as a frozen
...
def format_banner_version_label() -> str:
    ...
    base = f"Pilotage Agent v{VERSION} ({RELEASE_DATE})"
...
def _banner_snapshot_path() -> Path:
    return get_pilotage_home() / "cache" / "banner_snapshot.json"
...
def banner_snapshot_fingerprint() -> Optional[str]:
    ...
    try:
        ...
        for p in (get_config_path(), get_pilotage_home() / ".env"):
            try:
                st = p.stat()
...
def compute_toolset_availability(enabled_toolsets: List[str] = None) -> Dict[str, Any]:
    ...
    # toolsets that aren't part of this agent's platform set at all. Those
    # must never surface in the
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else PILOTAGE_CADUCEUS
    except Exception:
        _bskin = None
        _hero = PILOTAGE_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or pilotage setup[/]"
            )
        else:
    ...
    try:
        from pilotage_cli.codex_runtime_switch import get_current_runtime
        from pilotage_cli.config import load_config as _load_cfg
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else PILOTAGE_AGENT_LOGO
        console.print(_logo)
        console.print()
