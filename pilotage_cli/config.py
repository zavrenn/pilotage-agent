"""
Configuration management for Pilotage Agent.

Config files are stored in ~/.pilotage/ for easy access:
- ~/.pilotage/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.pilotage/.env         - API keys and secrets

This module provides:
- pilotage config          - Show current configuration
- pilotage config edit     - Open config in editor
- pilotage config get      - Print a resolved configuration value
- pilotage config set      - Set a specific value
- pilotage config unset    - Remove a user configuration value
- pilotage config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``pilotage config set`` (which rewrites
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.pilotage/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``pilotage logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  pilotage config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Pilotage makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Pilotage itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``pilotage update``, the TUI build.
...
#   on every plugin install / ``pilotage update``.
...
# * ``PILOTAGE_HOME`` / ``PILOTAGE_PROFILE`` / ``PILOTAGE_CONFIG`` /
#   ``PILOTAGE_ENV`` — Pilotage runtime location flags. Writing these into
...
#
# IMPORTANT: ``PILOTAGE_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (PILOTAGE_MEM0_API_KEY,
# PILOTAGE_HONCHO_API_KEY, ...). The
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Pilotage runtime location "
            "(PILOTAGE_HOME, PILOTAGE_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.pilotage/.env directly."
        )
...
from pilotage_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$PILOTAGE_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/pilotage`` in the published image, which instead bakes the stamp at
...
# Re-export from pilotage_constants — canonical definition lives there.
from pilotage_constants import get_pilotage_home, get_process_pilotage_home  # noqa: F811,E402
...
def _resolve_pilotage_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the PILOTAGE_UID / PILOTAGE_GID env vars set by Docker deployments.

    Docker containers running Pilotage commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level PILOTAGE_HOME once, but
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("PILOTAGE_UID", "").strip()
    gid_str = os.environ.get("PILOTAGE_GID", "").strip()
    try:
...
def _chown_to_pilotage_uid(path) -> None:
    """Chown ``path`` to ``PILOTAGE_UID:PILOTAGE_GID`` if those env vars are set.
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into PILOTAGE_HOME, upgrading legacy empty templates.
...
# ensure_pilotage_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_PILOTAGE_HOME_ENSURED: set = set()
...
def ensure_pilotage_home():
    ...
    After the first successful pass for a given ``PILOTAGE_HOME`` we only re-run
    ...
    ``get_pilotage_home()`` and therefore re-run for the new path.
    """
    home = get_pilotage_home()
    key = str(home)

    if key in _PILOTAGE_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``pilotage profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old PILOTAGE_HOME would resurrect an empty skeleton
...
from pilotage_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `pilotage config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.pilotage.config`` entries, then checks
    ...
    try:
        ...
        # should never break `pilotage update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Pilotage' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Pilotage versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``pilotage_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (pilotage_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (pilotage_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (pilotage_cli/setup.py)
    "known_plugin_toolsets", # written/read by pilotage_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'pilotage setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Pilotage won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, pilotage send) so
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        ...
        hint_path = display_pilotage_home()
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "pilotage update / pilotage -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from pilotage_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ pilotage config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in pilotage_cli.config_migrations as a
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: pilotage config set <key> <value>")
    ...
    # `pilotage update`, but only the version bump is persisted.
    ...
    # metadata.pilotage.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.pilotage/.env); "
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``PilotageCLI`` construction: the model becomes a plain
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    backends) — while display paths (``pilotage status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _pilotage_home / "config.yaml"`` local variable
...
# Back-compat alias — canonical set lives in pilotage_cli.personality.
from pilotage_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`pilotage_cli.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from pilotage_cli.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from pilotage_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``PILOTAGE_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`pilotage_cli.personality` (single owner).
    """
    from pilotage_cli.personality import resolve_ephemeral_system_prompt

    return resolve_ephemeral_system_prompt(cfg)


def read_raw_config() -> Dict[str, Any]:
    """Read ~/.pilotage/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_pilotage_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.pilotage/config.yaml.
    ...
    (which change ``PILOTAGE_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/pilotage/config.yaml invalidates the
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.pilotage/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Pilotage writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.pilotage/.env.
    ...
    interactive menu render (`pilotage tools`, `pilotage setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `pilotage tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.pilotage/.env in place.
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``PILOTAGE_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"PILOTAGE_CUSTOM_{slug}_API_KEY" if slug else "PILOTAGE_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.pilotage/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.pilotage/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Pilotage — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Pilotage vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.pilotage/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.pilotage/.env`` over ``os.environ``.

    Used for Pilotage-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Pilotage Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        from pilotage_cli.personality import active_personality_name
    ...
    whatsapp_enabled = get_env_value('WHATSAPP_ENABLED')
    
    print(f"  Telegram:     {'configured' if telegram_token else color('not configured', Colors.DIM)}")
    print(f"  WhatsApp:     {'configured' if whatsapp_enabled else color('not configured', Colors.DIM)}")
    ...
    print(color("  pilotage config edit     # Edit config file", Colors.DIM))
    print(color("  pilotage config set <key> <value>", Colors.DIM))
    print(color("  pilotage setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``PILOTAGE_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("PILOTAGE_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `pilotage cron list`, then pin the "
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``pilotage config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``pilotage config set``
...
def _validate_config_key(key: str) -> tuple[bool, Optional[str]]:
    ...
    ``telegram.gateway_restart_notification`` (platform configs live at the
    ...
    if top in _OPEN_DICT_TOP_LEVEL_KEYS or top in _DYNAMIC_TOP_LEVEL_KEYS or top in _SCHEMA_DEFINED_DICT_KEYS:
        ...
        # shape themselves (mcp_servers.<name>.command, telegram.<extras>,
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (pilotage config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    # ``pilotage config set model.provider openai`` doesn't silently
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    pilotage config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    pilotage config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: pilotage config get <key> [--json]")
            print()
            print("Examples:")
            print("  pilotage config get model")
            print("  pilotage config get terminal.backend")
            print("  pilotage config get skills.config --json")
            sys.exit(1)
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: pilotage config unset <key>")
            print()
            print("Examples:")
        ...
        if missing_config:
            ...
            print("    Run 'pilotage config migrate' to add them")
        ...
        print("  pilotage config           Show current configuration")
        print("  pilotage config edit      Open config in editor")
        print("  pilotage config get <key>          Print a resolved config value")
        print("  pilotage config set <key> <value>   Set a config value")
        print("  pilotage config unset <key>        Remove a config value")
        print("  pilotage config check     Check for missing/outdated config")
        print("  pilotage config migrate   Update config with new options")
        print("  pilotage config path      Show config file path")
        print("  pilotage config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``pilotage config`` UI so users
# can configure a bundled platform without the core repo ever needing to
# know it exists.
#
...
#       description: "Example bot client secret"
#       url: "https://example.com/"
#       password: true
#       prompt: "Example client secret"
#
