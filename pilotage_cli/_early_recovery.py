"""Dependency-light venv recovery that runs BEFORE pilotage_cli.main's imports.

The ``pilotage`` console entry point is ``pilotage_cli.main:main``.  Importing
``pilotage_cli.main`` pulls in third-party packages at module level (``dotenv``
...
a corrupted venv.  ``pilotage_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``pilotage_cli.main`` to become
...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which Pilotage already vendors
    (``~/.pilotage/bin/uv.exe``) or the user has on PATH.  Stdlib-only.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    candidates = [
        Path.home() / ".pilotage" / "bin" / exe,
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``pilotage acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``pilotage_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``pilotage_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``pilotage update`` exists AND an import probe
...
# every `pilotage` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> None:
    ...
    ``pilotage_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    failure does not block every launch (``pilotage acp`` included).

    Never raises: any failure leaves the marker for the post-import path.
    """
    try:
        from pilotage_cli import _install_repair as ir
        ...
        try:
            ...
            print(
                "  The next launch will retry; pilotage will keep working from "
