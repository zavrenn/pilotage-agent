"""pilotage memory setup|status — configure memory provider plugins.
...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    if provider_name == "hindsight":
        try:
            import json
            cfg_path = get_pilotage_home() / "hindsight" / "config.json"
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as pilotage tools)
# ---------------------------------------------------------------------------
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'pilotage memory setup' to see available providers.\n")
        return
    ...
    if hasattr(provider, "post_setup"):
        ...
        provider.post_setup(pilotage_home, config)
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.pilotage/plugins/ and try again.\n")
        return
    ...
    if provider_config and hasattr(provider, "save_config"):
        try:
            provider.save_config(provider_config, pilotage_home)
...
def cmd_status(args) -> None:
    ...
    # canonical resolver (handles composite toolsets like pilotage-cli).
    ...
    if provider_name:
        ...
        if provider:
            ...
            if provider.is_available():
                ...
                if required_fields:
                    ...
                    for f in required_fields:
                        ...
                        print(line)
                print(
                    "  Note: systemd/gateway services do not inherit ~/.pilotage/.env —"
                )
                print(
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.pilotage/plugins/")
