"""
Profile management for multiple isolated Pilotage instances.

Each profile is a fully independent PILOTAGE_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.pilotage/profiles/<name>/`` by default.

The "default" profile is ``~/.pilotage`` itself — backward compatible,
zero migration needed.

Usage::

    pilotage profile create coder          # fresh profile + bundled skills
    pilotage profile create coder --clone  # also copy config, .env, SOUL.md, skills
    pilotage profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    pilotage -p coder chat                 # or via flag
    pilotage profile use coder             # set as sticky default
    pilotage profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See pilotage_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.pilotage``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   pilotage-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Pilotage infrastructure directories that only the default profile
         (``~/.pilotage``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_pilotage_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.pilotage) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "pilotage-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when PILOTAGE_HOME equals the
...
# into the export. The set below identifies the *known Pilotage profile
# artifacts* at the root of PILOTAGE_HOME; everything else is excluded.
...
_RESERVED_NAMES = frozenset({
    "pilotage", "default", "test", "tmp", "root", "sudo",
})
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the pilotage root, NOT to the current PILOTAGE_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where PILOTAGE_HOME points outside
    ``~/.pilotage``, profiles live under ``PILOTAGE_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_pilotage_home() / "profiles"


def _get_default_pilotage_home() -> Path:
    """Return the default (pre-profile) PILOTAGE_HOME path.

    In standard deployments this is ``~/.pilotage``.
    In Docker/custom deployments where PILOTAGE_HOME is outside ``~/.pilotage``
    (e.g. ``/opt/data``), returns PILOTAGE_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_pilotage_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Pilotage installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its PILOTAGE_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_pilotage_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, pilotage subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _PILOTAGE_SUBCOMMANDS:
        return f"'{canon}' conflicts with a pilotage subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            pilotage_exe = shutil.which("pilotage") or "pilotage"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(pilotage_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    try:
        ...
        token = set_pilotage_home_override(str(profile_dir))
        try:
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``pilotage -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``pilotage profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``pilotage -p X``)
...
class ProfileInfo:
    ...
    # installs). Used to route work to the right profile based on role
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``PILOTAGE_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Pilotage config
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, pilotage_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own PILOTAGE_HOME. When
    ...
    The returned ``pilotage_home`` is the path to pass to
    ``set_pilotage_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_pilotage_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``pilotage update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.pilotage)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.pilotage/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `pilotage -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Pilotage profile.\n"
    ...
    if not soul_path.exists():
        try:
            from pilotage_cli.default_soul import DEFAULT_SOUL_MD
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_pilotage_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Pilotage *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Pilotage backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``PILOTAGE_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `pilotage -p <canon> profile
    ...
    pilotage_markers = ("pilotage_cli.main", "pilotage-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Pilotage process: either an entrypoint marker in argv, or
            # a resolved executable named `pilotage`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_pilotage = (
                any(marker in joined for marker in pilotage_markers)
                or exe_name == "pilotage"
                or exe_name.startswith("pilotage")
            )
            if not is_pilotage:
                continue
            ...
            # ...or by PILOTAGE_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("PILOTAGE_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_pilotage_home
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set PILOTAGE_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("PILOTAGE_HOME")
    try:
        os.environ["PILOTAGE_HOME"] = str(profile_dir)
        from pilotage_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["PILOTAGE_HOME"] = old_home
        elif "PILOTAGE_HOME" in os.environ:
            del os.environ["PILOTAGE_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.pilotage/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: pilotage profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from PILOTAGE_HOME.

    Returns ``"default"`` if PILOTAGE_HOME is not set or points to ``~/.pilotage``.
    Returns the profile name if PILOTAGE_HOME points into ``~/.pilotage/profiles/<name>``.
    Returns ``"custom"`` if PILOTAGE_HOME is set to an unrecognized path.
    """
    from pilotage_constants import get_pilotage_home
    pilotage_home = get_pilotage_home()
    resolved = pilotage_home.resolve()

    default_resolved = _get_default_pilotage_home().resolve()
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``pilotage sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``PILOTAGE_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.pilotage itself — its parent is ~/ and its
        # directory name is ".pilotage", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: pilotage profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.pilotage itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.pilotage). "
            "Specify a different name: pilotage profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="pilotage_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"pilotage_{old_name}"
    legacy_old_host = f"pilotage.{old_name}"
    new_host = f"pilotage_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_pilotage_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("pilotage_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a PILOTAGE_HOME path string.

    Called early in the CLI entry point, before any pilotage modules
    are imported, to set the PILOTAGE_HOME environment variable.
    """
