...
def bang_shell_enabled() -> bool:
    ...
    if env_var_enabled("PILOTAGE_GATEWAY_SESSION"):
        return False
    if env_var_enabled("PILOTAGE_CRON_SESSION"):
        return False
    if (os.getenv("PILOTAGE_SESSION_PLATFORM") or "").strip():
        return False
    return True
...
def _bang_env() -> dict:
    """Environment for a bang command, with Pilotage-managed secrets filtered.
...
def run_bang_command(
    ...
    *,
    ...
    try:
        ...
        # Ctrl+C interrupts the command, not the Pilotage session.
        proc.kill()
        emit("!: interrupted")
        return 130
    finally:
