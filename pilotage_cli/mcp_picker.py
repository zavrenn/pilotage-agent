"""MCP picker — interactive `pilotage mcp picker` (also the default `pilotage mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``pilotage mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `pilotage plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from pilotage_cli.cli_output import prompt_yes_no
from pilotage_cli.curses_ui import curses_single_select
from pilotage_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Pilotage session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from pilotage_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `pilotage mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: pilotage mcp install <name>    Picker: pilotage mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Pilotage is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Pilotage — run `pilotage update` "
...
def show_catalog() -> None:
    """`pilotage mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`pilotage mcp picker` (and default `pilotage mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`pilotage mcp install <name>` — non-interactive entry-point.

    Returns 0 on success, non-zero on failure (so the CLI can propagate
    exit codes).
    """
    from pilotage_cli.mcp_catalog import get_entry

    entry = get_entry(identifier)
    if entry is None:
        print(color(
            f"  ✗ '{identifier}' is not in the catalog. "
            "Run `pilotage mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
