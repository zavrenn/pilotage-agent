"""
Gateway subcommand for pilotage CLI.

Handles: pilotage gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_pilotage_home is imported lazily at call sites to avoid ImportError
# when pilotage_constants is cached from a pre-update version during `pilotage update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "pilotage CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _scan_gateway_pids(
    ...
    current_home = str(get_pilotage_home().resolve())
    # Forward slashes on both sides of the PILOTAGE_HOME= match — see
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"pilotage_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. PILOTAGE_HOME
        ...
        # PILOTAGE_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "pilotage_home=" in command_lc
            and f"pilotage_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``pilotage update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Pilotage profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Pilotage profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "pilotage_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m pilotage_cli.main gateway
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``pilotage update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Pilotage's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose PILOTAGE_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``pilotage update`` users
    # with no running gateway until they re-invoke ``pilotage gateway``
    ...
    from pilotage_cli._subprocess_compat import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / PILOTAGE_HOME) the windowless
        # base interpreter needs to import pilotage_cli.  Empty on POSIX, where
...
def _pilotage_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``PILOTAGE_HOME`` from the on-disk unit file (not ``systemctl show``).
    ...
    for line in text.splitlines():
        ...
        if body.startswith("PILOTAGE_HOME="):
            value = body.split("=", 1)[1].strip().strip('"')
            return value or None
    return None


def _sync_pilotage_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``PILOTAGE_HOME``.

    Under ``sudo``, ``PILOTAGE_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_pilotage_home` falls back to ``/root/.pilotage`` — the wrong
    profile. The unit file pins ``PILOTAGE_HOME`` for the actual gateway
    ...
    unit_home = (_pilotage_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("PILOTAGE_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("PILOTAGE_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["PILOTAGE_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}pilotage gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}pilotage gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from pilotage_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: pilotage gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `pilotage gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``pilotage gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from pilotage_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    in-process under a ``gateway restart`` argv (pilotage_cli/gateway.py restart
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (PILOTAGE_HOME-scoped).
...
from pilotage_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Pilotage Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``pilotage gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``PILOTAGE_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("PILOTAGE_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "pilotage-gateway"
SERVICE_DESCRIPTION = "Pilotage Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current PILOTAGE_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (PILOTAGE_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from pilotage_constants import get_default_pilotage_root

    home = get_pilotage_home().resolve()
    default = get_default_pilotage_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary PILOTAGE_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(pilotage_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when PILOTAGE_HOME is a named profile.

    For ``~/.pilotage/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        pilotage_home: Optional explicit PILOTAGE_HOME path. Defaults to the current
            ``get_pilotage_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Pilotage root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_pilotage_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from pilotage_constants import get_default_pilotage_root

    home = Path(pilotage_home or str(get_pilotage_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_pilotage_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(pilotage_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".pilotage"
    try:
        Path(pilotage_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(pilotage_home, default_root=target_root)
    except ValueError:
        return _profile_arg(pilotage_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this PILOTAGE_HOME.

    Default ``~/.pilotage`` returns ``pilotage-gateway`` (backward compatible).
    Profile ``~/.pilotage/profiles/coder`` returns ``pilotage-gateway-coder``.
    Any other PILOTAGE_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Pilotage installs that predate the
# pilotage-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (pilotage-gateway-*.service) and unrelated third-party
# "pilotage" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("pilotage.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "pilotage_cli.main gateway",
    "pilotage_cli/main.py gateway",
    "gateway/run.py",
    " pilotage gateway ",
    "/pilotage gateway ",
)
...
def _find_legacy_pilotage_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Pilotage gateway units.

    Detects unit files installed by older Pilotage versions that used a
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``pilotage-gateway-coder.service`` and unrelated third-party
      ``pilotage-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``pilotage.service`` running an unrelated
...
def has_legacy_pilotage_units() -> bool:
    """Return True when any legacy Pilotage gateway unit files exist."""
    return bool(_find_legacy_pilotage_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Pilotage gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_pilotage_units()
    if not legacy:
        return
    print_warning("Legacy Pilotage gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current pilotage-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    pilotage gateway migrate-legacy")


def remove_legacy_pilotage_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Pilotage gateway unit files.

    Iterates over whatever ``_find_legacy_pilotage_units()`` returns — which is
    ...
    legacy = _find_legacy_pilotage_units()
    if not legacy:
        print("No legacy Pilotage gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Pilotage gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: pilotage gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo pilotage gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    pilotage gateway uninstall")
    print_info("    sudo pilotage gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: pilotage gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    ...
    The zero-decision service path used by ``pilotage setup`` (end of wizard)
    and ``pilotage import``: if this host supports a service manager and no
    ...
    ``pilotage gateway install --system`` explicitly (that path prompts and
    ...
    if is_container():
        ...
        print_info("   pilotage gateway run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: pilotage gateway")
        return False
    ...
    try:
        ...
        print_info("  You can retry manually: pilotage gateway install")
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Pilotage often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.pilotage`` → ``ai.pilotage.gateway.plist`` (backward compatible).
    Profile ``~/.pilotage/profiles/coder`` → ``ai.pilotage.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.pilotage.gateway-{suffix}" if suffix else "ai.pilotage.gateway"
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # pilotage_constants cached from before this symbol existed. See
            # _reload_pilotage_constants() in pilotage_cli/managed_uv.py.
            from pilotage_cli.managed_uv import _reload_pilotage_constants

            venv_python_path = _reload_pilotage_constants().venv_python_path
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.pilotage/pilotage-agent  -> /home/alice/.pilotage/pilotage-agent
      /opt/pilotage                 -> /opt/pilotage  (kept as-is)
...
def _pilotage_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current PILOTAGE_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_pilotage_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.pilotage                    → /home/alice/.pilotage
      /root/.pilotage/profiles/coder     → /home/alice/.pilotage/profiles/coder
      /opt/custom-pilotage               → /opt/custom-pilotage  (kept as-is)
    """
    current_pilotage_raw = os.environ.get("PILOTAGE_HOME", "").strip()
    current_pilotage = (
        Path(current_pilotage_raw).expanduser()
        if current_pilotage_raw
        else get_pilotage_home()
    )
    ...
    # different PILOTAGE_HOME into the generated service unit.
    current_default = Path.home() / ".pilotage"
    target_default = Path(target_home_dir) / ".pilotage"

    # Default ~/.pilotage → remap to target user's default
    if current_pilotage == current_default:
        return str(target_default)

    # Profile or subdir of ~/.pilotage → preserve the relative structure
    try:
        relative = current_pilotage.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.pilotage) — keep as-is
        return str(current_pilotage)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    pilotage_node = pilotage_home / "node" / "bin"
    if _is_dir(pilotage_node):
        candidates.append(str(pilotage_node))
    pilotage_nm = pilotage_home / "node_modules" / ".bin"
    if _is_dir(pilotage_nm):
        candidates.append(str(pilotage_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m pilotage_cli.main``, so module
    ...
    dir, or a clone that ``pilotage update`` later relocates/removes — the path
    ...
    ``PILOTAGE_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if PILOTAGE_HOME
...
def _systemd_watchdog_seconds(pilotage_home: str | Path | None = None) -> int:
    """Resolve the managed-overlay-aware watchdog setting for a service home."""
    override_token = None
    reset_home_override = None
    if pilotage_home is not None:
        from pilotage_constants import (
            reset_pilotage_home_override,
            set_pilotage_home_override,
        )

        override_token = set_pilotage_home_override(pilotage_home)
        reset_home_override = reset_pilotage_home_override
    try:
...
def _systemd_watchdog_service_fields(
    pilotage_home: str | Path | None = None,
) -> tuple[str, str]:
    """Return systemd service fields for the effective gateway config."""
    seconds = _systemd_watchdog_seconds(pilotage_home)
    if seconds <= 0:
        return "simple", ""
...
def _append_node_dir_for_service(
    path_entries: list[str], pilotage_root: Path | None = None
) -> None:
    """Add the Node directory a generated service unit should use to *path_entries*.

    The Pilotage-managed Node under ``$PILOTAGE_HOME/node`` goes first when it
    ...
    *pilotage_root* is the Pilotage home the unit will run against. System units
    ...
    """
    from pilotage_constants import iter_pilotage_node_dirs

    for directory in iter_pilotage_node_dirs(pilotage_root):
        entry = str(directory)
        try:
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's Pilotage home is known — probing here would stat the calling
    ...
    if system:
        ...
        pilotage_home = _pilotage_home_for_target_user(home_dir)
        systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
            pilotage_home
        )
        profile_arg = _profile_arg_for_target_user(pilotage_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's PILOTAGE_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(pilotage_home) if pilotage_home else _remap_path_for_user(working_dir, home_dir)
        venv_dir = _remap_path_for_user(venv_dir, home_dir)
        path_entries = [_remap_path_for_user(p, home_dir) for p in path_entries]
        # Managed Node for the TARGET user's tree (see the skip above): probe
        # the remapped pilotage_home, not the calling user's. Prepend — the
        ...
        _append_node_dir_for_service(
            _target_node_entries, Path(pilotage_home) if pilotage_home else None
        )
        path_entries = [
...
Group={group_name}
ExecStart={python_path} -m pilotage_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="PILOTAGE_HOME={pilotage_home}"
Restart=always
RestartSec=5
...
"""
    ...
    pilotage_home = str(get_pilotage_home().resolve())
    systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
        pilotage_home
    )
    profile_arg = _profile_arg(pilotage_home)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m pilotage_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="PILOTAGE_HOME={pilotage_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned PILOTAGE_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo pilotage gateway … --system``, PILOTAGE_HOME is often stripped
    # and falls back to ``/root/.pilotage``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / PILOTAGE_HOME comparisons
    ...
    _sync_pilotage_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir PILOTAGE_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved PILOTAGE_HOME
    ...
    exported a throwaway ``PILOTAGE_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``pilotage gateway restart``
    with ``PILOTAGE_HOME=/tmp/pilotage-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="PILOTAGE_HOME=..."`` lines and launchd
    ``<key>PILOTAGE_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'PILOTAGE_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>PILOTAGE_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp PILOTAGE_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: PILOTAGE_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported PILOTAGE_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single PILOTAGE_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only PILOTAGE_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp PILOTAGE_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../pilotage_test`` path into
    # ``Environment="PILOTAGE_HOME=..."``. Writing that to the developer's
    ...
    # tmp dir, and Telegram/WhatsApp all show as "not configured").
    ...
    if not system and (
        ...
        or '/pilotage_test"' in new_unit
        or "/pilotage_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # PILOTAGE_HOME into the unit (manual E2E homes like /tmp/pilotage-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Pilotage install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo pilotage gateway uninstall --system")
    print_info("         pilotage gateway install")
    print_info("         pilotage gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("PILOTAGE_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_install(
    ...
    # Offer to remove legacy units (pilotage.service from pre-rename installs)
    # before installing the new pilotage-gateway.service. If both remain, they
    ...
    if has_legacy_pilotage_units():
        print()
        print_legacy_unit_warning()
        print()
        if non_interactive or prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_pilotage_units(interactive=False)
            print()

    unit_path = get_systemd_unit_path(system=system)
    scope_flag = " --system" if system else ""

    # Existing system units already pin PILOTAGE_HOME; adopt it before any
    ...
    # ``sudo pilotage gateway install --system --force`` would bake /root/.pilotage
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}pilotage gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}pilotage gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}pilotage gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # PILOTAGE_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `pilotage gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `pilotage gateway status` or logs for final state."
            )
            return
        _wait_for_systemd_service_restart(system=system, previous_pid=pid)
        return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}pilotage gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}pilotage gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}pilotage gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}pilotage gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.pilotage.gateway-{suffix}" if suffix else "ai.pilotage.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``pilotage gateway start/stop/status`` call).
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_pilotage_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_pilotage_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m pilotage_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active PILOTAGE_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "pilotage_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    working.
    """
    from pilotage_cli._subprocess_compat import windows_detach_popen_kwargs

    log_dir = get_pilotage_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup pilotage gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    # without this block every plist rewrite (e.g. `pilotage gateway start`)
    ...
    nofile_block = ""
    try:
        from pilotage_cli.resource_limits import configured_nofile_soft_limit
...
<dict>
    ...
    <dict>
        ...
        <key>PILOTAGE_HOME</key>
        <string>{pilotage_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.pilotage/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See pilotage-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_pilotage_home() / "logs" / "launchd-reload.log"
        try:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Pilotage install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  pilotage gateway status             # Check status")
    from pilotage_constants import display_pilotage_home as _dhh
...
def launchd_stop():
    ...
    # `pilotage gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    # Pilotage PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Pilotage PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Pilotage install")
    else:
        print("⚠ Service definition is stale relative to the current Pilotage install")
        print("  Run: pilotage gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: pilotage gateway stop")
            else:
    ...
    if deep:
        log_file = get_pilotage_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``PILOTAGE_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``pilotage gateway run`` is always a misconfiguration, so we
    ...
    try:
        from pilotage_constants import get_default_pilotage_root
        default_root = get_default_pilotage_root()
    ...
    try:
        ...
        from pilotage_cli.profiles import normalize_profile_name
    ...
    print()
    print("    pilotage gateway restart")
    print()
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``pilotage gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``pilotage gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other PILOTAGE_HOME roots.
    """
    ...
    print("  Use 'pilotage gateway restart' to replace it,")
    print("  or 'pilotage gateway stop' first.")
    print("  Or use 'pilotage gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("PILOTAGE_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Pilotage gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'pilotage' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/pilotage/docker/entrypoint.sh before the Pilotage command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$PILOTAGE_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set PILOTAGE_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `pilotage gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set PILOTAGE_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `pilotage gateway restart` which already
    ...
    # `pilotage gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Pilotage Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # PILOTAGE_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("PILOTAGE_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
    ...
    # the env vars ``PILOTAGE_GATEWAY_MAX_STARTS`` /
    # ``PILOTAGE_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("PILOTAGE_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("PILOTAGE_GATEWAY_START_WINDOW_S")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``pilotage gateway run`` enters through this CLI wrapper, not through
...
# Every shipped platform now lives in ``plugins/platforms/<name>/`` and is
# discovered through the platform registry, so there are no built-in catalog
# entries left. Kept as an (empty) seed for ``_all_platforms``.
...
def _all_platforms() -> list[dict]:
    ...
    platforms auto-load via ``kind: platform`` and appear in
    ``pilotage setup gateway`` without needing the gateway to be running.
    ...
    # User-installed platform plugins under ~/.pilotage/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_pilotage_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    """Interactive setup for a platform declaring a ``vars`` schema."""
    # Same hidden-knob list the dashboard/Desktop channel cards use.
    from pilotage_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from pilotage_cli.telegram_managed_bot import (
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'pilotage pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: pilotage pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped — configure later with 'pilotage gateway setup'"
                    )
            continue
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``pilotage_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from pilotage_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    Bundled platform plugins auto-load, so no plugin enable step
    is needed here. User-installed platform plugins under ~/.pilotage/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.pilotage/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if supports_systemd_services() and has_legacy_pilotage_units():
        print_legacy_unit_warning()
        print()
    ...
    if any_configured:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: pilotage gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: pilotage gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo pilotage gateway install --system"
                        )
                    print_info("  Or run in foreground:  pilotage gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: pilotage gateway run")
                print_info(
                    "  For persistence:   tmux new -s pilotage 'pilotage gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: pilotage gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup pilotage gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'pilotage gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`pilotage_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from pilotage_cli.service_manager import (
    ...
    if profile is None:
        ...
        # PILOTAGE_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``pilotage gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``pilotage gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``pilotage gateway run`` are unaffected.
      2. ``PILOTAGE_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``pilotage gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``PILOTAGE_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("PILOTAGE_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("PILOTAGE_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${PILOTAGE_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if PILOTAGE_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or PILOTAGE_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-pilotage service's
    # pattern in docker/s6-rc.d/main-pilotage/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: pilotage gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: pilotage gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s pilotage 'pilotage gateway run'"
                )
                print()
            ...
            print()
            print(
                "  pilotage gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s pilotage 'pilotage gateway run'         # persistent via tmux"
            )
            print(
                "  nohup pilotage gateway run > ~/.pilotage/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  pilotage profile create <name>     # creates the s6 service slot")
                print("  pilotage -p <name> gateway start   # bring it up via s6")
                print("  pilotage status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: pilotage gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: pilotage gateway run")
            sys.exit(1)
        ...
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  pilotage profile delete <name>     # tears down the s6 service slot")
                print("  pilotage -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `pilotage -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: pilotage gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_PILOTAGE_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `pilotage gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_PILOTAGE_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `pilotage gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: pilotage gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  pilotage gateway install")
                else:
                    print("To install as a service:")
                    print("  pilotage gateway install")
                    print("  sudo pilotage gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  pilotage gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup pilotage gateway run > ~/.pilotage/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s pilotage 'pilotage gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  pilotage gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  pilotage gateway install  # Install as user service")
                    print(
                        "  sudo pilotage gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Pilotage gateway unit files from
        # pre-rename installs (e.g. pilotage.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_pilotage_units(interactive=not yes, dry_run=dry_run)
