"""CLI subcommand: ``pilotage send`` — pipe text from shell scripts to any
configured messaging platform (Telegram, WhatsApp).
...
* For platforms that send via bot token (Telegram, WhatsApp-CloudAPI) no
  running gateway is required. The tool talks directly to each platform's
  REST endpoint. For platforms that rely on a persistent adapter connection
  (WhatsApp via its plugin) a live gateway is needed; the underlying tool
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"pilotage send: {file_path} is not a text file. --file reads the "
                ...
                f'  pilotage send --to telegram "MEDIA:{file_path}"\n'
                f'  pilotage send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  pilotage send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"pilotage send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"pilotage send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"pilotage send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"pilotage send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    # setup used only for outbound `pilotage send`) would otherwise be
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"pilotage send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `pilotage gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.pilotage/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_pilotage_env() -> None:
    """Populate ``os.environ`` from ``~/.pilotage/.env`` AND bridge top-level
    ...
    startup that ``pilotage send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.pilotage/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.pilotage/config.yaml`` into
       ...
       via ``pilotage config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``pilotage send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.pilotage/.env and ~/.pilotage/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "pilotage send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  pilotage send --to telegram \"hello\"\n"
            "  pilotage send --to whatsapp --file report.md\n"
            "  pilotage send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "pilotage send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `pilotage send --help` stays fast and does not pull in
    # the full tool registry / gateway config stack.
    from tools.send_message_tool import send_message_tool

    # send_message_tool auto-loads gateway config + env and routes to the
    # appropriate platform adapter (bot-token path for Telegram and
    # WhatsApp Cloud; live-adapter path for plugin platforms).
    #
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Pilotage "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.pilotage/.env + ~/.pilotage/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram."
        ),
        epilog=(
            "Examples:\n"
            "  pilotage send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | pilotage send --to telegram:-1001234567890\n"
            "  pilotage send --to whatsapp --file /tmp/report.md\n"
            "  pilotage send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  pilotage send --list                  # all platforms\n"
            "  pilotage send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "telegram:-1001234567890:17585, whatsapp:+15551234567."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `pilotage send --list telegram`.",
    )
