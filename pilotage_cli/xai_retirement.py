...
Pure logic: walks a Pilotage config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `pilotage doctor` and a future `pilotage migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Pilotage config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Pilotage config and return retirement issues.
