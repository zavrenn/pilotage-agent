...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, pilotage_bin
        cli_args: the original CLI arguments (everything after 'pilotage')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    pilotage_bin = container_info["pilotage_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo pilotage {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo pilotage {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _try_fast_chat_launch() -> bool:
    ...
    try:
        from pilotage_cli.config import get_container_exec_info
