...
configured for "cli" in `pilotage tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (PILOTAGE_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `pilotage chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"pilotage -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "pilotage -z: --toolsets all enables every toolset; "
    ...
    if unresolved:
        try:
            ...
            from pilotage_cli.tools_config import _parse_enabled_flag
    ...
    if unknown:
        sys.stderr.write(f"pilotage -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "pilotage -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "pilotage -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to PILOTAGE_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("PILOTAGE_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "pilotage -z: --provider requires --model (or PILOTAGE_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"pilotage -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write("pilotage -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``pilotage -z`` / oneshot mode.

    Oneshot bypasses ``PilotageCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when pilotage is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from pilotage_cli.config import load_config
    from pilotage_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("PILOTAGE_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from pilotage_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("PILOTAGE_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    # PilotageCLI._init_agent's wait — it builds AIAgent directly here, so the
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                PILOTAGE_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via PILOTAGE_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via PILOTAGE_YOLO_MODE=1
