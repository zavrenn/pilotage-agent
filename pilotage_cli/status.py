...
def show_status(args):
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: pilotage portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: pilotage auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: pilotage auth add xai-oauth)'}"
    )
    ...
    if _effective_provider_label() == "LM Studio":
        from pilotage_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'pilotage-agent[vercel]')"
