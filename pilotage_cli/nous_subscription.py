...
from pilotage_cli.config import get_env_value, load_config
...
# coverage category (pilotage_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `pilotage tools` picker scope its entitlement gate to the selected backend, so a
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # Pilotage-managed Node dirs (Windows installer / POSIX $PILOTAGE_HOME/node)
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `pilotage model`, so direct credentials should NOT
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from pilotage_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`pilotage tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``pilotage tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``pilotage model``'s Nous
    login, this:
