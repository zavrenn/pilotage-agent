"""``pilotage sessions`` command — extracted from ``pilotage_cli/main.py``.
...
Helpers that stay in ``pilotage_cli.main`` (``get_pilotage_home``,
...
``pilotage_cli.main.<name>`` keep reaching this code path, and so imports stay
...
def _m():
    """Lazy ``pilotage_cli.main`` reference (call-time, keeps patches working)."""
    from pilotage_cli import main

    return main


def get_pilotage_home():
    return _m().get_pilotage_home()
...
#: Default age floor for `pilotage sessions prune --never-active`.  Deliberately
...
def _prune_never_active_keyed(db, args):
    ...
    """
    from pilotage_cli.session_filters import format_epoch, parse_duration_seconds
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    pilotage sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "recover":
        ...
        from pilotage_cli.session_recovery import (
    ...
    if action == "list":
        from pilotage_state import workspace_key as _ws_key
        ...
        from pilotage_cli.session_filters import (
            build_prune_filters,
            describe_filters,
        )
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = (
                        ...
                        else get_pilotage_home() / "session-exports"
                    )
        ...
        # Markdown / QMD export
        from pilotage_cli.session_export_md import (
        ...
        if args.output == "-":
            ...
            db.close()
            return
        output_dir = Path(args.output).expanduser() if args.output else get_pilotage_home() / "session-exports"
        ...
        # `pilotage sessions prune`: no time window and no filters at all
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`pilotage sessions optimize` later to reclaim freed space.)")
