...
def active_session_limit_message(
    ...
    return (
        f"Pilotage is at the active session limit ({active_count}/{max_sessions})."
        f"{detail} Try again when another session finishes."
    )


def _state_dir() -> Path:
    return Path(get_pilotage_home()) / "runtime"
...
class ActiveSessionLease:
    ...
    # root ``PILOTAGE_HOME`` must release against the same registry even when
...
def release_orphaned_leases(live_lease_ids: set[str]) -> int:
    ...
    that runs for days (``pilotage dashboard`` / ``serve``) never trips that
