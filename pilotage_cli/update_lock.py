"""Cross-process mutual exclusion for in-flight Pilotage updates.

Both the terminal and dashboard can start ``pilotage update`` against the same
...
under each other. The lock format is intentionally small and inspectable:
    ...
    <PILOTAGE_HOME>/.pilotage-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
MARKER_NAME = ".pilotage-update-in-progress"
...
def update_marker_path() -> Path:
    ...
    Uses the process Pilotage home, never a context-local profile override, so
    every update entrypoint coordinates through one lock.
    """
    from pilotage_constants import get_process_pilotage_home

    return get_process_pilotage_home() / MARKER_NAME
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Pilotage update is already running (PID {holder.pid}, "
