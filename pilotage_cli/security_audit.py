"""On-demand supply-chain audit for Pilotage Agent installs.

Scans three surfaces a Pilotage user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Pilotage venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.pilotage/plugins``
...
def _discover_plugins(pilotage_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.pilotage/plugins``.
    ...
    """
    plugins_dir = pilotage_home / "plugins"
...
def _discover_components(
    *,
    ...
    home = pilotage_home or Path(get_pilotage_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `pilotage security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, pilotage_home=home
    )
