#!/usr/bin/env python3
"""
Hermes file operations extracted for Pilotage's local Linux shell.

The read, write, patch and search mechanisms remain Hermes' implementation.
Pilotage intentionally wires only its existing local ``Shell``; Hermes' remote
and container environment management is not part of this runtime.
"""
...
def _lint_toml_inproc(content: str) -> tuple[bool, str]:
    ...
    except ImportError:  # Python 3.10, which Pilotage still supports
...
class ShellFileOperations(FileOperations):
    ...
    Pilotage supplies its local Linux ``Shell`` with execute(command, cwd).
    """
    ...
    def _has_command(self, cmd: str) -> bool:
        ...
        if cmd not in self._command_cache:
            # ``command -v`` can find an unusable executable mounted from the
            # Windows host into WSL. Probe execution too before selecting rg
            # over the working grep fallback.
            result = self._exec(
                f"command -v {cmd} >/dev/null 2>&1 "
                f"&& {cmd} --version >/dev/null 2>&1 && echo 'yes'"
            )
    ...
    def _search_with_grep(self, pattern: str, path: str, file_glob: Optional[str],
        ...
        # Match ripgrep for hidden files too (.env, transaction backups).
        # GNU grep chooses its default include/exclude state from the first
        # file filter, so this must follow --include when a glob is present.
