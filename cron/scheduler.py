...
Uses a file-based lock (~/.pilotage/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `pilotage update` reloads
# the module) fail with ModuleNotFoundError for pilotage_time et al.
...
. This callback retrieves and closes that eventual late result.
    """
    try:
        db = future.result()
        if db is not None:
...
def _set_cron_session_title(session_db, session_id, base_title):
    ...
    the connection down (issues,):

    -: never leaves the session blank. base_title already carries a
      cron-id fallback for nameless jobs; this also guards a failed write.
    -: a duplicate title makes set_session_title raise ValueError (the
    ...
    -: this runs synchronously in the cron finally block ahead of the
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    # fallback was attempted @jbagdonas, @daxro).
    ...
    # Match 429 as a whole token @cation98): bare substring matching
    ...
    # upstream applied for script timeouts vs provider timeouts
    ...
    # Sibling scheduler-side timeout: the TERMINAL_CWD lock-wait
...
class CronPromptInjectionBlocked(Exception):
    ...
    gap from: create-time scanning only covers the user-supplied
...
def _resolve_cron_disabled_toolsets(cfg: dict) -> list[str]:
    ...
    ordinary agent runs — LLM-supplied enabled_toolsets was widening
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy pilotage_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from pilotage_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    1. Per-job ``enabled_toolsets`` (set via ``cronjob`` tool on create/update).
       Keeps the agent's job-scoped toolset override intact. Enabled
    ...
    2. Per-platform ``pilotage tools`` config for the ``cron`` platform.
    ...
    try:
        from pilotage_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _is_cron_silence_response(text: str) -> bool:
    ...
    variants the model emits when it drops the brackets.
...
# Token keying keeps an interruption scoped to that exact
...
def try_register_running_job(job_id: str) -> bool:
    ...
    and run the same job concurrently (idea from by @izumi0uu).

    Registration also makes the run visible to ``get_running_job_ids`` (the
    gateway shutdown drain,) and ``mark_running_jobs_interrupted``.
...
def _inflight_min_allowance_minutes() -> float:
    ...
    ``PILOTAGE_CRON_INFLIGHT_MAX_MINUTES`` env var is kept as an internal
    escape hatch only.
    """
    try:
        ...
        pass
    raw = os.getenv("PILOTAGE_CRON_INFLIGHT_MAX_MINUTES", "").strip()
    if raw:
        try:
            val = float(raw)
            if val > 0:
            ...
            logger.warning(
                "Invalid PILOTAGE_CRON_INFLIGHT_MAX_MINUTES=%r; using default %s",
                raw,
...
def _record_forced_release(job_id: str, name: str, age_seconds: float, allowance_seconds: float) -> None:
    ...
    entry = {
        ...
        "at": _pilotage_now().isoformat(),
    }
    with _running_lock:
        _forced_releases.append(entry)
        del _forced_releases[:-_FORCED_RELEASE_HISTORY]
    try:
        path = _get_pilotage_home() / "cron" / "inflight_forced_releases.jsonl"
    ...
    *,
    ...
    with _running_lock:
        ...
        if only_owners is None:
            active_fires.extend(
                (None, job_id, None, _get_pilotage_home())
    ...
    for _token, job_id, fire_owner, profile_home in active_fires:
        if not fire_owner:
            ...
            # The in-memory interrupt flag WAS recorded above —
...
# FAILING (fail-closed,). Derived from the cron inactivity limit
# (PILOTAGE_CRON_TIMEOUT, default 600s): a wedged lock holder stops touching
...
def _cron_inactivity_seconds() -> float:
    """Parse PILOTAGE_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600.
    ...
    """
    raw = os.getenv("PILOTAGE_CRON_TIMEOUT", "").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
    except (ValueError, TypeError):
        logger.warning("Invalid PILOTAGE_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
# Resolves through _get_pilotage_home() so profile-scoped paths work correctly.
def _usage_audit_path() -> Path:
    return _get_pilotage_home() / "cron" / "usage_audit.jsonl"
...
def _write_usage_audit(record: dict) -> None:
    """Append a single JSONL line to ~/.pilotage/cron/usage_audit.jsonl.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``pilotage update`` / ``pilotage gateway stop`` / systemd restart, or an
    ...
    this to skip gracefully with a warning instead of crashing,
   ).
    ...
    if exc is not None:
        ...
        # ThreadPoolExecutor). Both are documented in. The common prefix
...
_pilotage_home: Path | None = None


def _get_pilotage_home() -> Path:
    """Resolve Pilotage home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design: the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active PILOTAGE_HOME at call time
    ...
    """
    return _pilotage_home or get_pilotage_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    pilotage_home = _get_pilotage_home()
    lock_dir = pilotage_home / "cron"
...
def _resolve_origin(job: dict) -> Optional[dict]:
    ...
    crashed identically until the field was patched manually.
    """
...
def _maybe_mirror_cron_delivery(
    ...
    *,
    ...
    try:
        ...
        # alternation (, the exact failure removed). A
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"Pilotage — {task_name}"
    try:
    ...
    try:
        ...
        if session_store is not None:
            ...
            if platform_enum is not None:
                # Telegram uses chat_id == parent_channel for thread messages,
                # so the parent chat_id is correct. See the matching guard in
                # GatewayRunner._process_handoff.
...
def _get_config_home_channel(platform_name: str):
    ...
    ``/sethome`` declares ``config.yaml`` canonical and mirrors the value into
    the legacy ``<PLATFORM>_HOME_CHANNEL`` env var only as a best-effort
    compatibility shim.  Cron historically read ONLY the env mirror, so a home
    channel that existed solely in config.yaml was invisible and jobs silently
    fell back to local-only.  Reading the canonical store here fixes that.
    """
    try:
...
def _get_home_target_thread_id(platform_name: str) -> Optional[str]:
    ...
    the lobby reminder and drops ``reply_to_message_id``. Pointing
...
def _confirm_adapter_delivery(send_result) -> bool:
    ...
    gateway never actually sees the message.
...
def _is_channel_dm_topic(
    ...
    identical for both cases, so shape alone cannot decide (this was the
    ...
    probe, or any probe error/timeout — that is the pre- behaviour and the
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    if not targets:
        ...
        # "no delivery target resolved" error on every run. The output
    ...
    for target in targets:
        ...
        # Prefer the resolved live transport when the gateway is running.
        ...
        if live_adapter_ready:
            # Telegram topic routing (, regression fixed): a
            ...
            if route_via_dm_topic:
                # Genuine Bot API channel Direct-Messages topic mode 2):
                ...
                # target: route via message_thread_id. Put thread_id in
            ...
            try:
                ...
                if text_to_send:
                    ...
                    if future is None:
                        ...
                        try:
                            ...
                            #: a slow confirmation does NOT necessarily
                ...
                # — media previously used a bare thread_id and landed in
                ...
                # payload is already assumed delivered. Record the
        ...
        if not delivered:
            ...
            try:
                ...
                # remaining target. Wrap the fallback in its own
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("PILOTAGE_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid PILOTAGE_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(
    ...
    Scripts must reside within PILOTAGE_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Pilotage-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against PILOTAGE_HOME/scripts/.  Absolute and ~-prefixed paths
...
.
    ...
    """
    scripts_dir = _get_pilotage_home() / "scripts"
    ...
    # escape — scripts MUST reside within PILOTAGE_HOME/scripts/.
    try:
...
def _parse_wake_gate(script_output: str) -> bool:
    ...
    The convention (ported from nanoclaw): if the last stdout line is
...
def _build_job_prompt(
    ...
    Args:
        ...
        extra_prompt: Optional per-run context (from ``cronjob(action='run')``,
            — salvaged from by @liuhao1024). Appended to the
...
def _scan_assembled_cron_prompt(
    ...
    *,
    ...
    Plugs the gap: ``_scan_cron_prompt`` runs on the user-supplied
...
# Marker prefix for a drift-guard skip. Same alert-once contract as
...
# deliver again" (the drift_alerted bit on the job record, shape).
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    model = job.get("model") or os.getenv("PILOTAGE_MODEL") or ""
    ...
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `pilotage setup`), or pin a "
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            return (
                ...
                "`pilotage setup` or change the job's `deliver` target."
            )
    return None
...
def _preflight_job_config(job: dict, cfg: dict) -> Optional[str]:
    ...
    Same fail-before-spend spirit as the drift guard and the
    fail-loud-on-hidden-tools direction in; alert dedup follows the
    alert-once pattern from the dead-pin auto-pause.
    """
    for name, check in (
...
def run_job(
    ...
    *,
    ...
    This closes the ordering window in where delivery ran against a
    ...
    ``extra_prompt``: optional per-run context from ``cronjob(action='run',
    prompt=..)``. Appended to the stored prompt for this fire only —
    ...
    if job.get("no_agent"):
        ...
        # "no delivery target resolved". load_pilotage_dotenv does not override
        # already-set vars, so the gateway's in-process tick is unaffected.
        try:
            from pilotage_cli.env_loader import load_pilotage_dotenv

            load_pilotage_dotenv(pilotage_home=_get_pilotage_home())
        except Exception:
            logger.debug(
        ...
        # bug where os.chdir leaks into concurrent gateway sessions.
        ...
        now_iso = _pilotage_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if job_has_monitor(job):
        ...
        _mon_now = _pilotage_now().strftime("%Y-%m-%d %H:%M:%S")
        if not _mon.ok:
    ...
    #
    # Bounded with its own timeout (separate from PILOTAGE_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("PILOTAGE_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid PILOTAGE_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
        ...
        if _session_db_timeout > 0:
            ...
            try:
                ...
                # and closes any eventual late result.
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_pilotage_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    _cron_session_id = f"cron_{job_id}_{_pilotage_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # gateway message. Do not seed PILOTAGE_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads PILOTAGE_SESSION_PLATFORM
    #     and PILOTAGE_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via PILOTAGE_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     PILOTAGE_SESSION_PLATFORM == "telegram".
    ...
    #     PILOTAGE_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read PILOTAGE_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the PILOTAGE_CRON_AUTO_DELIVER_* vars set
    # below, so clearing PILOTAGE_SESSION_* here does not affect delivery.
    ...
    # internally). This avoids a separate import/set/clear dance.
    _job_workdir = (job.get("workdir") or "").strip() or None
    if _job_workdir and not Path(_job_workdir).is_dir():
        logger.warning(
            "Job '%s': configured workdir %r no longer exists — running without it",
    ...
    _ctx_tokens = set_session_vars(
        ...
        # PILOTAGE_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ PILOTAGE_SESSION_KEY, which risks routing a cron subagent's output
        ...
        # See declare_stateless_channel. Upstream:,.
        async_delivery=False,
        cwd=_job_workdir or "",
    )
    _cron_delivery_vars = (
        "PILOTAGE_CRON_AUTO_DELIVER_PLATFORM",
        "PILOTAGE_CRON_AUTO_DELIVER_CHAT_ID",
        "PILOTAGE_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    # their own cwd, not the cron's workdir.
    ...
    try:
        ...
        # load_pilotage_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this PILOTAGE_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # Clearing the cache forces the re-pull; the resolved secret
        ...
        # forced re-pull off the network. load_pilotage_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from pilotage_cli.env_loader import (
        ...
        if delivery_target:
            _VAR_MAP["PILOTAGE_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["PILOTAGE_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["PILOTAGE_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        # cron-fleet default) > PILOTAGE_MODEL env > config.yaml ``model:``
        ...
        # axis resolves from here, the drift guard is skipped for that
        ...
        try:
            ...
            _cfg_path = str(_get_pilotage_home() / "config.yaml")
        ...
        # model otherwise reaches the provider as an opaque 400.
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                f"Cron job '{job_name}' has no model configured "
                f"(job.model={job.get('model')!r}, "
                f"PILOTAGE_MODEL={os.getenv('PILOTAGE_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`cronjob action=update job_id={job_id} model=<name>` or set a "
                "default with `pilotage model <name>`."
            )

        # Apply IPv4 preference if configured.
        try:
            from pilotage_constants import apply_ipv4_preference
        ...
        prefill_file = (
            os.getenv("PILOTAGE_PREFILL_MESSAGES_FILE", "")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_pilotage_home() / pfpath
            if pfpath.exists():
                try:
        ...
        # `preflight_alerted` bit — the alert-once shape).
        ...
        # Provider/model-drift fail-closed guard.
        #
        ...
        if cron_model_drift_guard_enabled(_cfg):
            ...
            if _drift:
                ...
                # Lifecycle-aware remediation (, @sashmatash): a finite
                ...
                # Alert-once shape): persist the drift_alerted bit so
        ...
        # override via PILOTAGE_CRON_TIMEOUT env var.  0 = unlimited.
        #
        ...
        _POLL_INTERVAL = 5.0
        # Keep the one-shot run_claim fresh while the run is alive:
        ...
        # builds the proper failure tuple. 
        ...
        # replaces a blank/empty model turn with a "⚠️ No reply: …"
        ...
        # and soft-failure marking below apply — restoring pre- silence
        ...
        if final_response.strip() and turn_exit_reason:
            ...
            _explainer_variants = []
            try:
                from pilotage_state import PERSISTENCE_ERROR_CAUSES as _causes
            except Exception:
...
**Job ID:** {job_id}
**Run Time:** {_pilotage_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            # close can never run over an in-flight title write. The
            # run-time suffix keeps it unique against the sessions.title index
            # across runs; _set_cron_session_title dedupes and the
            # except-fallback below guarantees a non-blank title.
            try:
                _title_base = " ".join(job_name.split())[:60].strip() or f"cron {job_id}"
                _cron_title = f"{_title_base} · {_pilotage_now().strftime('%b %d %H:%M')}"
                ...
                # Last-resort: never leave the session blank. Try the
        ...
        # until it hits EMFILE / "too many open files").
        #
        ...
        # live async client, and the caller tears down afterwards.
...
def _teardown_cron_agent(
    ...
    (to deliver first) can invoke the identical cleanup AFTER delivery.
...
def run_one_job(
    ...
    *,
    ...
    profile_home = _get_pilotage_home().resolve()
    with _running_lock:
...
def _run_one_job_body(
    ...
    *,
    ...
    delivery_error = None
    try:
        # Pre-run dispatch claim : atomically commit a finite
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_pilotage_home())
        )
        # Defer the cron agent's async-resource teardown until AFTER delivery.
        # run_job normally closes the agent (and reaps stale async clients) in
        # its finally block; doing that before _deliver_result runs means the
        # live send races a torn-down async client. Passing a holder
        ...
        try:
            ...
            #, then re-raise into the outer handler. BaseException
        ...
        # swallow the error and leak the agent's subprocesses/clients.
        ...
        try:
            ...
            # mid-flight, the agent may still have produced a
            ...
            # the same alert every tick would be spam alert-once
            # shape).
            ...
            # Drift-guard skip: same alert-once contract as
            ...
            # a real report that merely quoted "[SILENT]" mid-sentence,
            #). Keeps the intentional bracketed-prefix / trailing-line
        ...
        if interrupted:
            if delivery_error:
                ...
                # adapters were torn down first,). Record the delivery
        ...
        # BaseException, not Exception: the inner run_job handler
...
# Dead-owner claim reclaim throttle: recover_interrupted_executions
...
def tick(
    ...
    *,
    ...
    try:
        # Global emergency stop (`pilotage pause`): skip dispatch entirely while
        # the ESTOP sentinel exists. Never touches in-flight runs — due jobs
        # simply wait for the next tick after `pilotage resume`. Logged once per
        ...
        # Dead-owner claim reclaim: execution rows carry their owner
        # pid + process start time, but recovery previously ran only at
        # scheduler STARTUP. A one-shot `pilotage cron run` that claimed a job
        ...
        # 60s ticks don't pay a ledger connection every cycle.
        ...
        if not due_jobs:
            ...
            # — the gateway ticker calls tick(verbose=False) every
        ...
        if verbose:
            logger.info("%s - %s job(s) due", _pilotage_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set PILOTAGE_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("PILOTAGE_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid PILOTAGE_CRON_MAX_PARALLEL value; defaulting to unbounded")
