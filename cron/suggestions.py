...
A *suggestion* is a ready-to-run cron job spec that Pilotage surfaces to the
...
Storage mirrors ``cron/jobs.py``: ``~/.pilotage/cron/suggestions.json``, atomic
...
def _save_raw(suggestions: List[Dict[str, Any]]) -> None:
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            json.dump(
                {"suggestions": suggestions, "updated_at": _pilotage_now().isoformat()},
                f,
                indent=2,
            )
            f.flush()
    ...
    *,
    ...
    with _suggestions_lock:
        ...
        record = {
            ...
            "created_at": _pilotage_now().isoformat(),
        }
    ...
    with _suggestions_lock:
        ...
        for s in suggestions:
            if s.get("id") == suggestion_id:
                s["status"] = status
                s["resolved_at"] = _pilotage_now().isoformat()
                changed = True
                break
        if changed:
            _save_raw(suggestions)
