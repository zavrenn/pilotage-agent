...
or ``systemctl restart pilotage-gateway``).  When the cron fires, the
...
``pilotage cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``pilotage gateway``, ``launchctl ... pilotage-gateway``,
``systemctl ... pilotage-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_PILOTAGE_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``pilotage gateway
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `pilotage gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:pilotage\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a pilotage-gateway label. macOS launchd
    # labels look like `ai.pilotage.gateway` / `pilotage-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated pilotage services (e.g.
    # `launchctl unload ai.pilotage.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.pilotage.gateway-<suffix> --
    ...
    # Branch C: systemctl ops on a pilotage-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bpilotage[.\-]?gateway)"
    # Branch D: pkill / kill targeting the pilotage gateway process. Both
...
# for `systemctl restart pilotage-gateway` in syslog, a SQL LIKE literal over a
...
# to execution (`grep 'systemctl restart pilotage-gateway' f | sh`); never mask
# such a line.
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart pilotage-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        grep -c 'systemctl restart pilotage-gateway' /var/log/syslog
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart pilotage-gateway%'"
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    under ``<PILOTAGE_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.pilotage/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_pilotage_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_pilotage_home() falls back to Path.home(), which raises when
        # neither PILOTAGE_HOME nor HOME is resolvable (launchd/systemd
...
def _read_script_for_scanning(script_path: str) -> str:
    ...
    if unsafe:
        return "pilotage gateway restart"
