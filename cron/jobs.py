...
Jobs are stored in ~/.pilotage/cron/jobs.json
Output is saved to ~/.pilotage/cron/output/{job_id}/{timestamp}.md
"""
...
# `get_pilotage_home()` (the active profile home), NOT `get_default_pilotage_root()`
...
# threshold in `pilotage cron status` (pilotage_cli/cron.py), so the two never
# drift apart.
...
def _current_cron_store() -> _CronStorePaths:
    ...
    3. the ACTIVE profile home, resolved fresh via get_pilotage_home()
       (context-local override, then the PILOTAGE_HOME env var) — so a test
       or embedder that re-points PILOTAGE_HOME after this module was
...
# TTL only recovers a claim left by a tick that DIED mid-run. PILOTAGE_CRON_TIMEOUT
...
def _oneshot_run_claim_ttl_seconds() -> float:
    ...
    Derived from ``PILOTAGE_CRON_TIMEOUT`` (the cron inactivity timeout the
...
def _jobs_lock():
    ...
    and standalone ``pilotage`` CLI invocations, which previously shared no lock
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED Pilotage timezone, not the server's local
            # timezone. The due-check (`get_due_jobs`) compares `next_run_at`
            # against `pilotage_time.now()`, which uses the configured zone. If a
    ...
    try:
        ...
        run_at = _pilotage_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Pilotage configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Pilotage timezone.

    This preserves relative ordering for legacy naive timestamps across
    timezone changes and avoids false not-due results.
    """
    target_tz = _pilotage_now().tzinfo
    if dt.tzinfo is None:
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Pilotage/system local time
    ...
    if kind == "once":
        ...
        if not _ensure_croniter():
            logger.warning(
                ...
                "reinstall pilotage-agent or run 'pip install croniter' in your "
                "runtime env.",
                expr,
            )
            return None
...
# =============================================================================
# Ticker heartbeat (liveness signal for `pilotage cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    in another process (``pilotage cron status``) never sees a torn/truncated
    ...
    try:
        for _attempt in range(5):
            ...
            try:
                with os.fdopen(fd, "w", encoding="utf-8") as f:
                    json.dump(
                        {"jobs": jobs, "updated_at": _pilotage_now().isoformat()},
                        f,
                        indent=2,
                        ensure_ascii=False,
                    )
...
def _resolve_default_model_snapshot() -> Optional[str]:
    ...
    """
    try:
        from pilotage_cli.config import _expand_env_vars, read_user_config_raw

        cfg_path = get_pilotage_home() / "config.yaml"
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.pilotage/scripts/; ``.sh`` / ``.bash`` files run via bash,
        ...
        monitor_script: Optional path to a cheap monitor source script (same
                ...
                ~/.pilotage/scripts/, .sh/.bash via bash, else Python). Each
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _pilotage_now().isoformat(),
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _pilotage_now().isoformat(),
        },
    )
...
def _write_wedged_oneshot_diagnostic(job: Dict[str, Any]) -> None:
    ...
    try:
        ...
        text = (
            ...
            f"- removed at: {_pilotage_now().isoformat()}\n\n"
    ...
    with _jobs_lock():
        ...
        for job in jobs:
            ...
            if not isinstance(claim, dict) or claim.get("by") != expected_owner:
                return False
            claim["at"] = _pilotage_now().isoformat()
            save_jobs(jobs)
            return True
    return False
...
def _machine_id() -> str:
    ...
    Uses ``PILOTAGE_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
    comes from the file lock + the fresh-claim check, not from this value.
    """
    explicit = os.getenv("PILOTAGE_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    # (derived from PILOTAGE_CRON_TIMEOUT). See _oneshot_run_claim_ttl_seconds.
    ...
    for job in jobs:
        ...
        try:
            ...
            if next_run_dt <= now:
                ...
                # (_oneshot_run_claim_ttl_seconds, derived from PILOTAGE_CRON_TIMEOUT),
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _pilotage_now().strftime("%Y-%m-%d_%H-%M-%S")
...
def _canonical_skill_ref(raw: Any) -> str:
    ...
    A job may store an absolute path under ``PILOTAGE_HOME/skills`` or an
